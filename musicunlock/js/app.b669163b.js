(function(A) {
	function t(t) {
		for (var g, r, I = t[0], o = t[1], a = t[2], C = 0, Q = []; C < I.length; C++) r = I[C], Object.prototype.hasOwnProperty.call(i, r) && i[r] && Q.push(i[r][0]), i[r] = 0;
		for (g in o) Object.prototype.hasOwnProperty.call(o, g) && (A[g] = o[g]);
		B && B(t);
		while (Q.length) Q.shift()();
		return n.push.apply(n, a || []), e()
	}

	function e() {
		for (var A, t = 0; t < n.length; t++) {
			for (var e = n[t], g = !0, I = 1; I < e.length; I++) {
				var o = e[I];
				0 !== i[o] && (g = !1)
			}
			g && (n.splice(t--, 1), A = r(r.s = e[0]))
		}
		return A
	}
	var g = {}, i = {
			app: 0
		}, n = [];

	function r(t) {
		if (g[t]) return g[t].exports;
		var e = g[t] = {
			i: t,
			l: !1,
			exports: {}
		};
		return A[t].call(e.exports, e, e.exports, r), e.l = !0, e.exports
	}
	r.m = A, r.c = g, r.d = function(A, t, e) {
		r.o(A, t) || Object.defineProperty(A, t, {
			enumerable: !0,
			get: e
		})
	}, r.r = function(A) {
		"undefined" !== typeof Symbol && Symbol.toStringTag && Object.defineProperty(A, Symbol.toStringTag, {
			value: "Module"
		}), Object.defineProperty(A, "__esModule", {
			value: !0
		})
	}, r.t = function(A, t) {
		if (1 & t && (A = r(A)), 8 & t) return A;
		if (4 & t && "object" === typeof A && A && A.__esModule) return A;
		var e = Object.create(null);
		if (r.r(e), Object.defineProperty(e, "default", {
			enumerable: !0,
			value: A
		}), 2 & t && "string" != typeof A)
			for (var g in A) r.d(e, g, function(t) {
				return A[t]
			}.bind(null, g));
		return e
	}, r.n = function(A) {
		var t = A && A.__esModule ? function() {
				return A["default"]
			} : function() {
				return A
			};
		return r.d(t, "a", t), t
	}, r.o = function(A, t) {
		return Object.prototype.hasOwnProperty.call(A, t)
	}, r.p = "";
	var I = window["webpackJsonp"] = window["webpackJsonp"] || [],
		o = I.push.bind(I);
	I.push = t, I = I.slice();
	for (var a = 0; a < I.length; a++) t(I[a]);
	var B = o;
	n.push([0, "chunk-vendors"]), e()
})({
	0: function(A, t, e) {
		A.exports = e("cd49")
	},
	"0565": function(A, t, e) {
		"use strict";
		(function(A) {
			var g = e("1da1"),
				i = (e("96cf"), e("99af"), e("b0c0"), e("7327")),
				n = e("731b"),
				r = e("25f1"),
				I = e("507a"),
				o = e("73ec"),
				a = e("b7c7");
			t["a"] = {
				name: "FileSelector",
				data: function() {
					return {
						task_all: 0,
						task_finished: 0,
						queue: new o["a"],
						parallel: !1
					}
				},
				computed: {
					progress_value: function() {
						return this.task_all ? this.task_finished / this.task_all * 100 : 0
					},
					progress_show: function() {
						return this.task_all !== this.task_finished
					}
				},
				mounted: function() {
					window.Worker && "file:" !== window.location.protocol ? (console.log("Using Worker Pool"), this.queue = Object(i["a"])((function() {
						return Object(n["a"])(new r["a"](A))
					}), navigator.hardwareConcurrency || 1), this.parallel = !0) : console.log("Using Queue in Main Thread")
				},
				methods: {
					progress_string: function() {
						return "".concat(this.task_finished, " / ").concat(this.task_all)
					},
					addFile: function(A) {
						var t = this;
						return Object(g["a"])(regeneratorRuntime.mark((function e() {
							return regeneratorRuntime.wrap((function(e) {
								while (1) switch (e.prev = e.next) {
									case 0:
										t.task_all++, t.queue.queue(Object(g["a"])(regeneratorRuntime.mark((function e() {
											var g, i = arguments;
											return regeneratorRuntime.wrap((function(e) {
												while (1) switch (e.prev = e.next) {
													case 0:
														return g = i.length > 0 && void 0 !== i[0] ? i[0] : I["a"], console.log("start handling", A.name), e.prev = 2, e.t0 = t, e.t1 = g, e.t2 = A, e.next = 8, a["a"].getAll();
													case 8:
														return e.t3 = e.sent, e.next = 11, (0, e.t1)(e.t2, e.t3);
													case 11:
														e.t4 = e.sent, e.t0.$emit.call(e.t0, "success", e.t4), e.next = 19;
														break;
													case 15:
														e.prev = 15, e.t5 = e["catch"](2), console.error(e.t5), t.$emit("error", e.t5, A.name);
													case 19:
														return e.prev = 19, t.task_finished++, e.finish(19);
													case 22:
													case "end":
														return e.stop()
												}
											}), e, null, [
												[2, 15, 19, 22]
											])
										}))));
									case 2:
									case "end":
										return e.stop()
								}
							}), e)
						})))()
					}
				}
			}
		}).call(this, e("2e83"))
	},
	"08fe": function(A, t, e) {
		"use strict";
		e.d(t, "a", (function() {
			return g
		}));
		e("ace4"), e("d3b7");
		var g = function() {
			function A(t, e) {
				if (void 0 === e && (e = A.numRounds), 16 != t.length) throw Error("incorrect key size");
				if (0 != (1 & e)) throw Error("odd number of rounds specified");
				var g = new DataView(t.buffer);
				this.k0 = g.getUint32(0, !1), this.k1 = g.getUint32(4, !1), this.k2 = g.getUint32(8, !1), this.k3 = g.getUint32(12, !1), this.rounds = e
			}
			return A.prototype.encrypt = function(t, e) {
				for (var g = e.getUint32(0, !1), i = e.getUint32(4, !1), n = 0, r = 0; r < this.rounds / 2; r++) n += A.delta, g += (i << 4) + this.k0 ^ i + n ^ (i >>> 5) + this.k1, i += (g << 4) + this.k2 ^ g + n ^ (g >>> 5) + this.k3;
				t.setUint32(0, g, !1), t.setUint32(4, i, !1)
			}, A.prototype.decrypt = function(t, e) {
				for (var g = e.getUint32(0, !1), i = e.getUint32(4, !1), n = A.delta * this.rounds / 2, r = 0; r < this.rounds / 2; r++) i -= (g << 4) + this.k2 ^ g + n ^ (g >>> 5) + this.k3, g -= (i << 4) + this.k0 ^ i + n ^ (i >>> 5) + this.k1, n -= A.delta;
				t.setUint32(0, g, !1), t.setUint32(4, i, !1)
			}, A.BlockSize = 8, A.KeySize = 16, A.delta = 2654435769, A.numRounds = 64, A
		}()
	},
	"0b72": function(A, t, e) {
		(function(A, g, i, n, r) {
			var I, o, a = e("7037").
			default;
			e("d3b7"), e("25f0"), e("ace4"), e("5cc6"), e("907a"), e("9a8c"), e("a975"), e("735e"), e("c1ac"), e("d139"), e("3a7b"), e("d5d6"), e("82f8"), e("e91f"), e("60bd"), e("5f96"), e("3280"), e("3fcc"), e("ca91"), e("25a1"), e("cd26"), e("3c5d"), e("2954"), e("649e"), e("219c"), e("170b"), e("b39a"), e("72f7"), e("ac1f"), e("5319"), e("fb6a"), e("a9e3"), e("fd87"), e("8b09"), e("143c"), e("84c3"), e("fb2c"), e("cfc3"), e("4a9b"), e("2ca0"), e("b0c0"), e("159b"), e("99af"), e("caad"), e("2532"), e("a15b"), e("d81d"), e("c19f"), e("8a59"), e("a874");
			var B = function() {
				var t = "undefined" !== typeof document && document.currentScript ? document.currentScript.src : void 0;
				return t = t || A,
				function(A) {
					A = A || {};
					var r, I, o = "undefined" !== typeof A ? A : {};
					o["ready"] = new Promise((function(A, t) {
						r = A, I = t
					}));
					var B, C = {};
					for (B in o) o.hasOwnProperty(B) && (C[B] = o[B]);
					var Q, E, c, s, u, l = [],
						f = "object" === ("undefined" === typeof window ? "undefined" : a(window)),
						d = "function" === typeof importScripts,
						h = "object" === ("undefined" === typeof g ? "undefined" : a(g)) && "object" === a(g.versions) && "string" === typeof g.versions.node,
						w = "";

					function y(A) {
						return o["locateFile"] ? o["locateFile"](A, w) : w + A
					}

					function D(A) {
						if (!(A instanceof Xt)) {
							var t = A;
							b("exiting due to exception: " + t)
						}
					}
					h ? (w = d ? e("df7c").dirname(w) + "/" : i + "/", Q = function(A, t) {
						var g = jt(A);
						return g ? t ? g : g.toString() : (s || (s = e("3e8f")), u || (u = e("df7c")), A = u["normalize"](A), s["readFileSync"](A, t ? null : "utf8"))
					}, c = function(A) {
						var t = Q(A, !0);
						return t.buffer || (t = new Uint8Array(t)), k(t.buffer), t
					}, E = function(A, t, g) {
						var i = jt(A);
						i && t(i), s || (s = e("3e8f")), u || (u = e("df7c")), A = u["normalize"](A), s["readFile"](A, (function(A, e) {
							A ? g(A) : t(e.buffer)
						}))
					}, g["argv"].length > 1 && g["argv"][1].replace(/\\/g, "/"), l = g["argv"].slice(2), g["on"]("uncaughtException", (function(A) {
						if (!(A instanceof Xt)) throw A
					})), g["on"]("unhandledRejection", (function(A) {
						throw A
					})), function(A, t) {
						if (oA()) throw g["exitCode"] = A, t;
						D(t), g["exit"](A)
					}, o["inspect"] = function() {
						return "[Emscripten Module object]"
					}) : (f || d) && (d ? w = self.location.href : "undefined" !== typeof document && document.currentScript && (w = document.currentScript.src), t && (w = t), w = 0 !== w.indexOf("blob:") ? w.substr(0, w.replace(/[?#].*/, "").lastIndexOf("/") + 1) : "", Q = function(A) {
						try {
							var t = new XMLHttpRequest;
							return t.open("GET", A, !1), t.send(null), t.responseText
						} catch (b) {
							var e = jt(A);
							if (e) return Ht(e);
							throw b
						}
					}, d && (c = function(A) {
						try {
							var t = new XMLHttpRequest;
							return t.open("GET", A, !1), t.responseType = "arraybuffer", t.send(null), new Uint8Array(t.response)
						} catch (b) {
							var e = jt(A);
							if (e) return e;
							throw b
						}
					}), E = function(A, t, e) {
						var g = new XMLHttpRequest;
						g.open("GET", A, !0), g.responseType = "arraybuffer", g.onload = function() {
							if (200 == g.status || 0 == g.status && g.response) t(g.response);
							else {
								var i = jt(A);
								i ? t(i.buffer) : e()
							}
						}, g.onerror = e, g.send(null)
					});
					var m = o["print"] || console.log.bind(console),
						b = o["printErr"] || console.warn.bind(console);
					for (B in C) C.hasOwnProperty(B) && (o[B] = C[B]);
					C = null, o["arguments"] && (l = o["arguments"]), o["thisProgram"] && o["thisProgram"], o["quit"] && o["quit"];
					var p, N = function(A) {
							A
						};
					o["wasmBinary"] && (p = o["wasmBinary"]);
					var F, v = o["noExitRuntime"] || !0;

					function R(A, t, e) {
						switch (t = t || "i8", "*" === t.charAt(t.length - 1) && (t = "i32"), t) {
							case "i1":
								return K[A >> 0];
							case "i8":
								return K[A >> 0];
							case "i16":
								return j[A >> 1];
							case "i32":
								return T[A >> 2];
							case "i64":
								return T[A >> 2];
							case "float":
								return q[A >> 2];
							case "double":
								return Number(Z[A >> 3]);
							default:
								hA("invalid type for getValue: " + t)
						}
						return null
					}
					"object" !== ("undefined" === typeof WebAssembly ? "undefined" : a(WebAssembly)) && hA("no native wasm support detected");
					var G = !1;

					function k(A, t) {
						A || hA("Assertion failed: " + t)
					}
					var U = "undefined" !== typeof TextDecoder ? new TextDecoder("utf8") : void 0;

					function S(A, t, e) {
						var g = t + e,
							i = t;
						while (A[i] && !(i >= g))++i;
						if (i - t > 16 && A.subarray && U) return U.decode(A.subarray(t, i));
						var n = "";
						while (t < i) {
							var r = A[t++];
							if (128 & r) {
								var I = 63 & A[t++];
								if (192 != (224 & r)) {
									var o = 63 & A[t++];
									if (r = 224 == (240 & r) ? (15 & r) << 12 | I << 6 | o : (7 & r) << 18 | I << 12 | o << 6 | 63 & A[t++], r < 65536) n += String.fromCharCode(r);
									else {
										var a = r - 65536;
										n += String.fromCharCode(55296 | a >> 10, 56320 | 1023 & a)
									}
								} else n += String.fromCharCode((31 & r) << 6 | I)
							} else n += String.fromCharCode(r)
						}
						return n
					}

					function Y(A, t) {
						return A ? S(x, A, t) : ""
					}

					function J(A, t, e, g) {
						if (!(g > 0)) return 0;
						for (var i = e, n = e + g - 1, r = 0; r < A.length; ++r) {
							var I = A.charCodeAt(r);
							if (I >= 55296 && I <= 57343) {
								var o = A.charCodeAt(++r);
								I = 65536 + ((1023 & I) << 10) | 1023 & o
							}
							if (I <= 127) {
								if (e >= n) break;
								t[e++] = I
							} else if (I <= 2047) {
								if (e + 1 >= n) break;
								t[e++] = 192 | I >> 6, t[e++] = 128 | 63 & I
							} else if (I <= 65535) {
								if (e + 2 >= n) break;
								t[e++] = 224 | I >> 12, t[e++] = 128 | I >> 6 & 63, t[e++] = 128 | 63 & I
							} else {
								if (e + 3 >= n) break;
								t[e++] = 240 | I >> 18, t[e++] = 128 | I >> 12 & 63, t[e++] = 128 | I >> 6 & 63, t[e++] = 128 | 63 & I
							}
						}
						return t[e] = 0, e - i
					}

					function M(A, t, e) {
						return J(A, x, t, e)
					}

					function L(A) {
						for (var t = 0, e = 0; e < A.length; ++e) {
							var g = A.charCodeAt(e);
							g >= 55296 && g <= 57343 && (g = 65536 + ((1023 & g) << 10) | 1023 & A.charCodeAt(++e)), g <= 127 ? ++t : t += g <= 2047 ? 2 : g <= 65535 ? 3 : 4
						}
						return t
					}
					var H, K, x, j, O, T, W, q, Z, X = "undefined" !== typeof TextDecoder ? new TextDecoder("utf-16le") : void 0;

					function P(A, t) {
						var e = A,
							g = e >> 1,
							i = g + t / 2;
						while (!(g >= i) && O[g])++g;
						if (e = g << 1, e - A > 32 && X) return X.decode(x.subarray(A, e));
						for (var n = "", r = 0; !(r >= t / 2); ++r) {
							var I = j[A + 2 * r >> 1];
							if (0 == I) break;
							n += String.fromCharCode(I)
						}
						return n
					}

					function V(A, t, e) {
						if (void 0 === e && (e = 2147483647), e < 2) return 0;
						e -= 2;
						for (var g = t, i = e < 2 * A.length ? e / 2 : A.length, n = 0; n < i; ++n) {
							var r = A.charCodeAt(n);
							j[t >> 1] = r, t += 2
						}
						return j[t >> 1] = 0, t - g
					}

					function _(A) {
						return 2 * A.length
					}

					function z(A, t) {
						var e = 0,
							g = "";
						while (!(e >= t / 4)) {
							var i = T[A + 4 * e >> 2];
							if (0 == i) break;
							if (++e, i >= 65536) {
								var n = i - 65536;
								g += String.fromCharCode(55296 | n >> 10, 56320 | 1023 & n)
							} else g += String.fromCharCode(i)
						}
						return g
					}

					function $(A, t, e) {
						if (void 0 === e && (e = 2147483647), e < 4) return 0;
						for (var g = t, i = g + e - 4, n = 0; n < A.length; ++n) {
							var r = A.charCodeAt(n);
							if (r >= 55296 && r <= 57343) {
								var I = A.charCodeAt(++n);
								r = 65536 + ((1023 & r) << 10) | 1023 & I
							}
							if (T[t >> 2] = r, t += 4, t + 4 > i) break
						}
						return T[t >> 2] = 0, t - g
					}

					function AA(A) {
						for (var t = 0, e = 0; e < A.length; ++e) {
							var g = A.charCodeAt(e);
							g >= 55296 && g <= 57343 && ++e, t += 4
						}
						return t
					}

					function tA(A, t) {
						K.set(A, t)
					}

					function eA(A) {
						H = A, o["HEAP8"] = K = new Int8Array(A), o["HEAP16"] = j = new Int16Array(A), o["HEAP32"] = T = new Int32Array(A), o["HEAPU8"] = x = new Uint8Array(A), o["HEAPU16"] = O = new Uint16Array(A), o["HEAPU32"] = W = new Uint32Array(A), o["HEAPF32"] = q = new Float32Array(A), o["HEAPF64"] = Z = new Float64Array(A)
					}
					o["INITIAL_MEMORY"];
					var gA, iA = [],
						nA = [],
						rA = [],
						IA = 0;

					function oA() {
						return v || IA > 0
					}

					function aA() {
						if (o["preRun"]) {
							"function" == typeof o["preRun"] && (o["preRun"] = [o["preRun"]]);
							while (o["preRun"].length) QA(o["preRun"].shift())
						}
						FA(iA)
					}

					function BA() {
						!0, FA(nA)
					}

					function CA() {
						if (o["postRun"]) {
							"function" == typeof o["postRun"] && (o["postRun"] = [o["postRun"]]);
							while (o["postRun"].length) cA(o["postRun"].shift())
						}
						FA(rA)
					}

					function QA(A) {
						iA.unshift(A)
					}

					function EA(A) {
						nA.unshift(A)
					}

					function cA(A) {
						rA.unshift(A)
					}
					var sA = 0,
						uA = null,
						lA = null;

					function fA(A) {
						sA++, o["monitorRunDependencies"] && o["monitorRunDependencies"](sA)
					}

					function dA(A) {
						if (sA--, o["monitorRunDependencies"] && o["monitorRunDependencies"](sA), 0 == sA && (null !== uA && (clearInterval(uA), uA = null), lA)) {
							var t = lA;
							lA = null, t()
						}
					}

					function hA(A) {
						o["onAbort"] && o["onAbort"](A), A = "Aborted(" + A + ")", b(A), G = !0, 1, A += ". Build with -s ASSERTIONS=1 for more info.";
						var t = new WebAssembly.RuntimeError(A);
						throw I(t), t
					}
					o["preloadedImages"] = {}, o["preloadedAudios"] = {};
					var wA, yA = "data:application/octet-stream;base64,";

					function DA(A) {
						return A.startsWith(yA)
					}

					function mA(A) {
						return A.startsWith("file://")
					}

					function bA(A) {
						try {
							if (A == wA && p) return new Uint8Array(p);
							var t = jt(A);
							if (t) return t;
							if (c) return c(A);
							throw "both async and sync fetching of the wasm failed"
						} catch (b) {
							hA(b)
						}
					}

					function pA() {
						if (!p && (f || d)) {
							if ("function" === typeof fetch && !mA(wA)) return fetch(wA, {
								credentials: "same-origin"
							}).then((function(A) {
								if (!A["ok"]) throw "failed to load wasm binary file at '" + wA + "'";
								return A["arrayBuffer"]()
							})).
							catch ((function() {
								return bA(wA)
							}));
							if (E) return new Promise((function(A, t) {
								E(wA, (function(t) {
									A(new Uint8Array(t))
								}), t)
							}))
						}
						return Promise.resolve().then((function() {
							return bA(wA)
						}))
					}

					function NA() {
						var A = {
							a: Tt
						};

						function t(A, t) {
							var e = A.exports;
							o["asm"] = e, F = o["asm"]["r"], eA(F.buffer), gA = o["asm"]["u"], EA(o["asm"]["s"]), dA("wasm-instantiate")
						}

						function e(A) {
							t(A["instance"])
						}

						function g(t) {
							return pA().then((function(t) {
								return WebAssembly.instantiate(t, A)
							})).then((function(A) {
								return A
							})).then(t, (function(A) {
								b("failed to asynchronously prepare wasm: " + A), hA(A)
							}))
						}

						function i() {
							return p || "function" !== typeof WebAssembly.instantiateStreaming || DA(wA) || mA(wA) || "function" !== typeof fetch ? g(e) : fetch(wA, {
								credentials: "same-origin"
							}).then((function(t) {
								var i = WebAssembly.instantiateStreaming(t, A);
								return i.then(e, (function(A) {
									return b("wasm streaming compile failed: " + A), b("falling back to ArrayBuffer instantiation"), g(e)
								}))
							}))
						}
						if (fA("wasm-instantiate"), o["instantiateWasm"]) try {
							var n = o["instantiateWasm"](A, t);
							return n
						} catch (r) {
							return b("Module.instantiateWasm callback failed with error: " + r), !1
						}
						return i().
						catch (I), {}
					}

					function FA(A) {
						while (A.length > 0) {
							var t = A.shift();
							if ("function" != typeof t) {
								var e = t.func;
								"number" === typeof e ? void 0 === t.arg ? RA(e)() : RA(e)(t.arg) : e(void 0 === t.arg ? null : t.arg)
							} else t(o)
						}
					}
					wA = "data:application/octet-stream;base64,", DA(wA) || (wA = y(wA));
					var vA = [];

					function RA(A) {
						var t = vA[A];
						return t || (A >= vA.length && (vA.length = A + 1), vA[A] = t = gA.get(A)), t
					}

					function GA(A, t, e, g, i) {}

					function kA(A) {
						switch (A) {
							case 1:
								return 0;
							case 2:
								return 1;
							case 4:
								return 2;
							case 8:
								return 3;
							default:
								throw new TypeError("Unknown type size: " + A)
						}
					}

					function UA() {
						for (var A = new Array(256), t = 0; t < 256; ++t) A[t] = String.fromCharCode(t);
						SA = A
					}
					var SA = void 0;

					function YA(A) {
						var t = "",
							e = A;
						while (x[e]) t += SA[x[e++]];
						return t
					}
					var JA = {}, MA = {}, LA = {}, HA = 48,
						KA = 57;

					function xA(A) {
						if (void 0 === A) return "_unknown";
						A = A.replace(/[^a-zA-Z0-9_]/g, "$");
						var t = A.charCodeAt(0);
						return t >= HA && t <= KA ? "_" + A : A
					}

					function jA(A, t) {
						return A = xA(A),
						function() {
							return t.apply(this, arguments)
						}
					}

					function OA(A, t) {
						var e = jA(t, (function(A) {
							this.name = t, this.message = A;
							var e = new Error(A).stack;
							void 0 !== e && (this.stack = this.toString() + "\n" + e.replace(/^Error(:[^\n]*)?\n/, ""))
						}));
						return e.prototype = Object.create(A.prototype), e.prototype.constructor = e, e.prototype.toString = function() {
							return void 0 === this.message ? this.name : this.name + ": " + this.message
						}, e
					}
					var TA = void 0;

					function WA(A) {
						throw new TA(A)
					}
					var qA = void 0;

					function ZA(A) {
						throw new qA(A)
					}

					function XA(A, t, e) {
						function g(t) {
							var g = e(t);
							g.length !== A.length && ZA("Mismatched type converter count");
							for (var i = 0; i < A.length; ++i) PA(A[i], g[i])
						}
						A.forEach((function(A) {
							LA[A] = t
						}));
						var i = new Array(t.length),
							n = [],
							r = 0;
						t.forEach((function(A, t) {
							MA.hasOwnProperty(A) ? i[t] = MA[A] : (n.push(A), JA.hasOwnProperty(A) || (JA[A] = []), JA[A].push((function() {
								i[t] = MA[A], ++r, r === n.length && g(i)
							})))
						})), 0 === n.length && g(i)
					}

					function PA(A, t, e) {
						if (e = e || {}, !("argPackAdvance" in t)) throw new TypeError("registerType registeredInstance requires argPackAdvance");
						var g = t.name;
						if (A || WA('type "' + g + '" must have a positive integer typeid pointer'), MA.hasOwnProperty(A)) {
							if (e.ignoreDuplicateRegistrations) return;
							WA("Cannot register type '" + g + "' twice")
						}
						if (MA[A] = t, delete LA[A], JA.hasOwnProperty(A)) {
							var i = JA[A];
							delete JA[A], i.forEach((function(A) {
								A()
							}))
						}
					}

					function VA(A, t, e, g, i) {
						var n = kA(e);
						t = YA(t), PA(A, {
							name: t,
							fromWireType: function(A) {
								return !!A
							},
							toWireType: function(A, t) {
								return t ? g : i
							},
							argPackAdvance: 8,
							readValueFromPointer: function(A) {
								var g;
								if (1 === e) g = K;
								else if (2 === e) g = j;
								else {
									if (4 !== e) throw new TypeError("Unknown boolean type size: " + t);
									g = T
								}
								return this["fromWireType"](g[A >> n])
							},
							destructorFunction: null
						})
					}
					var _A = [],
						zA = [{}, {
							value: void 0
						}, {
							value: null
						}, {
							value: !0
						}, {
							value: !1
						}];

					function $A(A) {
						A > 4 && 0 === --zA[A].refcount && (zA[A] = void 0, _A.push(A))
					}

					function At() {
						for (var A = 0, t = 5; t < zA.length; ++t) void 0 !== zA[t] && ++A;
						return A
					}

					function tt() {
						for (var A = 5; A < zA.length; ++A)
							if (void 0 !== zA[A]) return zA[A];
						return null
					}

					function et() {
						o["count_emval_handles"] = At, o["get_first_emval"] = tt
					}
					var gt = {
						toValue: function(A) {
							return A || WA("Cannot use deleted val. handle = " + A), zA[A].value
						},
						toHandle: function(A) {
							switch (A) {
								case void 0:
									return 1;
								case null:
									return 2;
								case !0:
									return 3;
								case !1:
									return 4;
								default:
									var t = _A.length ? _A.pop() : zA.length;
									return zA[t] = {
										refcount: 1,
										value: A
									}, t
							}
						}
					};

					function it(A) {
						return this["fromWireType"](W[A >> 2])
					}

					function nt(A, t) {
						t = YA(t), PA(A, {
							name: t,
							fromWireType: function(A) {
								var t = gt.toValue(A);
								return $A(A), t
							},
							toWireType: function(A, t) {
								return gt.toHandle(t)
							},
							argPackAdvance: 8,
							readValueFromPointer: it,
							destructorFunction: null
						})
					}

					function rt(A) {
						if (null === A) return "null";
						var t = a(A);
						return "object" === t || "array" === t || "function" === t ? A.toString() : "" + A
					}

					function It(A, t) {
						switch (t) {
							case 2:
								return function(A) {
									return this["fromWireType"](q[A >> 2])
								};
							case 3:
								return function(A) {
									return this["fromWireType"](Z[A >> 3])
								};
							default:
								throw new TypeError("Unknown float type: " + A)
						}
					}

					function ot(A, t, e) {
						var g = kA(e);
						t = YA(t), PA(A, {
							name: t,
							fromWireType: function(A) {
								return A
							},
							toWireType: function(A, t) {
								return t
							},
							argPackAdvance: 8,
							readValueFromPointer: It(t, g),
							destructorFunction: null
						})
					}

					function at(A) {
						while (A.length) {
							var t = A.pop(),
								e = A.pop();
							e(t)
						}
					}

					function Bt(A, t, e, g, i) {
						var n = t.length;
						n < 2 && WA("argTypes array size mismatch! Must at least get return value and 'this' types!");
						for (var r = null !== t[1] && null !== e, I = !1, o = 1; o < t.length; ++o)
							if (null !== t[o] && void 0 === t[o].destructorFunction) {
								I = !0;
								break
							}
						var a = "void" !== t[0].name,
							B = n - 2,
							C = new Array(B),
							Q = [],
							E = [];
						return function() {
							var e;
							arguments.length !== B && WA("function " + A + " called with " + arguments.length + " arguments, expected " + B + " args!"), E.length = 0, Q.length = r ? 2 : 1, Q[0] = i, r && (e = t[1].toWireType(E, this), Q[1] = e);
							for (var n = 0; n < B; ++n) C[n] = t[n + 2].toWireType(E, arguments[n]), Q.push(C[n]);
							var o = g.apply(null, Q);

							function c(A) {
								if (I) at(E);
								else
									for (var g = r ? 1 : 2; g < t.length; g++) {
										var i = 1 === g ? e : C[g - 2];
										null !== t[g].destructorFunction && t[g].destructorFunction(i)
									}
								if (a) return t[0].fromWireType(A)
							}
							return c(o)
						}
					}

					function Ct(A, t, e) {
						if (void 0 === A[t].overloadTable) {
							var g = A[t];
							A[t] = function() {
								return A[t].overloadTable.hasOwnProperty(arguments.length) || WA("Function '" + e + "' called with an invalid number of arguments (" + arguments.length + ") - expects one of (" + A[t].overloadTable + ")!"), A[t].overloadTable[arguments.length].apply(this, arguments)
							}, A[t].overloadTable = [], A[t].overloadTable[g.argCount] = g
						}
					}

					function Qt(A, t, e) {
						o.hasOwnProperty(A) ? ((void 0 === e || void 0 !== o[A].overloadTable && void 0 !== o[A].overloadTable[e]) && WA("Cannot register public name '" + A + "' twice"), Ct(o, A, A), o.hasOwnProperty(e) && WA("Cannot register multiple overloads of a function with the same number of arguments (" + e + ")!"), o[A].overloadTable[e] = t) : (o[A] = t, void 0 !== e && (o[A].numArguments = e))
					}

					function Et(A, t) {
						for (var e = [], g = 0; g < A; g++) e.push(T[(t >> 2) + g]);
						return e
					}

					function ct(A, t, e) {
						o.hasOwnProperty(A) || ZA("Replacing nonexistant public symbol"), void 0 !== o[A].overloadTable && void 0 !== e ? o[A].overloadTable[e] = t : (o[A] = t, o[A].argCount = e)
					}

					function st(A, t, e) {
						var g = o["dynCall_" + A];
						return e && e.length ? g.apply(null, [t].concat(e)) : g.call(null, t)
					}

					function ut(A, t, e) {
						return A.includes("j") ? st(A, t, e) : RA(t).apply(null, e)
					}

					function lt(A, t) {
						var e = [];
						return function() {
							e.length = arguments.length;
							for (var g = 0; g < arguments.length; g++) e[g] = arguments[g];
							return ut(A, t, e)
						}
					}

					function ft(A, t) {
						function e() {
							return A.includes("j") ? lt(A, t) : RA(t)
						}
						A = YA(A);
						var g = e();
						return "function" !== typeof g && WA("unknown function pointer with signature " + A + ": " + t), g
					}
					var dt = void 0;

					function ht(A) {
						var t = qt(A),
							e = YA(t);
						return Zt(t), e
					}

					function wt(A, t) {
						var e = [],
							g = {};

						function i(A) {
							g[A] || MA[A] || (LA[A] ? LA[A].forEach(i) : (e.push(A), g[A] = !0))
						}
						throw t.forEach(i), new dt(A + ": " + e.map(ht).join([", "]))
					}

					function yt(A, t, e, g, i, n) {
						var r = Et(t, e);
						A = YA(A), i = ft(g, i), Qt(A, (function() {
							wt("Cannot call " + A + " due to unbound types", r)
						}), t - 1), XA([], r, (function(e) {
							var g = [e[0], null].concat(e.slice(1));
							return ct(A, Bt(A, g, null, i, n), t - 1), []
						}))
					}

					function Dt(A, t, e) {
						switch (t) {
							case 0:
								return e ? function(A) {
									return K[A]
								} : function(A) {
									return x[A]
								};
							case 1:
								return e ? function(A) {
									return j[A >> 1]
								} : function(A) {
									return O[A >> 1]
								};
							case 2:
								return e ? function(A) {
									return T[A >> 2]
								} : function(A) {
									return W[A >> 2]
								};
							default:
								throw new TypeError("Unknown integer type: " + A)
						}
					}

					function mt(A, t, e, g, i) {
						t = YA(t), -1 === i && (i = 4294967295);
						var n = kA(e),
							r = function(A) {
								return A
							};
						if (0 === g) {
							var I = 32 - 8 * e;
							r = function(A) {
								return A << I >>> I
							}
						}
						var o = t.includes("unsigned");
						PA(A, {
							name: t,
							fromWireType: r,
							toWireType: function(A, e) {
								if ("number" !== typeof e && "boolean" !== typeof e) throw new TypeError('Cannot convert "' + rt(e) + '" to ' + this.name);
								if (e < g || e > i) throw new TypeError('Passing a number "' + rt(e) + '" from JS side to C/C++ side to an argument of type "' + t + '", which is outside the valid range [' + g + ", " + i + "]!");
								return o ? e >>> 0 : 0 | e
							},
							argPackAdvance: 8,
							readValueFromPointer: Dt(t, n, 0 !== g),
							destructorFunction: null
						})
					}

					function bt(A, t, e) {
						var g = [Int8Array, Uint8Array, Int16Array, Uint16Array, Int32Array, Uint32Array, Float32Array, Float64Array],
							i = g[t];

						function n(A) {
							A >>= 2;
							var t = W,
								e = t[A],
								g = t[A + 1];
							return new i(H, g, e)
						}
						e = YA(e), PA(A, {
							name: e,
							fromWireType: n,
							argPackAdvance: 8,
							readValueFromPointer: n
						}, {
							ignoreDuplicateRegistrations: !0
						})
					}

					function pt(A, t) {
						t = YA(t);
						var e = "std::string" === t;
						PA(A, {
							name: t,
							fromWireType: function(A) {
								var t, g = W[A >> 2];
								if (e)
									for (var i = A + 4, n = 0; n <= g; ++n) {
										var r = A + 4 + n;
										if (n == g || 0 == x[r]) {
											var I = r - i,
												o = Y(i, I);
											void 0 === t ? t = o : (t += String.fromCharCode(0), t += o), i = r + 1
										}
									} else {
										var a = new Array(g);
										for (n = 0; n < g; ++n) a[n] = String.fromCharCode(x[A + 4 + n]);
										t = a.join("")
									}
								return Zt(A), t
							},
							toWireType: function(A, t) {
								var g;
								t instanceof ArrayBuffer && (t = new Uint8Array(t));
								var i = "string" === typeof t;
								i || t instanceof Uint8Array || t instanceof Uint8ClampedArray || t instanceof Int8Array || WA("Cannot pass non-string to std::string"), g = e && i ? function() {
									return L(t)
								} : function() {
									return t.length
								};
								var n = g(),
									r = Wt(4 + n + 1);
								if (W[r >> 2] = n, e && i) M(t, r + 4, n + 1);
								else if (i)
									for (var I = 0; I < n; ++I) {
										var o = t.charCodeAt(I);
										o > 255 && (Zt(r), WA("String has UTF-16 code units that do not fit in 8 bits")), x[r + 4 + I] = o
									} else
										for (I = 0; I < n; ++I) x[r + 4 + I] = t[I];
								return null !== A && A.push(Zt, r), r
							},
							argPackAdvance: 8,
							readValueFromPointer: it,
							destructorFunction: function(A) {
								Zt(A)
							}
						})
					}

					function Nt(A, t, e) {
						var g, i, n, r, I;
						e = YA(e), 2 === t ? (g = P, i = V, r = _, n = function() {
							return O
						}, I = 1) : 4 === t && (g = z, i = $, r = AA, n = function() {
							return W
						}, I = 2), PA(A, {
							name: e,
							fromWireType: function(A) {
								for (var e, i = W[A >> 2], r = n(), o = A + 4, a = 0; a <= i; ++a) {
									var B = A + 4 + a * t;
									if (a == i || 0 == r[B >> I]) {
										var C = B - o,
											Q = g(o, C);
										void 0 === e ? e = Q : (e += String.fromCharCode(0), e += Q), o = B + t
									}
								}
								return Zt(A), e
							},
							toWireType: function(A, g) {
								"string" !== typeof g && WA("Cannot pass non-string to C++ string type " + e);
								var n = r(g),
									o = Wt(4 + n + t);
								return W[o >> 2] = n >> I, i(g, o + 4, n + t), null !== A && A.push(Zt, o), o
							},
							argPackAdvance: 8,
							readValueFromPointer: it,
							destructorFunction: function(A) {
								Zt(A)
							}
						})
					}

					function Ft(A, t) {
						t = YA(t), PA(A, {
							isVoid: !0,
							name: t,
							argPackAdvance: 0,
							fromWireType: function() {},
							toWireType: function(A, t) {}
						})
					}

					function vt() {
						hA("")
					}

					function Rt(A, t, e) {
						x.copyWithin(A, t, t + e)
					}

					function Gt(A) {
						hA("OOM")
					}

					function kt(A) {
						x.length;
						A >>>= 0, Gt(A)
					}
					var Ut = {
						mappings: {},
						buffers: [null, [],
							[]
						],
						printChar: function(A, t) {
							var e = Ut.buffers[A];
							0 === t || 10 === t ? ((1 === A ? m : b)(S(e, 0)), e.length = 0) : e.push(t)
						},
						varargs: void 0,
						get: function() {
							Ut.varargs += 4;
							var A = T[Ut.varargs - 4 >> 2];
							return A
						},
						getStr: function(A) {
							var t = Y(A);
							return t
						},
						get64: function(A, t) {
							return A
						}
					};

					function St(A) {
						return 0
					}

					function Yt(A, t, e, g, i) {}

					function Jt(A, t, e, g) {
						for (var i = 0, n = 0; n < e; n++) {
							var r = T[t >> 2],
								I = T[t + 4 >> 2];
							t += 8;
							for (var o = 0; o < I; o++) Ut.printChar(A, x[r + o]);
							i += I
						}
						return T[g >> 2] = i, 0
					}

					function Mt(A) {
						N(A)
					}
					UA(), TA = o["BindingError"] = OA(Error, "BindingError"), qA = o["InternalError"] = OA(Error, "InternalError"), et(), dt = o["UnboundTypeError"] = OA(Error, "UnboundTypeError");
					var Lt = !1;

					function Ht(A) {
						for (var t = [], e = 0; e < A.length; e++) {
							var g = A[e];
							g > 255 && (Lt && k(!1, "Character code " + g + " (" + String.fromCharCode(g) + ")  at offset " + e + " not in 0x00-0xFF."), g &= 255), t.push(String.fromCharCode(g))
						}
						return t.join("")
					}
					var Kt = "function" === typeof atob ? atob : function(A) {
							var t, e, g, i, n, r, I, o = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",
								a = "",
								B = 0;
							A = A.replace(/[^A-Za-z0-9\+\/\=]/g, "");
							do {
								i = o.indexOf(A.charAt(B++)), n = o.indexOf(A.charAt(B++)), r = o.indexOf(A.charAt(B++)), I = o.indexOf(A.charAt(B++)), t = i << 2 | n >> 4, e = (15 & n) << 4 | r >> 2, g = (3 & r) << 6 | I, a += String.fromCharCode(t), 64 !== r && (a += String.fromCharCode(e)), 64 !== I && (a += String.fromCharCode(g))
							} while (B < A.length);
							return a
						};

					function xt(A) {
						if ("boolean" === typeof h && h) {
							var t = n.from(A, "base64");
							return new Uint8Array(t["buffer"], t["byteOffset"], t["byteLength"])
						}
						try {
							for (var e = Kt(A), g = new Uint8Array(e.length), i = 0; i < e.length; ++i) g[i] = e.charCodeAt(i);
							return g
						} catch (r) {
							throw new Error("Converting base64 string to bytes failed.")
						}
					}

					function jt(A) {
						if (DA(A)) return xt(A.slice(yA.length))
					}
					var Ot, Tt = {
							l: GA,
							j: VA,
							i: nt,
							f: ot,
							c: yt,
							b: mt,
							a: bt,
							g: pt,
							e: Nt,
							q: Ft,
							d: vt,
							n: Rt,
							o: kt,
							p: St,
							k: Yt,
							h: Jt,
							m: Mt
						}, Wt = (NA(), o["___wasm_call_ctors"] = function() {
							return (o["___wasm_call_ctors"] = o["asm"]["s"]).apply(null, arguments)
						}, o["_malloc"] = function() {
							return (Wt = o["_malloc"] = o["asm"]["t"]).apply(null, arguments)
						}),
						qt = o["___getTypeName"] = function() {
							return (qt = o["___getTypeName"] = o["asm"]["v"]).apply(null, arguments)
						}, Zt = (o["___embind_register_native_and_builtin_types"] = function() {
							return (o["___embind_register_native_and_builtin_types"] = o["asm"]["w"]).apply(null, arguments)
						}, o["_free"] = function() {
							return (Zt = o["_free"] = o["asm"]["x"]).apply(null, arguments)
						});
					o["dynCall_jiji"] = function() {
						return (o["dynCall_jiji"] = o["asm"]["y"]).apply(null, arguments)
					};

					function Xt(A) {
						this.name = "ExitStatus", this.message = "Program terminated with exit(" + A + ")", this.status = A
					}

					function Pt(A) {
						function t() {
							Ot || (Ot = !0, o["calledRun"] = !0, G || (BA(), r(o), o["onRuntimeInitialized"] && o["onRuntimeInitialized"](), CA()))
						}
						A = A || l, sA > 0 || (aA(), sA > 0 || (o["setStatus"] ? (o["setStatus"]("Running..."), setTimeout((function() {
							setTimeout((function() {
								o["setStatus"]("")
							}), 1), t()
						}), 1)) : t()))
					}
					if (o["getValue"] = R, o["UTF8ToString"] = Y, o["writeArrayToMemory"] = tA, lA = function A() {
						Ot || Pt(), Ot || (lA = A)
					}, o["run"] = Pt, o["preInit"]) {
						"function" == typeof o["preInit"] && (o["preInit"] = [o["preInit"]]);
						while (o["preInit"].length > 0) o["preInit"].pop()()
					}
					return Pt(), A.ready
				}
			}();
			"object" === a(t) && "object" === a(r) ? r.exports = B : (I = [], o = function() {
				return B
			}.apply(t, I), void 0 === o || (r.exports = o))
		}).call(this, "/index.js", e("4362"), "/", e("1c35").Buffer, e("62e4")(A))
	},
	"0ba7": function(A, t, e) {
		"use strict";
		e.d(t, "a", (function() {
			return r
		}));
		e("d3b7"), e("ace4"), e("5cc6"), e("907a"), e("9a8c"), e("a975"), e("735e"), e("c1ac"), e("d139"), e("3a7b"), e("d5d6"), e("82f8"), e("e91f"), e("60bd"), e("5f96"), e("3280"), e("3fcc"), e("ca91"), e("25a1"), e("cd26"), e("3c5d"), e("2954"), e("649e"), e("219c"), e("170b"), e("b39a"), e("72f7"), e("3ca3"), e("ddb0"), e("2b3d"), e("9861");
		var g = e("9ab4"),
			i = e("cc74"),
			n = e("cb96");

		function r(A, t, e, r) {
			return void 0 === r && (r = !0), Object(g["a"])(this, void 0, Promise, (function() {
				var I, o, a, B, C, Q, E;
				return Object(g["c"])(this, (function(g) {
					switch (g.label) {
						case 0:
							return I = e, r ? (a = Uint8Array.bind, [4, Object(i["c"])(A)]) : [3, 2];
						case 1:
							o = new(a.apply(Uint8Array, [void 0, g.sent()])), I = Object(i["i"])(o, e), I !== e && (A = new Blob([o], {
								type: i["a"][I]
							})), g.label = 2;
						case 2:
							return [4, Object(n["parseBlob"])(A)];
						case 3:
							return B = g.sent(), C = Object(i["f"])(t, B.common.title, String(B.common.artists || B.common.artist || "")), Q = C.title, E = C.artist, [2, {
								title: Q,
								artist: E,
								ext: I,
								album: B.common.album,
								picture: Object(i["d"])(B),
								file: URL.createObjectURL(A),
								blob: A,
								mime: i["a"][I]
							}]
					}
				}))
			}))
		}
	},
	"0f92": function(A, t, e) {
		"use strict";
		(function(A) {
			e.d(t, "a", (function() {
				return B
			}));
			e("d3b7"), e("a15b"), e("d81d"), e("ac1f"), e("1276"), e("b0c0");
			var g = e("9ab4"),
				i = e("cb96"),
				n = e("acf9"),
				r = e.n(n),
				I = e("cc74"),
				o = e("97e5"),
				a = function(t) {
					return r.a.decode(new A(t || ""), "gbk")
				};

			function B(A, t, e, n) {
				return Object(g["a"])(this, void 0, Promise, (function() {
					var r, o, B, c, s;
					return Object(g["c"])(this, (function(g) {
						switch (g.label) {
							case 0:
								return [4, Object(i["parseBlob"])(A)];
							case 1:
								for (o in r = g.sent(), r.native) r.native.hasOwnProperty(o) && r.native[o].some((function(A) {
									return "TCON" === A.id && "(12)" === A.value
								})) && (console.warn("try using gbk encoding to decode meta"), r.common.artist = "", r.common.artists ? r.common.artist = r.common.artists.map(a).join() : r.common.artist = a(r.common.artist), r.common.title = a(r.common.title), r.common.album = a(r.common.album));
								if (n && "0" !== n) try {
									return [2, C(n, e, r, A)]
								} catch (u) {
									console.warn("在线获取曲目信息失败，回退到本地 meta 提取", u)
								}
								return B = Object(I["f"])(t, r.common.title, r.common.artist), B.artist = B.artist || "", c = Object(I["d"])(r), c ? [3, 3] : [4, Q(B.title, B.artist, r.common.album)];
							case 2:
								c = g.sent(), g.label = 3;
							case 3:
								return s = {
									title: B.title,
									artist: B.artist,
									album: r.common.album || "",
									imgUrl: c
								}, [4, E({
									title: B.title,
									artists: B.artist.split(I["m"]),
									ext: e,
									imageURL: c,
									musicMeta: r,
									blob: A
								})];
							case 4:
								return [2, (s.blob = g.sent(), s)]
						}
					}))
				}))
			}

			function C(A, t, e, i) {
				return Object(g["a"])(this, void 0, Promise, (function() {
					var n, r, I, a;
					return Object(g["c"])(this, (function(g) {
						switch (g.label) {
							case 0:
								return [4, Object(o["d"])(A)];
							case 1:
								return n = g.sent(), r = Object(o["b"])(n.track_info.album.pmid), I = n.track_info.singer.map((function(A) {
									return A.name
								})), a = {
									title: n.track_info.title,
									artist: I.join(","),
									album: n.track_info.album.name,
									imgUrl: r
								}, [4, E({
									title: n.track_info.title,
									artists: I,
									ext: t,
									imageURL: r,
									musicMeta: e,
									blob: i
								})];
							case 2:
								return [2, (a.blob = g.sent(), a)]
						}
					}))
				}))
			}

			function Q(A, t, e) {
				return Object(g["a"])(this, void 0, Promise, (function() {
					var i, n;
					return Object(g["c"])(this, (function(g) {
						switch (g.label) {
							case 0:
								return g.trys.push([0, 2, , 3]), [4, Object(o["c"])(A, t, e)];
							case 1:
								return i = g.sent(), [2, Object(o["b"])(i.Id, i.Type)];
							case 2:
								return n = g.sent(), console.warn(n), [3, 3];
							case 3:
								return [2, ""]
						}
					}))
				}))
			}

			function E(t) {
				return Object(g["a"])(this, void 0, Promise, (function() {
					var e, i, n, r, o, a, B;
					return Object(g["c"])(this, (function(g) {
						switch (g.label) {
							case 0:
								return g.trys.push([0, 3, , 4]), [4, Object(I["e"])(t.imageURL)];
							case 1:
								return e = g.sent(), e || console.warn("获取图像失败"), i = {
									picture: null === e || void 0 === e ? void 0 : e.buffer,
									title: t.title,
									artists: t.artists
								}, o = (r = A).from, [4, t.blob.arrayBuffer()];
							case 2:
								return n = o.apply(r, [g.sent()]), a = I["a"][t.ext] || I["a"].mp3, "mp3" === t.ext ? [2, new Blob([Object(I["l"])(n, i, t.musicMeta)], {
									type: a
								})] : "flac" === t.ext ? [2, new Blob([Object(I["k"])(n, i, t.musicMeta)], {
									type: a
								})] : (console.info("writing metadata for " + t.ext + " is not being supported for now"), [3, 4]);
							case 3:
								return B = g.sent(), console.warn("Error while appending cover image to file " + B), [3, 4];
							case 4:
								return [2, t.blob]
						}
					}))
				}))
			}
		}).call(this, e("1c35").Buffer)
	},
	1: function(A, t) {},
	1708: function(A, t, e) {
		"use strict";
		e("8c80")
	},
	2: function(A, t) {},
	"2e83": function(A, t, e) {
		A.exports = e.p + "js/0.f4b96d2f.worker.js"
	},
	3: function(A, t) {},
	"321b": function(A, t, e) {
		"use strict";
		(function(A) {
			var g = e("1da1"),
				i = (e("96cf"), e("e9c4"), e("d3b7"), e("159b"), e("3ca3"), e("ddb0"), e("2b3d"), e("9861"), e("ac1f"), e("1276"), e("25f0"), e("ab8f")),
				n = e("af85"),
				r = e("730d"),
				I = e("d4ec"),
				o = e("73ec"),
				a = e("cc74"),
				B = e("cb96");
			t["a"] = {
				name: "Home",
				components: {
					FileSelector: i["a"],
					PreviewTable: n["a"],
					ConfigDialog: r["a"],
					EditDialog: I["a"]
				},
				data: function() {
					return {
						showConfigDialog: !1,
						showEditDialog: !1,
						editing_data: {
							picture: "",
							title: "",
							artist: "",
							album: "",
							albumartist: "",
							genre: ""
						},
						tableData: [],
						playing_url: "",
						playing_auto: !1,
						filename_policy: o["e"].ArtistAndTitle,
						instant_save: !1,
						FilenamePolicies: o["d"],
						dir: null
					}
				},
				watch: {
					instant_save: function(A) {
						A && this.showDirectlySave()
					}
				},
				methods: {
					showSuccess: function(A) {
						var t = this;
						return Object(g["a"])(regeneratorRuntime.mark((function e() {
							var g;
							return regeneratorRuntime.wrap((function(e) {
								while (1) switch (e.prev = e.next) {
									case 0:
										if (!t.instant_save) {
											e.next = 6;
											break
										}
										return e.next = 3, t.saveFile(A);
									case 3:
										Object(o["f"])(A), e.next = 8;
										break;
									case 6:
										t.tableData.push(A), t.$notify.success({
											title: "解锁成功",
											message: "成功解锁 " + A.title,
											duration: 3e3
										});
									case 8:
										g = [A.title, A.artist, A.album], window._paq.push(["trackEvent", "Unlock", A.rawExt + "," + A.mime, JSON.stringify(g)]);
									case 9:
									case "end":
										return e.stop()
								}
							}), e)
						})))()
					},
					showFail: function(A, t) {
						console.error(A, t), this.$notify.error({
							title: "出现问题",
							message: A + "，" + t + '，参考<a target="_blank" href="https://github.com/ix64/unlock-music/wiki/使用提示">使用提示</a>',
							dangerouslyUseHTMLString: !0,
							duration: 6e3
						}), window._paq.push(["trackEvent", "Error", String(A), t])
					},
					changePlaying: function(A) {
						this.playing_url = A, this.playing_auto = !0
					},
					handleDeleteAll: function() {
						this.tableData.forEach((function(A) {
							Object(o["f"])(A)
						})), this.tableData = []
					},
					handleDecryptionConfig: function() {
						this.showConfigDialog = !0
					},
					handleDownloadAll: function() {
						var A = this,
							t = 0,
							e = setInterval((function() {
								t < A.tableData.length ? (A.saveFile(A.tableData[t]), t++) : clearInterval(e)
							}), 300)
					},
					handleEdit: function(t) {
						var e = this;
						return Object(g["a"])(regeneratorRuntime.mark((function g() {
							var i, n, r, I, o, C, Q, E;
							return regeneratorRuntime.wrap((function(g) {
								while (1) switch (g.prev = g.next) {
									case 0:
										return e.showEditDialog = !1, URL.revokeObjectURL(e.editing_data.file), t.picture && (URL.revokeObjectURL(e.editing_data.picture), e.editing_data.picture = URL.createObjectURL(t.picture)), e.editing_data.title = t.title, e.editing_data.artist = t.artist, e.editing_data.album = t.album, i = !0, n = "成功修改 " + e.editing_data.title, g.prev = 8, g.next = 11, Object(B["parseBlob"])(new Blob([e.editing_data.blob], {
											type: E
										}));
									case 11:
										if (I = g.sent, o = void 0, "" === e.editing_data.picture) {
											g.next = 18;
											break
										}
										return g.next = 16, Object(a["e"])(e.editing_data.picture);
									case 16:
										o = g.sent, o || console.warn("获取图像失败", e.editing_data.picture);
									case 18:
										return C = {
											picture: null === (r = o) || void 0 === r ? void 0 : r.buffer,
											title: t.title,
											artists: t.artist.split(a["m"]),
											album: t.album,
											albumartist: t.albumartist,
											genre: t.genre.split(a["m"])
										}, g.t0 = A, g.next = 22, e.editing_data.blob.arrayBuffer();
									case 22:
										g.t1 = g.sent, Q = g.t0.from.call(g.t0, g.t1), E = a["a"][e.editing_data.ext] || a["a"].mp3, "mp3" === e.editing_data.ext ? e.editing_data.blob = new Blob([Object(a["h"])(Q, C, I)], {
											type: E
										}) : "flac" === e.editing_data.ext ? e.editing_data.blob = new Blob([Object(a["g"])(Q, C, I)], {
											type: E
										}) : (i = void 0, n = e.editing_data.ext + "类型文件暂时不支持修改音乐标签"), g.next = 32;
										break;
									case 28:
										g.prev = 28, g.t2 = g["catch"](8), i = !1, n = "修改" + e.editing_data.title + "未能完成。在写入新的元数据时发生错误：" + g.t2;
									case 32:
										e.editing_data.file = URL.createObjectURL(e.editing_data.blob), !0 === i ? e.$notify.success({
											title: "修改成功",
											message: n,
											duration: 3e3
										}) : !1 === i ? e.$notify.error({
											title: "修改失败",
											message: n,
											duration: 3e3
										}) : e.$notify.warning({
											title: "修改取消",
											message: n,
											duration: 3e3
										});
									case 34:
									case "end":
										return g.stop()
								}
							}), g, null, [
								[8, 28]
							])
						})))()
					},
					editFile: function(A) {
						var t = this;
						return Object(g["a"])(regeneratorRuntime.mark((function e() {
							var g, i;
							return regeneratorRuntime.wrap((function(e) {
								while (1) switch (e.prev = e.next) {
									case 0:
										return t.editing_data = A, e.next = 3, Object(B["parseBlob"])(t.editing_data.blob);
									case 3:
										i = e.sent, t.editing_data.albumartist = i.common.albumartist || "", t.editing_data.genre = (null === (g = i.common.genre) || void 0 === g ? void 0 : g.toString()) || "", t.showEditDialog = !0;
									case 7:
									case "end":
										return e.stop()
								}
							}), e)
						})))()
					},
					saveFile: function(A) {
						var t = this;
						return Object(g["a"])(regeneratorRuntime.mark((function e() {
							return regeneratorRuntime.wrap((function(e) {
								while (1) switch (e.prev = e.next) {
									case 0:
										if (!t.dir) {
											e.next = 6;
											break
										}
										return e.next = 3, Object(o["b"])(A, t.filename_policy, t.dir);
									case 3:
										t.$notify({
											title: "保存成功",
											message: A.title,
											position: "top-left",
											type: "success",
											duration: 3e3
										}), e.next = 7;
										break;
									case 6:
										Object(o["c"])(A, t.filename_policy);
									case 7:
									case "end":
										return e.stop()
								}
							}), e)
						})))()
					},
					showDirectlySave: function() {
						var A = this;
						return Object(g["a"])(regeneratorRuntime.mark((function t() {
							var e;
							return regeneratorRuntime.wrap((function(t) {
								while (1) switch (t.prev = t.next) {
									case 0:
										if (window.showDirectoryPicker) {
											t.next = 2;
											break
										}
										return t.abrupt("return");
									case 2:
										return t.prev = 2, t.next = 5, A.$confirm("您的浏览器支持文件直接保存到磁盘，是否使用？", "新特性提示", {
											confirmButtonText: "使用",
											cancelButtonText: "不使用",
											type: "warning",
											center: !0
										});
									case 5:
										t.next = 11;
										break;
									case 7:
										return t.prev = 7, t.t0 = t["catch"](2), console.log(t.t0), t.abrupt("return");
									case 11:
										return t.prev = 11, t.next = 14, window.showDirectoryPicker();
									case 14:
										return A.dir = t.sent, e = "__unlock_music_write_test.txt", t.next = 18, A.dir.getFileHandle(e, {
											create: !0
										});
									case 18:
										return t.next = 20, A.dir.removeEntry(e);
									case 20:
										t.next = 25;
										break;
									case 22:
										t.prev = 22, t.t1 = t["catch"](11), console.error(t.t1);
									case 25:
									case "end":
										return t.stop()
								}
							}), t, null, [
								[2, 7],
								[11, 22]
							])
						})))()
					}
				}
			}
		}).call(this, e("1c35").Buffer)
	},
	4: function(A, t) {},
	5: function(A, t) {},
	"507a": function(A, t, e) {
		"use strict";
		e.d(t, "a", (function() {
			return tA
		}));
		e("d3b7"), e("b0c0");
		var g = e("9ab4"),
			i = e("9b2d"),
			n = e("6d95"),
			r = (e("ace4"), e("5cc6"), e("907a"), e("9a8c"), e("a975"), e("735e"), e("c1ac"), e("d139"), e("3a7b"), e("d5d6"), e("82f8"), e("e91f"), e("60bd"), e("5f96"), e("3280"), e("3fcc"), e("ca91"), e("25a1"), e("cd26"), e("3c5d"), e("2954"), e("649e"), e("219c"), e("170b"), e("b39a"), e("72f7"), e("3ca3"), e("ddb0"), e("2b3d"), e("9861"), e("cc74")),
			I = e("cb96");

		function o(A, t, e) {
			return Object(g["a"])(this, void 0, Promise, (function() {
				var i, n, o, a, B, C, Q, E, c;
				return Object(g["c"])(this, (function(g) {
					switch (g.label) {
						case 0:
							return n = Uint8Array.bind, [4, Object(r["c"])(A)];
						case 1:
							for (i = new(n.apply(Uint8Array, [void 0, g.sent()])), o = i.length, a = 0; a < o; a++) i[a] ^= 163;
							return B = Object(r["i"])(i, e), B !== e && (A = new Blob([i], {
								type: r["a"][B]
							})), [4, Object(I["parseBlob"])(A)];
						case 2:
							return C = g.sent(), Q = Object(r["f"])(t, C.common.title, String(C.common.artists || C.common.artist || "")), E = Q.title, c = Q.artist, [2, {
								title: E,
								artist: c,
								ext: B,
								album: C.common.album,
								picture: Object(r["d"])(C),
								file: URL.createObjectURL(A),
								blob: A,
								mime: r["a"][B]
							}]
					}
				}))
			}))
		}
		e("fb6a");
		var a = e("0ba7"),
			B = [105, 102, 109, 116],
			C = [254, 254, 254, 254],
			Q = {
				" WAV": ".wav",
				FLAC: ".flac",
				" MP3": ".mp3",
				" A4M": ".m4a"
			};

		function E(A, t, e) {
			return Object(g["a"])(this, void 0, Promise, (function() {
				var i, n, o, E, c, s, u, l, f, d, h, w, y, D, m;
				return Object(g["c"])(this, (function(g) {
					switch (g.label) {
						case 0:
							return n = Uint8Array.bind, [4, Object(r["c"])(A)];
						case 1:
							if (i = new(n.apply(Uint8Array, [void 0, g.sent()])), Object(r["b"])(i, B) && Object(r["b"])(i.slice(8, 12), C)) return [3, 4];
							if ("xm" !== e) return [3, 2];
							throw Error("此xm文件已损坏");
						case 2:
							return [4, Object(a["a"])(A, t, e, !0)];
						case 3:
							return [2, g.sent()];
						case 4:
							if (o = (new TextDecoder).decode(i.slice(4, 8)), !Q.hasOwnProperty(o)) throw Error("未知的.xm文件类型");
							for (E = i[15], c = i[12] | i[13] << 8 | i[14] << 16, s = i.slice(16), u = s.length, l = c; l < u; ++l) s[l] = s[l] - E ^ 255;
							return f = Q[o], d = r["a"][f], h = new Blob([s], {
								type: d
							}), [4, Object(I["parseBlob"])(h)];
						case 5:
							return w = g.sent(), "wav" === f && (console.info(w.common), w.common.album = "", w.common.artist = "", w.common.title = ""), y = Object(r["f"])(t, w.common.title, String(w.common.artists || w.common.artist || ""), -1 === t.indexOf("_") ? "-" : "_"), D = y.title, m = y.artist, [2, {
								title: D,
								artist: m,
								ext: f,
								mime: d,
								album: w.common.album,
								picture: Object(r["d"])(w),
								file: URL.createObjectURL(h),
								blob: h,
								rawExt: "xm"
							}]
					}
				}))
			}))
		}
		e("6c57"), e("c740");
		var c = function() {
			function A() {}
			return A.prototype.getMask = function(t) {
				return t > 32767 && (t %= 32767), A.staticCipherBox[t * t + 27 & 255]
			}, A.prototype.decrypt = function(A, t) {
				for (var e = 0; e < A.length; e++) A[e] ^= this.getMask(t + e)
			}, A.staticCipherBox = new Uint8Array([119, 72, 50, 115, 222, 242, 192, 200, 149, 236, 48, 178, 81, 195, 225, 160, 158, 230, 157, 207, 250, 127, 20, 209, 206, 184, 220, 195, 74, 103, 147, 214, 40, 194, 145, 112, 202, 141, 162, 164, 240, 8, 97, 144, 126, 111, 162, 224, 235, 174, 62, 182, 103, 199, 146, 244, 145, 181, 246, 108, 94, 132, 64, 247, 243, 27, 2, 127, 213, 171, 65, 137, 40, 244, 37, 204, 82, 17, 173, 67, 104, 166, 65, 139, 132, 181, 255, 44, 146, 74, 38, 216, 71, 106, 124, 149, 97, 204, 230, 203, 187, 63, 71, 88, 137, 117, 195, 117, 161, 217, 175, 204, 8, 115, 23, 220, 170, 154, 162, 22, 65, 216, 162, 6, 198, 139, 252, 102, 52, 159, 207, 24, 35, 160, 10, 116, 231, 43, 39, 112, 146, 233, 175, 55, 230, 140, 167, 188, 98, 101, 156, 194, 8, 201, 136, 179, 243, 67, 172, 116, 44, 15, 212, 175, 161, 195, 1, 100, 149, 78, 72, 159, 244, 53, 120, 149, 122, 57, 214, 106, 160, 109, 64, 232, 79, 168, 239, 17, 29, 243, 27, 63, 63, 7, 221, 111, 91, 25, 48, 25, 251, 239, 14, 55, 240, 14, 205, 22, 73, 254, 83, 71, 19, 26, 189, 164, 241, 64, 25, 96, 14, 237, 104, 9, 6, 95, 77, 207, 61, 26, 254, 32, 119, 228, 217, 218, 249, 164, 43, 118, 28, 113, 219, 0, 188, 253, 12, 108, 165, 71, 247, 246, 0, 121, 74, 17]), A
		}(),
			s = function() {
				function A(A) {
					if (0 == A.length) throw Error("qmc/cipher_map: invalid key size");
					this.key = A, this.n = A.length
				}
				return A.rotate = function(A, t) {
					var e = (t + 4) % 8,
						g = A << e,
						i = A >> e;
					return 255 & (g | i)
				}, A.prototype.decrypt = function(A, t) {
					for (var e = 0; e < A.length; e++) A[e] ^= this.getMask(t + e)
				}, A.prototype.getMask = function(t) {
					t > 32767 && (t %= 32767);
					var e = (t * t + 71214) % this.n;
					return A.rotate(this.key[e], 7 & e)
				}, A
			}(),
			u = function() {
				function A(A) {
					var t;
					if (0 == A.length) throw Error("invalid key size");
					this.key = A, this.N = A.length, this.S = new Uint8Array(this.N);
					for (var e = 0; e < this.N; ++e) this.S[e] = 255 & e;
					var g = 0;
					for (e = 0; e < this.N; ++e) g = (this.S[e] + g + this.key[e % this.N]) % this.N, t = [this.S[g], this.S[e]], this.S[e] = t[0], this.S[g] = t[1];
					this.hash = 1;
					for (e = 0; e < this.N; e++) {
						var i = this.key[e];
						if (i) {
							var n = this.hash * i >>> 0;
							if (0 == n || n <= this.hash) break;
							this.hash = n
						}
					}
				}
				return A.prototype.decrypt = function(t, e) {
					var g = t.length,
						i = 0,
						n = function(A) {
							return g -= A, i += A, e += A, 0 == g
						};
					if (e < A.FIRST_SEGMENT_SIZE) {
						var r = Math.min(t.length, A.FIRST_SEGMENT_SIZE - e);
						if (this.encFirstSegment(t.subarray(0, r), e), n(r)) return
					}
					if (e % A.SEGMENT_SIZE != 0) {
						r = Math.min(A.SEGMENT_SIZE - e % A.SEGMENT_SIZE, g);
						if (this.encASegment(t.subarray(i, i + r), e), n(r)) return
					}
					while (g > A.SEGMENT_SIZE) this.encASegment(t.subarray(i, i + A.SEGMENT_SIZE), e), n(A.SEGMENT_SIZE);
					g > 0 && this.encASegment(t.subarray(i), e)
				}, A.prototype.encFirstSegment = function(A, t) {
					for (var e = 0; e < A.length; e++) A[e] ^= this.key[this.getSegmentKey(t + e)]
				}, A.prototype.encASegment = function(t, e) {
					for (var g, i = this.S.slice(0), n = e % A.SEGMENT_SIZE + this.getSegmentKey(Math.floor(e / A.SEGMENT_SIZE)), r = 0, I = 0, o = -n; o < t.length; o++) r = (r + 1) % this.N, I = (i[r] + I) % this.N, g = [i[r], i[I]], i[I] = g[0], i[r] = g[1], o >= 0 && (t[o] ^= i[(i[r] + i[I]) % this.N])
				}, A.prototype.getSegmentKey = function(A) {
					var t = this.key[A % this.N],
						e = Math.floor(this.hash / ((A + 1) * t) * 100);
					return e % this.N
				}, A.FIRST_SEGMENT_SIZE = 128, A.SEGMENT_SIZE = 5120, A
			}(),
			l = e("fa01"),
			f = e("0b72"),
			d = e.n(f);
		e("159b");

		function h(A) {
			var t = 0;
			A.forEach((function(A) {
				t += A.length
			}));
			var e = new Uint8Array(t),
				g = 0;
			return A.forEach((function(A) {
				e.set(A, g), g += A.length
			})), e
		}
		var w = 2097152;

		function y(A, t) {
			return Object(g["a"])(this, void 0, Promise, (function() {
				var e, i, n, r, I, o, a, B, C, Q, E;
				return Object(g["c"])(this, (function(g) {
					switch (g.label) {
						case 0:
							e = {
								success: !1,
								data: new Uint8Array,
								songId: 0,
								error: ""
							}, g.label = 1;
						case 1:
							return g.trys.push([1, 3, , 4]), [4, d()()];
						case 2:
							return i = g.sent(), [3, 4];
						case 3:
							return n = g.sent(), e.error = (null === n || void 0 === n ? void 0 : n.message) || "wasm 加载失败", [2, e];
						case 4:
							if (!i) return e.error = "wasm 加载失败", [2, e];
							if (r = new Uint8Array(A), I = i._malloc(w), i.writeArrayToMemory(r.slice(-w), I), t = "." + t, o = i.preDec(I, w, t), -1 == o) return e.error = i.getError(), [2, e];
							e.songId = i.getSongId(), e.songId = "0" == e.songId ? 0 : e.songId, a = [], B = 0, C = r.length - o;
							while (C > 0) Q = Math.min(C, w), E = new Uint8Array(r.slice(B, B + Q)), i.writeArrayToMemory(E, I), a.push(i.HEAPU8.slice(I, I + i.decBlob(I, Q, B))), B += Q, C -= Q;
							return i._free(I), e.data = h(a), e.success = !0, [2, e]
					}
				}))
			}))
		}
		var D = e("0f92"),
			m = {
				mgg: {
					ext: "ogg",
					version: 2
				},
				mgg0: {
					ext: "ogg",
					version: 2
				},
				mggl: {
					ext: "ogg",
					version: 2
				},
				mgg1: {
					ext: "ogg",
					version: 2
				},
				mflac: {
					ext: "flac",
					version: 2
				},
				mflac0: {
					ext: "flac",
					version: 2
				},
				mmp4: {
					ext: "mmp4",
					version: 2
				},
				qmcflac: {
					ext: "flac",
					version: 2
				},
				qmcogg: {
					ext: "ogg",
					version: 2
				},
				qmc0: {
					ext: "mp3",
					version: 2
				},
				qmc2: {
					ext: "ogg",
					version: 2
				},
				qmc3: {
					ext: "mp3",
					version: 2
				},
				qmc4: {
					ext: "ogg",
					version: 2
				},
				qmc6: {
					ext: "ogg",
					version: 2
				},
				qmc8: {
					ext: "ogg",
					version: 2
				},
				bkcmp3: {
					ext: "mp3",
					version: 1
				},
				bkcm4a: {
					ext: "m4a",
					version: 1
				},
				bkcflac: {
					ext: "flac",
					version: 1
				},
				bkcwav: {
					ext: "wav",
					version: 1
				},
				bkcape: {
					ext: "ape",
					version: 1
				},
				bkcogg: {
					ext: "ogg",
					version: 1
				},
				bkcwma: {
					ext: "wma",
					version: 1
				},
				tkm: {
					ext: "m4a",
					version: 1
				},
				"666c6163": {
					ext: "flac",
					version: 1
				},
				"6d7033": {
					ext: "mp3",
					version: 1
				},
				"6f6767": {
					ext: "ogg",
					version: 1
				},
				"6d3461": {
					ext: "m4a",
					version: 1
				},
				776176: {
					ext: "wav",
					version: 1
				}
			};

		function b(A, t, e) {
			return Object(g["a"])(this, void 0, Promise, (function() {
				var i, n, I, o, a, B, C, Q, E, c, s, u, l, f, d;
				return Object(g["c"])(this, (function(g) {
					switch (g.label) {
						case 0:
							if (!(e in m)) throw "Qmc cannot handle type: ".concat(e);
							return i = m[e], n = i.version, [4, Object(r["c"])(A)];
						case 1:
							return I = g.sent(), 2 === n && globalThis.WebAssembly ? (console.log("qmc: using wasm decoder"), [4, y(I, e)]) : [3, 3];
						case 2:
							B = g.sent(), B.success ? (o = B.data, a = B.songId, console.log("qmc wasm decoder suceeded")) : console.warn("QmcWasm failed with error %s", B.error || "(unknown error)"), g.label = 3;
						case 3:
							return o || (console.log("qmc: using js decoder"), C = new p(new Uint8Array(I)), o = C.decrypt(), a = C.songID), Q = Object(r["i"])(o, i.ext), E = r["a"][Q], [4, Object(D["a"])(new Blob([o], {
								type: E
							}), t, Q, a)];
						case 4:
							return c = g.sent(), s = c.album, u = c.artist, l = c.imgUrl, f = c.blob, d = c.title, [2, {
								title: d,
								artist: u,
								ext: Q,
								album: s,
								picture: l,
								file: URL.createObjectURL(f),
								blob: f,
								mime: E
							}]
					}
				}))
			}))
		}
		var p = function() {
			function A(A) {
				this.decoded = !1, this.file = A, this.size = A.length, this.searchKey()
			}
			return Object.defineProperty(A.prototype, "songID", {
				get: function() {
					return this._songID
				},
				enumerable: !1,
				configurable: !0
			}), A.prototype.decrypt = function() {
				if (!this.cipher) throw new Error("no cipher found");
				if (!this.audioSize || this.audioSize <= 0) throw new Error("invalid audio size");
				var A = this.file.subarray(0, this.audioSize);
				return this.decoded || (this.cipher.decrypt(A, 0), this.decoded = !0), A
			}, A.prototype.searchKey = function() {
				var t = this.file.slice(-4),
					e = new TextDecoder;
				if ("STag" === e.decode(t)) throw new Error("文件中没有写入密钥，无法解锁，请降级App并重试");
				if ("QTag" === e.decode(t)) {
					var g = this.file.slice(-8, -4),
						i = new DataView(g.buffer, g.byteOffset),
						n = i.getUint32(0, !1);
					this.audioSize = this.size - n - 8;
					var r = this.file.subarray(this.audioSize, this.size - 8),
						I = r.findIndex((function(t) {
							return t == A.BYTE_COMMA
						}));
					if (I < 0) throw new Error("invalid key: search raw key failed");
					this.setCipher(r.subarray(0, I));
					var o = r.subarray(I + 1),
						a = o.findIndex((function(t) {
							return t == A.BYTE_COMMA
						}));
					if (I < 0) throw new Error("invalid key: search song id failed");
					this._songID = parseInt(e.decode(o.subarray(0, a)), 10)
				} else {
					i = new DataView(t.buffer, t.byteOffset), n = i.getUint32(0, !0);
					if (n < 1024) {
						this.audioSize = this.size - n - 4;
						r = this.file.subarray(this.audioSize, this.size - 4);
						this.setCipher(r)
					} else this.audioSize = this.size, this.cipher = new c
				}
			}, A.prototype.setCipher = function(A) {
				var t = Object(l["a"])(A);
				t.length > 300 ? this.cipher = new u(t) : this.cipher = new s(t)
			}, A.BYTE_COMMA = ",".charCodeAt(0), A
		}();

		function N(A, t, e) {
			return Object(g["a"])(this, void 0, Promise, (function() {
				var i, n, o, a, B, C, Q, E, c, s, u, l, f;
				return Object(g["c"])(this, (function(g) {
					switch (g.label) {
						case 0:
							return [4, Object(r["c"])(A)];
						case 1:
							return i = g.sent(), globalThis.WebAssembly ? (console.log("qmc: using wasm decoder"), [4, y(i, e)]) : [3, 3];
						case 2:
							o = g.sent(), o.success ? (n = o.data, console.log("qmc wasm decoder suceeded")) : console.warn("QmcWasm failed with error %s", o.error || "(unknown error)"), g.label = 3;
						case 3:
							if (!n)
								for (n = new Uint8Array(i), a = n.length, B = 0; B < a; B++) C = 244 ^ n[B], C = (63 & C) << 2 | C >> 6, n[B] = C;
							if (Q = Object(r["i"])(n, ""), E = Object(r["j"])(t), "" === Q && "mp3" !== E.ext) {
								if (E.ext in m) return c = new Blob([n], {
									type: "application/octet-stream"
								}), [2, b(c, E.name, E.ext)];
								throw "不支持的QQ音乐缓存格式"
							}
							return c = new Blob([n], {
								type: r["a"][Q]
							}), [4, Object(I["parseBlob"])(c)];
						case 4:
							return s = g.sent(), u = Object(r["f"])(t, s.common.title, String(s.common.artists || s.common.artist || "")), l = u.title, f = u.artist, [2, {
								title: l,
								artist: f,
								ext: Q,
								album: s.common.album,
								picture: Object(r["d"])(s),
								file: URL.createObjectURL(c),
								blob: c,
								mime: r["a"][Q]
							}]
					}
				}))
			}))
		}
		e("a630");
		var F = e("54ed"),
			v = e.n(F),
			R = 2097152;

		function G(A, t) {
			return Object(g["a"])(this, void 0, Promise, (function() {
				var e, i, n, r, I, o, a, B, C, Q, E;
				return Object(g["c"])(this, (function(g) {
					switch (g.label) {
						case 0:
							e = {
								success: !1,
								data: new Uint8Array,
								error: ""
							}, g.label = 1;
						case 1:
							return g.trys.push([1, 3, , 4]), [4, v()()];
						case 2:
							return i = g.sent(), [3, 4];
						case 3:
							return n = g.sent(), e.error = (null === n || void 0 === n ? void 0 : n.message) || "wasm 加载失败", [2, e];
						case 4:
							if (!i) return e.error = "wasm 加载失败", [2, e];
							r = new Uint8Array(A), I = i._malloc(R), i.writeArrayToMemory(r.slice(0, R), I), o = i.preDec(I, R, t), console.log(o), r = r.slice(o), a = [], B = 0, C = r.length;
							while (C > 0) Q = Math.min(C, R), E = new Uint8Array(r.slice(B, B + Q)), i.writeArrayToMemory(E, I), i.decBlob(I, Q, B), a.push(i.HEAPU8.slice(I, I + Q)), B += Q, C -= Q;
							return i._free(I), e.data = h(a), e.success = !0, [2, e]
					}
				}))
			}))
		}
		var k = e("2680"),
			U = [5, 40, 188, 150, 233, 228, 90, 67, 145, 170, 189, 208, 122, 245, 54, 49],
			S = [124, 213, 50, 235, 134, 2, 127, 75, 168, 175, 166, 142, 15, 255, 153, 20];

		function Y(A, t, e) {
			return Object(g["a"])(this, void 0, Promise, (function() {
				var i, n, o, a, B, C, Q, E, c, s, u, l, f, d, h, w;
				return Object(g["c"])(this, (function(g) {
					switch (g.label) {
						case 0:
							return [4, Object(r["c"])(A)];
						case 1:
							if (i = g.sent(), "vpr" === e) {
								if (!Object(r["b"])(new Uint8Array(i), U)) throw Error("Not a valid vpr file!")
							} else if (!Object(r["b"])(new Uint8Array(i), S)) throw Error("Not a valid kgm(a) file!");
							return globalThis.WebAssembly ? (console.log("kgm: using wasm decoder"), [4, G(i, e)]) : [3, 3];
						case 2:
							o = g.sent(), o.success ? (n = o.data, console.log("kgm wasm decoder suceeded")) : console.warn("KgmWasm failed with error %s", o.error || "(unknown error)"), g.label = 3;
						case 3:
							if (!n)
								for (n = new Uint8Array(i), a = new DataView(n.slice(16, 20).buffer), B = a.getUint32(0, !0), C = Array.from(n.slice(28, 44)), C.push(0), n = n.slice(B), Q = n.length, E = "vpr" === e ? k["decryptVprByteAtOffset"] : k["decryptKgmByteAtOffsetV2"], c = 0; c < Q; c++) n[c] = E(n[c], C, c);
							return s = Object(r["i"])(n), u = r["a"][s], l = new Blob([n], {
								type: u
							}), [4, Object(I["parseBlob"])(l)];
						case 4:
							return f = g.sent(), d = Object(r["f"])(t, f.common.title, String(f.common.artists || f.common.artist || "")), h = d.title, w = d.artist, [2, {
								album: f.common.album,
								picture: Object(r["d"])(f),
								file: URL.createObjectURL(l),
								blob: l,
								ext: s,
								mime: u,
								title: h,
								artist: w
							}]
					}
				}))
			}))
		}
		e("25f0"), e("843c");
		var J = [121, 101, 101, 108, 105, 111, 110, 45, 107, 117, 119, 111, 45, 116, 109, 101],
			M = [121, 101, 101, 108, 105, 111, 110, 45, 107, 117, 119, 111, 0, 0, 0, 0],
			L = "MoOtOiTvINGwd2E6n0E1i7L5t2IoOoNk";

		function H(A, t, e) {
			return Object(g["a"])(this, void 0, Promise, (function() {
				var e, i, n, o, B, C, Q, E, c, s, u, l, f, d;
				return Object(g["c"])(this, (function(g) {
					switch (g.label) {
						case 0:
							return i = Uint8Array.bind, [4, Object(r["c"])(A)];
						case 1:
							return e = new(i.apply(Uint8Array, [void 0, g.sent()])), Object(r["b"])(e, J) || Object(r["b"])(e, M) ? [3, 4] : "aac" !== Object(r["i"])(e) ? [3, 3] : [4, Object(a["a"])(A, t, "aac", !1)];
						case 2:
							return [2, g.sent()];
						case 3:
							throw Error("not a valid kwm file");
						case 4:
							for (n = e.slice(24, 32), o = K(n), B = e.slice(1024), C = B.length, Q = 0; Q < C; ++Q) B[Q] ^= o[Q % 32];
							return E = Object(r["i"])(B), c = r["a"][E], s = new Blob([B], {
								type: c
							}), [4, Object(I["parseBlob"])(s)];
						case 5:
							return u = g.sent(), l = Object(r["f"])(t, u.common.title, String(u.common.artists || u.common.artist || "")), f = l.title, d = l.artist, [2, {
								album: u.common.album,
								picture: Object(r["d"])(u),
								file: URL.createObjectURL(s),
								blob: s,
								mime: c,
								title: f,
								artist: d,
								ext: E
							}]
					}
				}))
			}))
		}

		function K(A) {
			for (var t = new DataView(A.buffer), e = t.getBigUint64(0, !0).toString(), g = x(e), i = new Uint8Array(32), n = 0; n < 32; n++) i[n] = L.charCodeAt(n) ^ g.charCodeAt(n);
			return i
		}

		function x(A) {
			var t = A.length,
				e = A;
			return t > 32 ? e = A.slice(0, 32) : t < 32 && (e = A.padEnd(32, A)), e
		}
		var j = [0, 0, 0, 32, 102, 116, 121, 112];

		function O(A, t) {
			return Object(g["a"])(this, void 0, Promise, (function() {
				var e, i, n, I;
				return Object(g["c"])(this, (function(g) {
					switch (g.label) {
						case 0:
							return i = Uint8Array.bind, [4, Object(r["c"])(A)];
						case 1:
							for (e = new(i.apply(Uint8Array, [void 0, g.sent()])), n = 0; n < 8; ++n) e[n] = j[n];
							return I = new Blob([e], {
								type: "audio/mp4"
							}), [4, Object(a["a"])(I, t, "m4a", !1)];
						case 2:
							return [2, g.sent()]
					}
				}))
			}))
		}
		e("ac1f"), e("466d");
		var T = e("dc71"),
			W = e.n(T),
			q = e("b7c7");

		function Z(A, t, e) {
			var i;
			return Object(g["a"])(this, void 0, Promise, (function() {
				var e, n, I, o, a, B, C, Q, E, c, s, u, l, f;
				return Object(g["c"])(this, (function(g) {
					switch (g.label) {
						case 0:
							return [4, q["a"].loadJooxUUID("")];
						case 1:
							if (e = g.sent(), !e || 32 !== e.length) throw new Error("请在“解密设定”填写应用 Joox 应用的 UUID。");
							return I = Uint8Array.bind, [4, Object(r["c"])(A)];
						case 2:
							if (n = new(I.apply(Uint8Array, [void 0, g.sent()])), o = W()(n, e), !o) throw new Error("不支持的 joox 加密格式");
							return a = h(o.decryptFile(n)), B = Object(r["i"])(a), C = r["a"][B], Q = null === (i = t.match(/^(\d+)\s\[mqms\d*]$/i)) || void 0 === i ? void 0 : i[1], [4, Object(D["a"])(new Blob([a], {
								type: C
							}), t, B, Q)];
						case 3:
							return E = g.sent(), c = E.album, s = E.artist, u = E.imgUrl, l = E.blob, f = E.title, [2, {
								title: f,
								artist: s,
								ext: B,
								album: c,
								picture: u,
								file: URL.createObjectURL(l),
								blob: l,
								mime: C
							}]
					}
				}))
			}))
		}
		e("4ec9");
		var X = new Map([
			["x2m", V],
			["x3m", _]
		]);

		function P(A, t, e) {
			return Object(g["a"])(this, void 0, Promise, (function() {
				var i, n, o, a, B, C, Q, E, c;
				return Object(g["c"])(this, (function(g) {
					switch (g.label) {
						case 0:
							return n = Uint8Array.bind, [4, Object(r["c"])(A)];
						case 1:
							if (i = new(n.apply(Uint8Array, [void 0, g.sent()])), o = X.get(e), !o) throw "File type is incorrect!";
							return a = o(i), B = Object(r["i"])(a, "m4a"), C = r["a"][B], Q = new Blob([a], {
								type: C
							}), [4, Object(I["parseBlob"])(Q)];
						case 2:
							return E = g.sent(), c = Object(r["f"])(t, E.common.title, E.common.artist), [2, {
								picture: "",
								title: c.title,
								artist: c.artist,
								ext: B,
								album: E.common.album,
								blob: Q,
								file: URL.createObjectURL(Q),
								mime: C
							}]
					}
				}))
			}))
		}

		function V(A) {
			for (var t = 1024, e = [120, 109, 108, 121], g = A.slice(0, t), i = 0; i < t; i++) {
				var n = z[i];
				A[i] = g[n] ^ e[i % e.length]
			}
			return A
		}

		function _(A) {
			for (var t = 1024, e = [51, 57, 56, 57, 100, 49, 49, 49, 97, 97, 100, 53, 54, 49, 51, 57, 52, 48, 102, 52, 102, 99, 52, 52, 98, 54, 51, 57, 98, 50, 57, 50], g = A.slice(0, t), i = 0; i < t; i++) {
				var n = $[i];
				A[i] = g[n] ^ e[i % e.length]
			}
			return A
		}
		var z = [681, 683, 340, 682, 680, 684, 339, 679, 685, 338, 678, 1023, 0, 341, 686, 337, 677, 1022, 1, 342, 687, 336, 676, 1021, 2, 343, 688, 335, 675, 1020, 3, 344, 689, 334, 674, 1019, 4, 345, 690, 333, 673, 1018, 5, 346, 691, 332, 672, 1017, 6, 347, 692, 331, 671, 1016, 7, 348, 693, 330, 670, 1015, 8, 349, 694, 329, 669, 1014, 9, 350, 695, 328, 668, 1013, 10, 351, 696, 327, 667, 1012, 11, 352, 697, 326, 666, 1011, 12, 353, 698, 325, 665, 1010, 13, 354, 699, 324, 664, 1009, 14, 355, 700, 323, 663, 1008, 15, 356, 701, 322, 662, 1007, 16, 357, 702, 321, 661, 1006, 17, 358, 703, 320, 660, 1005, 18, 359, 704, 319, 659, 1004, 19, 360, 705, 318, 658, 1003, 20, 361, 706, 317, 657, 1002, 21, 362, 707, 316, 656, 1001, 22, 363, 708, 315, 655, 1e3, 23, 364, 709, 314, 654, 999, 24, 365, 710, 313, 653, 998, 25, 366, 711, 312, 652, 997, 26, 367, 712, 311, 651, 996, 27, 368, 713, 310, 650, 995, 28, 369, 714, 309, 649, 994, 29, 370, 715, 308, 648, 993, 30, 371, 716, 307, 647, 992, 31, 372, 717, 169, 510, 855, 32, 373, 718, 170, 511, 856, 33, 374, 719, 171, 512, 857, 34, 375, 720, 172, 513, 858, 35, 376, 721, 173, 514, 859, 36, 377, 722, 174, 515, 860, 37, 378, 723, 175, 516, 861, 38, 379, 724, 176, 517, 862, 39, 380, 725, 177, 518, 863, 40, 381, 726, 178, 519, 864, 41, 382, 727, 179, 520, 865, 42, 383, 728, 180, 521, 866, 43, 384, 729, 181, 522, 867, 44, 385, 730, 182, 523, 868, 45, 386, 731, 183, 524, 869, 46, 387, 732, 184, 525, 870, 47, 388, 733, 185, 526, 871, 48, 389, 734, 186, 527, 872, 49, 390, 735, 187, 528, 873, 50, 391, 736, 188, 529, 874, 51, 392, 737, 189, 530, 875, 52, 393, 738, 190, 531, 876, 53, 394, 739, 191, 532, 877, 54, 395, 740, 192, 533, 878, 55, 396, 741, 193, 534, 879, 56, 397, 742, 194, 535, 880, 57, 398, 743, 195, 536, 881, 58, 399, 744, 196, 537, 882, 59, 400, 745, 197, 538, 883, 60, 401, 746, 198, 539, 884, 61, 402, 747, 199, 540, 885, 62, 403, 748, 200, 541, 886, 63, 404, 749, 201, 542, 887, 64, 405, 750, 202, 543, 888, 65, 406, 751, 203, 544, 889, 66, 407, 752, 204, 545, 890, 67, 408, 753, 205, 546, 891, 68, 409, 754, 206, 547, 892, 69, 410, 755, 207, 548, 893, 70, 411, 756, 208, 549, 894, 71, 412, 757, 209, 550, 895, 72, 413, 758, 210, 551, 896, 73, 414, 759, 211, 552, 897, 74, 415, 760, 212, 553, 898, 75, 416, 761, 213, 554, 899, 76, 417, 762, 214, 555, 900, 77, 418, 763, 215, 556, 901, 78, 419, 764, 216, 557, 902, 79, 420, 765, 217, 558, 903, 80, 421, 766, 218, 559, 904, 81, 422, 767, 219, 560, 905, 82, 423, 768, 220, 561, 906, 83, 424, 769, 221, 562, 907, 84, 425, 770, 222, 563, 908, 85, 426, 771, 223, 564, 909, 86, 427, 772, 224, 565, 910, 87, 428, 773, 225, 566, 911, 88, 429, 774, 226, 567, 912, 89, 430, 775, 227, 568, 913, 90, 431, 776, 228, 569, 914, 91, 432, 777, 229, 570, 915, 92, 433, 778, 230, 571, 916, 93, 434, 779, 231, 572, 917, 94, 435, 780, 232, 573, 918, 95, 436, 781, 233, 574, 919, 96, 437, 782, 234, 575, 920, 97, 438, 783, 235, 576, 921, 98, 439, 784, 236, 577, 922, 99, 440, 785, 237, 578, 923, 100, 441, 786, 238, 579, 924, 101, 442, 787, 239, 580, 925, 102, 443, 788, 240, 581, 926, 103, 444, 789, 241, 582, 927, 104, 445, 790, 242, 583, 928, 105, 446, 791, 243, 584, 929, 106, 447, 792, 244, 585, 930, 107, 448, 793, 245, 586, 931, 108, 449, 794, 246, 587, 932, 109, 450, 795, 247, 588, 933, 110, 451, 796, 248, 589, 934, 111, 452, 797, 249, 590, 935, 112, 453, 798, 250, 591, 936, 113, 454, 799, 251, 592, 937, 114, 455, 800, 252, 593, 938, 115, 456, 801, 253, 594, 939, 116, 457, 802, 254, 595, 940, 117, 458, 803, 255, 596, 941, 118, 459, 804, 256, 597, 942, 119, 460, 805, 257, 598, 943, 120, 461, 806, 258, 599, 944, 121, 462, 807, 259, 600, 945, 122, 463, 808, 260, 601, 946, 123, 464, 809, 261, 602, 947, 124, 465, 810, 262, 603, 948, 125, 466, 811, 263, 604, 949, 126, 467, 812, 264, 605, 950, 127, 468, 813, 265, 606, 951, 128, 469, 814, 266, 607, 952, 129, 470, 815, 267, 608, 953, 130, 471, 816, 268, 609, 954, 131, 472, 817, 269, 610, 955, 132, 473, 818, 270, 611, 956, 133, 474, 819, 271, 612, 957, 134, 475, 820, 272, 613, 958, 135, 476, 821, 273, 614, 959, 136, 477, 822, 274, 615, 960, 137, 478, 823, 275, 616, 961, 138, 479, 824, 276, 617, 962, 139, 480, 825, 277, 618, 963, 140, 481, 826, 278, 619, 964, 141, 482, 827, 279, 620, 965, 142, 483, 828, 280, 621, 966, 143, 484, 829, 281, 622, 967, 144, 485, 830, 282, 623, 968, 145, 486, 831, 283, 624, 969, 146, 487, 832, 284, 625, 970, 147, 488, 833, 285, 626, 971, 148, 489, 834, 286, 627, 972, 149, 490, 835, 287, 628, 973, 150, 491, 836, 288, 629, 974, 151, 492, 837, 289, 630, 975, 152, 493, 838, 290, 631, 976, 153, 494, 839, 291, 632, 977, 154, 495, 840, 292, 633, 978, 155, 496, 841, 293, 634, 979, 156, 497, 842, 294, 635, 980, 157, 498, 843, 295, 636, 981, 158, 499, 844, 296, 637, 982, 159, 500, 845, 297, 638, 983, 160, 501, 846, 298, 639, 984, 161, 502, 847, 299, 640, 985, 162, 503, 848, 300, 641, 986, 163, 504, 849, 301, 642, 987, 164, 505, 850, 302, 643, 988, 165, 506, 851, 303, 644, 989, 166, 507, 852, 304, 645, 990, 167, 508, 853, 305, 646, 991, 168, 509, 854, 306],
			$ = [598, 653, 531, 775, 342, 925, 98, 368, 970, 53, 237, 676, 484, 857, 211, 619, 613, 628, 593, 663, 514, 802, 294, 811, 279, 770, 348, 936, 87, 328, 896, 144, 502, 821, 268, 750, 373, 980, 43, 204, 608, 635, 573, 699, 438, 929, 94, 343, 926, 97, 367, 966, 57, 247, 697, 440, 927, 96, 358, 953, 70, 290, 796, 303, 829, 252, 714, 420, 972, 51, 230, 659, 521, 789, 317, 856, 213, 622, 606, 637, 570, 704, 433, 943, 80, 310, 838, 239, 682, 462, 886, 160, 528, 780, 332, 905, 130, 475, 871, 185, 574, 698, 439, 928, 95, 356, 951, 72, 293, 806, 284, 778, 335, 911, 112, 424, 967, 56, 245, 693, 445, 915, 108, 409, 993, 30, 179, 559, 727, 403, 1002, 21, 157, 522, 788, 318, 858, 210, 618, 615, 626, 595, 660, 520, 793, 311, 844, 231, 661, 517, 797, 302, 828, 254, 717, 416, 981, 42, 200, 600, 646, 554, 732, 398, 1015, 8, 124, 467, 880, 167, 541, 753, 369, 973, 50, 229, 658, 523, 787, 319, 860, 208, 614, 627, 594, 662, 516, 799, 298, 818, 271, 756, 364, 963, 60, 257, 722, 410, 992, 31, 181, 563, 713, 422, 969, 54, 240, 683, 459, 892, 149, 509, 808, 282, 774, 344, 930, 93, 341, 924, 99, 372, 979, 44, 207, 612, 629, 591, 665, 506, 812, 277, 767, 351, 939, 84, 323, 876, 173, 549, 741, 385, 1007, 16, 140, 497, 836, 243, 687, 452, 902, 136, 483, 859, 209, 617, 616, 621, 607, 636, 571, 703, 434, 942, 81, 315, 853, 218, 632, 584, 678, 476, 869, 192, 582, 680, 470, 877, 172, 548, 744, 382, 1003, 20, 156, 519, 794, 307, 833, 248, 700, 437, 931, 92, 338, 917, 106, 396, 1017, 6, 122, 465, 883, 164, 535, 766, 352, 941, 82, 316, 855, 215, 624, 604, 641, 565, 710, 426, 956, 67, 285, 781, 330, 900, 138, 487, 851, 221, 644, 558, 728, 402, 1004, 19, 153, 513, 803, 292, 801, 295, 813, 276, 765, 353, 944, 79, 309, 835, 244, 692, 446, 914, 109, 413, 987, 36, 190, 580, 688, 450, 906, 128, 473, 873, 182, 564, 712, 423, 968, 55, 241, 685, 454, 899, 141, 498, 827, 256, 721, 411, 990, 33, 187, 576, 694, 443, 921, 102, 378, 991, 32, 184, 572, 701, 436, 933, 90, 336, 912, 111, 421, 971, 52, 234, 669, 494, 840, 236, 675, 485, 854, 216, 625, 599, 649, 544, 748, 376, 985, 38, 194, 587, 673, 490, 845, 228, 657, 524, 786, 321, 864, 202, 602, 643, 560, 720, 412, 989, 34, 188, 577, 691, 447, 913, 110, 418, 977, 46, 214, 623, 605, 639, 567, 708, 428, 954, 69, 289, 792, 312, 846, 227, 655, 529, 779, 333, 908, 115, 451, 903, 132, 479, 866, 199, 597, 654, 530, 777, 339, 918, 105, 395, 1018, 5, 121, 464, 884, 162, 533, 769, 349, 937, 86, 327, 890, 152, 512, 804, 287, 790, 314, 850, 223, 648, 547, 745, 381, 1001, 22, 158, 525, 784, 324, 882, 165, 537, 762, 357, 952, 71, 291, 798, 301, 824, 263, 736, 392, 1022, 1, 117, 457, 894, 147, 505, 815, 274, 761, 359, 958, 65, 267, 743, 383, 1005, 18, 151, 511, 805, 286, 785, 322, 870, 186, 575, 696, 441, 923, 100, 374, 982, 41, 197, 592, 664, 508, 809, 281, 772, 346, 934, 89, 334, 910, 113, 429, 950, 73, 296, 814, 275, 764, 354, 946, 77, 305, 831, 250, 706, 431, 947, 76, 304, 830, 251, 711, 425, 957, 66, 278, 768, 350, 938, 85, 326, 888, 155, 518, 795, 306, 832, 249, 702, 435, 940, 83, 320, 861, 206, 610, 633, 583, 679, 471, 875, 174, 550, 739, 389, 1014, 9, 125, 468, 879, 168, 542, 752, 370, 974, 49, 222, 647, 552, 735, 393, 1021, 2, 118, 458, 893, 148, 507, 810, 280, 771, 347, 935, 88, 331, 904, 131, 477, 868, 193, 586, 674, 489, 848, 225, 651, 538, 760, 360, 959, 64, 266, 742, 384, 1006, 17, 145, 503, 820, 269, 751, 371, 975, 48, 220, 640, 566, 709, 427, 955, 68, 288, 791, 313, 847, 226, 652, 536, 763, 355, 948, 75, 300, 823, 264, 738, 390, 1020, 3, 119, 460, 891, 150, 510, 807, 283, 776, 340, 919, 104, 387, 1011, 12, 133, 480, 865, 201, 601, 645, 556, 730, 400, 1010, 13, 134, 481, 863, 203, 603, 642, 562, 716, 417, 978, 45, 212, 620, 611, 631, 585, 677, 478, 867, 198, 596, 656, 526, 783, 325, 887, 159, 527, 782, 329, 898, 142, 499, 826, 258, 723, 408, 994, 29, 178, 557, 729, 401, 1009, 14, 135, 482, 862, 205, 609, 634, 579, 689, 449, 907, 127, 472, 874, 180, 561, 719, 414, 986, 37, 191, 581, 684, 455, 897, 143, 501, 822, 265, 740, 388, 1012, 11, 129, 474, 872, 183, 569, 705, 432, 945, 78, 308, 834, 246, 695, 442, 922, 101, 377, 988, 35, 189, 578, 690, 448, 909, 114, 444, 920, 103, 386, 1008, 15, 139, 488, 849, 224, 650, 540, 755, 365, 964, 59, 255, 718, 415, 983, 40, 196, 590, 667, 496, 837, 242, 686, 453, 901, 137, 486, 852, 219, 638, 568, 707, 430, 949, 74, 299, 819, 270, 754, 366, 965, 58, 253, 715, 419, 976, 47, 217, 630, 588, 671, 492, 842, 233, 668, 495, 839, 238, 681, 463, 885, 161, 532, 773, 345, 932, 91, 337, 916, 107, 406, 998, 25, 171, 546, 746, 380, 997, 26, 175, 551, 737, 391, 1023, 0, 116, 456, 895, 146, 504, 817, 272, 758, 362, 961, 62, 260, 725, 405, 999, 24, 170, 545, 747, 379, 996, 27, 176, 553, 733, 397, 1016, 7, 123, 466, 881, 166, 539, 757, 363, 962, 61, 259, 724, 407, 995, 28, 177, 555, 731, 399, 1013, 10, 126, 469, 878, 169, 543, 749, 375, 984, 39, 195, 589, 670, 493, 841, 235, 672, 491, 843, 232, 666, 500, 825, 262, 734, 394, 1019, 4, 120, 461, 889, 154, 515, 800, 297, 816, 273, 759, 361, 960, 63, 261, 726, 404, 1e3, 23, 163, 534],
			AA = e("efc0");

		function tA(A, t) {
			return Object(g["a"])(this, void 0, Promise, (function() {
				var e, I, B;
				return Object(g["c"])(this, (function(g) {
					switch (g.label) {
						case 0:
							return q["a"] instanceof AA["a"] ? [4, q["a"].setAll(t)] : [3, 2];
						case 1:
							g.sent(), g.label = 2;
						case 2:
							switch (e = Object(r["j"])(A.name), B = e.ext, B) {
								case "mg3d":
									return [3, 3];
								case "ncm":
									return [3, 5];
								case "uc":
									return [3, 7];
								case "kwm":
									return [3, 9];
								case "xm":
									return [3, 11];
								case "wav":
									return [3, 11];
								case "mp3":
									return [3, 11];
								case "flac":
									return [3, 11];
								case "m4a":
									return [3, 11];
								case "ogg":
									return [3, 13];
								case "tm0":
									return [3, 15];
								case "tm3":
									return [3, 15];
								case "qmc0":
									return [3, 17];
								case "qmc3":
									return [3, 17];
								case "qmc2":
									return [3, 17];
								case "qmc4":
									return [3, 17];
								case "qmc6":
									return [3, 17];
								case "qmc8":
									return [3, 17];
								case "qmcflac":
									return [3, 17];
								case "qmcogg":
									return [3, 17];
								case "tkm":
									return [3, 17];
								case "bkcmp3":
									return [3, 17];
								case "bkcm4a":
									return [3, 17];
								case "bkcflac":
									return [3, 17];
								case "bkcwav":
									return [3, 17];
								case "bkcape":
									return [3, 17];
								case "bkcogg":
									return [3, 17];
								case "bkcwma":
									return [3, 17];
								case "mggl":
									return [3, 17];
								case "mflac":
									return [3, 17];
								case "mflac0":
									return [3, 17];
								case "mflach":
									return [3, 17];
								case "mgg":
									return [3, 17];
								case "mgg1":
									return [3, 17];
								case "mgg0":
									return [3, 17];
								case "mmp4":
									return [3, 17];
								case "666c6163":
									return [3, 17];
								case "6d7033":
									return [3, 17];
								case "6f6767":
									return [3, 17];
								case "6d3461":
									return [3, 17];
								case "776176":
									return [3, 17];
								case "tm2":
									return [3, 19];
								case "tm6":
									return [3, 19];
								case "cache":
									return [3, 21];
								case "vpr":
									return [3, 23];
								case "kgm":
									return [3, 23];
								case "kgma":
									return [3, 23];
								case "ofl_en":
									return [3, 25];
								case "x2m":
									return [3, 27];
								case "x3m":
									return [3, 27];
								case "mflach":
									return [3, 29]
							}
							return [3, 30];
						case 3:
							return [4, Object(i["a"])(A.raw, e.name)];
						case 4:
							return I = g.sent(), [3, 31];
						case 5:
							return [4, Object(n["a"])(A.raw, e.name, e.ext)];
						case 6:
							return I = g.sent(), [3, 31];
						case 7:
							return [4, o(A.raw, e.name, e.ext)];
						case 8:
							return I = g.sent(), [3, 31];
						case 9:
							return [4, H(A.raw, e.name, e.ext)];
						case 10:
							return I = g.sent(), [3, 31];
						case 11:
							return [4, E(A.raw, e.name, e.ext)];
						case 12:
							return I = g.sent(), [3, 31];
						case 13:
							return [4, Object(a["a"])(A.raw, e.name, e.ext)];
						case 14:
							return I = g.sent(), [3, 31];
						case 15:
							return [4, Object(a["a"])(A.raw, e.name, "mp3")];
						case 16:
							return I = g.sent(), [3, 31];
						case 17:
							return [4, b(A.raw, e.name, e.ext)];
						case 18:
							return I = g.sent(), [3, 31];
						case 19:
							return [4, O(A.raw, e.name)];
						case 20:
							return I = g.sent(), [3, 31];
						case 21:
							return [4, N(A.raw, e.name, e.ext)];
						case 22:
							return I = g.sent(), [3, 31];
						case 23:
							return [4, Y(A.raw, e.name, e.ext)];
						case 24:
							return I = g.sent(), [3, 31];
						case 25:
							return [4, Z(A.raw, e.name, e.ext)];
						case 26:
							return I = g.sent(), [3, 31];
						case 27:
							return [4, P(A.raw, e.name, e.ext)];
						case 28:
							return I = g.sent(), [3, 31];
						case 29:
							throw '网页版无法解锁，请使用<a target="_blank" href="https://git.unlock-music.dev/um/cli">CLI版本</a>';
						case 30:
							throw "不支持此文件格式";
						case 31:
							return I.rawExt || (I.rawExt = e.ext), I.rawFilename || (I.rawFilename = e.name), console.log(I), [2, I]
					}
				}))
			}))
		}
	},
	"54ed": function(A, t, e) {
		(function(A, g, i, n, r) {
			var I, o, a = e("7037").
			default;
			e("d3b7"), e("25f0"), e("ace4"), e("5cc6"), e("907a"), e("9a8c"), e("a975"), e("735e"), e("c1ac"), e("d139"), e("3a7b"), e("d5d6"), e("82f8"), e("e91f"), e("60bd"), e("5f96"), e("3280"), e("3fcc"), e("ca91"), e("25a1"), e("cd26"), e("3c5d"), e("2954"), e("649e"), e("219c"), e("170b"), e("b39a"), e("72f7"), e("ac1f"), e("5319"), e("fb6a"), e("a9e3"), e("fd87"), e("8b09"), e("143c"), e("84c3"), e("fb2c"), e("cfc3"), e("4a9b"), e("2ca0"), e("b0c0"), e("159b"), e("99af"), e("caad"), e("2532"), e("a15b"), e("d81d"), e("c19f"), e("8a59"), e("a874");
			var B = function() {
				var t = "undefined" !== typeof document && document.currentScript ? document.currentScript.src : void 0;
				return t = t || A,
				function(A) {
					A = A || {};
					var r, I, o = "undefined" !== typeof A ? A : {};
					o["ready"] = new Promise((function(A, t) {
						r = A, I = t
					}));
					var B, C = {};
					for (B in o) o.hasOwnProperty(B) && (C[B] = o[B]);
					var Q, E, c, s, u, l = [],
						f = "object" === ("undefined" === typeof window ? "undefined" : a(window)),
						d = "function" === typeof importScripts,
						h = "object" === ("undefined" === typeof g ? "undefined" : a(g)) && "object" === a(g.versions) && "string" === typeof g.versions.node,
						w = "";

					function y(A) {
						return o["locateFile"] ? o["locateFile"](A, w) : w + A
					}

					function D(A) {
						if (!(A instanceof xt)) {
							var t = A;
							b("exiting due to exception: " + t)
						}
					}
					h ? (w = d ? e("df7c").dirname(w) + "/" : i + "/", Q = function(A, t) {
						var g = Yt(A);
						return g ? t ? g : g.toString() : (s || (s = e("3e8f")), u || (u = e("df7c")), A = u["normalize"](A), s["readFileSync"](A, t ? null : "utf8"))
					}, c = function(A) {
						var t = Q(A, !0);
						return t.buffer || (t = new Uint8Array(t)), R(t.buffer), t
					}, E = function(A, t, g) {
						var i = Yt(A);
						i && t(i), s || (s = e("3e8f")), u || (u = e("df7c")), A = u["normalize"](A), s["readFile"](A, (function(A, e) {
							A ? g(A) : t(e.buffer)
						}))
					}, g["argv"].length > 1 && g["argv"][1].replace(/\\/g, "/"), l = g["argv"].slice(2), g["on"]("uncaughtException", (function(A) {
						if (!(A instanceof xt)) throw A
					})), g["on"]("unhandledRejection", (function(A) {
						throw A
					})), function(A, t) {
						if (rA()) throw g["exitCode"] = A, t;
						D(t), g["exit"](A)
					}, o["inspect"] = function() {
						return "[Emscripten Module object]"
					}) : (f || d) && (d ? w = self.location.href : "undefined" !== typeof document && document.currentScript && (w = document.currentScript.src), t && (w = t), w = 0 !== w.indexOf("blob:") ? w.substr(0, w.replace(/[?#].*/, "").lastIndexOf("/") + 1) : "", Q = function(A) {
						try {
							var t = new XMLHttpRequest;
							return t.open("GET", A, !1), t.send(null), t.responseText
						} catch (b) {
							var e = Yt(A);
							if (e) return kt(e);
							throw b
						}
					}, d && (c = function(A) {
						try {
							var t = new XMLHttpRequest;
							return t.open("GET", A, !1), t.responseType = "arraybuffer", t.send(null), new Uint8Array(t.response)
						} catch (b) {
							var e = Yt(A);
							if (e) return e;
							throw b
						}
					}), E = function(A, t, e) {
						var g = new XMLHttpRequest;
						g.open("GET", A, !0), g.responseType = "arraybuffer", g.onload = function() {
							if (200 == g.status || 0 == g.status && g.response) t(g.response);
							else {
								var i = Yt(A);
								i ? t(i.buffer) : e()
							}
						}, g.onerror = e, g.send(null)
					});
					o["print"] || console.log.bind(console);
					var m, b = o["printErr"] || console.warn.bind(console);
					for (B in C) C.hasOwnProperty(B) && (o[B] = C[B]);
					C = null, o["arguments"] && (l = o["arguments"]), o["thisProgram"] && o["thisProgram"], o["quit"] && o["quit"], o["wasmBinary"] && (m = o["wasmBinary"]);
					var p, N = o["noExitRuntime"] || !0;

					function F(A, t, e) {
						switch (t = t || "i8", "*" === t.charAt(t.length - 1) && (t = "i32"), t) {
							case "i1":
								return L[A >> 0];
							case "i8":
								return L[A >> 0];
							case "i16":
								return K[A >> 1];
							case "i32":
								return j[A >> 2];
							case "i64":
								return j[A >> 2];
							case "float":
								return T[A >> 2];
							case "double":
								return Number(W[A >> 3]);
							default:
								fA("invalid type for getValue: " + t)
						}
						return null
					}
					"object" !== ("undefined" === typeof WebAssembly ? "undefined" : a(WebAssembly)) && fA("no native wasm support detected");
					var v = !1;

					function R(A, t) {
						A || fA("Assertion failed: " + t)
					}
					var G = "undefined" !== typeof TextDecoder ? new TextDecoder("utf8") : void 0;

					function k(A, t, e) {
						var g = t + e,
							i = t;
						while (A[i] && !(i >= g))++i;
						if (i - t > 16 && A.subarray && G) return G.decode(A.subarray(t, i));
						var n = "";
						while (t < i) {
							var r = A[t++];
							if (128 & r) {
								var I = 63 & A[t++];
								if (192 != (224 & r)) {
									var o = 63 & A[t++];
									if (r = 224 == (240 & r) ? (15 & r) << 12 | I << 6 | o : (7 & r) << 18 | I << 12 | o << 6 | 63 & A[t++], r < 65536) n += String.fromCharCode(r);
									else {
										var a = r - 65536;
										n += String.fromCharCode(55296 | a >> 10, 56320 | 1023 & a)
									}
								} else n += String.fromCharCode((31 & r) << 6 | I)
							} else n += String.fromCharCode(r)
						}
						return n
					}

					function U(A, t) {
						return A ? k(H, A, t) : ""
					}

					function S(A, t, e, g) {
						if (!(g > 0)) return 0;
						for (var i = e, n = e + g - 1, r = 0; r < A.length; ++r) {
							var I = A.charCodeAt(r);
							if (I >= 55296 && I <= 57343) {
								var o = A.charCodeAt(++r);
								I = 65536 + ((1023 & I) << 10) | 1023 & o
							}
							if (I <= 127) {
								if (e >= n) break;
								t[e++] = I
							} else if (I <= 2047) {
								if (e + 1 >= n) break;
								t[e++] = 192 | I >> 6, t[e++] = 128 | 63 & I
							} else if (I <= 65535) {
								if (e + 2 >= n) break;
								t[e++] = 224 | I >> 12, t[e++] = 128 | I >> 6 & 63, t[e++] = 128 | 63 & I
							} else {
								if (e + 3 >= n) break;
								t[e++] = 240 | I >> 18, t[e++] = 128 | I >> 12 & 63, t[e++] = 128 | I >> 6 & 63, t[e++] = 128 | 63 & I
							}
						}
						return t[e] = 0, e - i
					}

					function Y(A, t, e) {
						return S(A, H, t, e)
					}

					function J(A) {
						for (var t = 0, e = 0; e < A.length; ++e) {
							var g = A.charCodeAt(e);
							g >= 55296 && g <= 57343 && (g = 65536 + ((1023 & g) << 10) | 1023 & A.charCodeAt(++e)), g <= 127 ? ++t : t += g <= 2047 ? 2 : g <= 65535 ? 3 : 4
						}
						return t
					}
					var M, L, H, K, x, j, O, T, W, q = "undefined" !== typeof TextDecoder ? new TextDecoder("utf-16le") : void 0;

					function Z(A, t) {
						var e = A,
							g = e >> 1,
							i = g + t / 2;
						while (!(g >= i) && x[g])++g;
						if (e = g << 1, e - A > 32 && q) return q.decode(H.subarray(A, e));
						for (var n = "", r = 0; !(r >= t / 2); ++r) {
							var I = K[A + 2 * r >> 1];
							if (0 == I) break;
							n += String.fromCharCode(I)
						}
						return n
					}

					function X(A, t, e) {
						if (void 0 === e && (e = 2147483647), e < 2) return 0;
						e -= 2;
						for (var g = t, i = e < 2 * A.length ? e / 2 : A.length, n = 0; n < i; ++n) {
							var r = A.charCodeAt(n);
							K[t >> 1] = r, t += 2
						}
						return K[t >> 1] = 0, t - g
					}

					function P(A) {
						return 2 * A.length
					}

					function V(A, t) {
						var e = 0,
							g = "";
						while (!(e >= t / 4)) {
							var i = j[A + 4 * e >> 2];
							if (0 == i) break;
							if (++e, i >= 65536) {
								var n = i - 65536;
								g += String.fromCharCode(55296 | n >> 10, 56320 | 1023 & n)
							} else g += String.fromCharCode(i)
						}
						return g
					}

					function _(A, t, e) {
						if (void 0 === e && (e = 2147483647), e < 4) return 0;
						for (var g = t, i = g + e - 4, n = 0; n < A.length; ++n) {
							var r = A.charCodeAt(n);
							if (r >= 55296 && r <= 57343) {
								var I = A.charCodeAt(++n);
								r = 65536 + ((1023 & r) << 10) | 1023 & I
							}
							if (j[t >> 2] = r, t += 4, t + 4 > i) break
						}
						return j[t >> 2] = 0, t - g
					}

					function z(A) {
						for (var t = 0, e = 0; e < A.length; ++e) {
							var g = A.charCodeAt(e);
							g >= 55296 && g <= 57343 && ++e, t += 4
						}
						return t
					}

					function $(A, t) {
						L.set(A, t)
					}

					function AA(A) {
						M = A, o["HEAP8"] = L = new Int8Array(A), o["HEAP16"] = K = new Int16Array(A), o["HEAP32"] = j = new Int32Array(A), o["HEAPU8"] = H = new Uint8Array(A), o["HEAPU16"] = x = new Uint16Array(A), o["HEAPU32"] = O = new Uint32Array(A), o["HEAPF32"] = T = new Float32Array(A), o["HEAPF64"] = W = new Float64Array(A)
					}
					o["INITIAL_MEMORY"];
					var tA, eA = [],
						gA = [],
						iA = [],
						nA = 0;

					function rA() {
						return N || nA > 0
					}

					function IA() {
						if (o["preRun"]) {
							"function" == typeof o["preRun"] && (o["preRun"] = [o["preRun"]]);
							while (o["preRun"].length) BA(o["preRun"].shift())
						}
						pA(eA)
					}

					function oA() {
						!0, pA(gA)
					}

					function aA() {
						if (o["postRun"]) {
							"function" == typeof o["postRun"] && (o["postRun"] = [o["postRun"]]);
							while (o["postRun"].length) QA(o["postRun"].shift())
						}
						pA(iA)
					}

					function BA(A) {
						eA.unshift(A)
					}

					function CA(A) {
						gA.unshift(A)
					}

					function QA(A) {
						iA.unshift(A)
					}
					var EA = 0,
						cA = null,
						sA = null;

					function uA(A) {
						EA++, o["monitorRunDependencies"] && o["monitorRunDependencies"](EA)
					}

					function lA(A) {
						if (EA--, o["monitorRunDependencies"] && o["monitorRunDependencies"](EA), 0 == EA && (null !== cA && (clearInterval(cA), cA = null), sA)) {
							var t = sA;
							sA = null, t()
						}
					}

					function fA(A) {
						o["onAbort"] && o["onAbort"](A), A = "Aborted(" + A + ")", b(A), v = !0, 1, A += ". Build with -s ASSERTIONS=1 for more info.";
						var t = new WebAssembly.RuntimeError(A);
						throw I(t), t
					}
					o["preloadedImages"] = {}, o["preloadedAudios"] = {};
					var dA, hA = "data:application/octet-stream;base64,";

					function wA(A) {
						return A.startsWith(hA)
					}

					function yA(A) {
						return A.startsWith("file://")
					}

					function DA(A) {
						try {
							if (A == dA && m) return new Uint8Array(m);
							var t = Yt(A);
							if (t) return t;
							if (c) return c(A);
							throw "both async and sync fetching of the wasm failed"
						} catch (b) {
							fA(b)
						}
					}

					function mA() {
						if (!m && (f || d)) {
							if ("function" === typeof fetch && !yA(dA)) return fetch(dA, {
								credentials: "same-origin"
							}).then((function(A) {
								if (!A["ok"]) throw "failed to load wasm binary file at '" + dA + "'";
								return A["arrayBuffer"]()
							})).
							catch ((function() {
								return DA(dA)
							}));
							if (E) return new Promise((function(A, t) {
								E(dA, (function(t) {
									A(new Uint8Array(t))
								}), t)
							}))
						}
						return Promise.resolve().then((function() {
							return DA(dA)
						}))
					}

					function bA() {
						var A = {
							a: Mt
						};

						function t(A, t) {
							var e = A.exports;
							o["asm"] = e, p = o["asm"]["n"], AA(p.buffer), tA = o["asm"]["p"], CA(o["asm"]["o"]), lA("wasm-instantiate")
						}

						function e(A) {
							t(A["instance"])
						}

						function g(t) {
							return mA().then((function(t) {
								return WebAssembly.instantiate(t, A)
							})).then((function(A) {
								return A
							})).then(t, (function(A) {
								b("failed to asynchronously prepare wasm: " + A), fA(A)
							}))
						}

						function i() {
							return m || "function" !== typeof WebAssembly.instantiateStreaming || wA(dA) || yA(dA) || "function" !== typeof fetch ? g(e) : fetch(dA, {
								credentials: "same-origin"
							}).then((function(t) {
								var i = WebAssembly.instantiateStreaming(t, A);
								return i.then(e, (function(A) {
									return b("wasm streaming compile failed: " + A), b("falling back to ArrayBuffer instantiation"), g(e)
								}))
							}))
						}
						if (uA("wasm-instantiate"), o["instantiateWasm"]) try {
							var n = o["instantiateWasm"](A, t);
							return n
						} catch (r) {
							return b("Module.instantiateWasm callback failed with error: " + r), !1
						}
						return i().
						catch (I), {}
					}

					function pA(A) {
						while (A.length > 0) {
							var t = A.shift();
							if ("function" != typeof t) {
								var e = t.func;
								"number" === typeof e ? void 0 === t.arg ? FA(e)() : FA(e)(t.arg) : e(void 0 === t.arg ? null : t.arg)
							} else t(o)
						}
					}
					dA = "data:application/octet-stream;base64,", wA(dA) || (dA = y(dA));
					var NA = [];

					function FA(A) {
						var t = NA[A];
						return t || (A >= NA.length && (NA.length = A + 1), NA[A] = t = tA.get(A)), t
					}

					function vA(A, t, e, g, i) {}

					function RA(A) {
						switch (A) {
							case 1:
								return 0;
							case 2:
								return 1;
							case 4:
								return 2;
							case 8:
								return 3;
							default:
								throw new TypeError("Unknown type size: " + A)
						}
					}

					function GA() {
						for (var A = new Array(256), t = 0; t < 256; ++t) A[t] = String.fromCharCode(t);
						kA = A
					}
					var kA = void 0;

					function UA(A) {
						var t = "",
							e = A;
						while (H[e]) t += kA[H[e++]];
						return t
					}
					var SA = {}, YA = {}, JA = {}, MA = 48,
						LA = 57;

					function HA(A) {
						if (void 0 === A) return "_unknown";
						A = A.replace(/[^a-zA-Z0-9_]/g, "$");
						var t = A.charCodeAt(0);
						return t >= MA && t <= LA ? "_" + A : A
					}

					function KA(A, t) {
						return A = HA(A),
						function() {
							return t.apply(this, arguments)
						}
					}

					function xA(A, t) {
						var e = KA(t, (function(A) {
							this.name = t, this.message = A;
							var e = new Error(A).stack;
							void 0 !== e && (this.stack = this.toString() + "\n" + e.replace(/^Error(:[^\n]*)?\n/, ""))
						}));
						return e.prototype = Object.create(A.prototype), e.prototype.constructor = e, e.prototype.toString = function() {
							return void 0 === this.message ? this.name : this.name + ": " + this.message
						}, e
					}
					var jA = void 0;

					function OA(A) {
						throw new jA(A)
					}
					var TA = void 0;

					function WA(A) {
						throw new TA(A)
					}

					function qA(A, t, e) {
						function g(t) {
							var g = e(t);
							g.length !== A.length && WA("Mismatched type converter count");
							for (var i = 0; i < A.length; ++i) ZA(A[i], g[i])
						}
						A.forEach((function(A) {
							JA[A] = t
						}));
						var i = new Array(t.length),
							n = [],
							r = 0;
						t.forEach((function(A, t) {
							YA.hasOwnProperty(A) ? i[t] = YA[A] : (n.push(A), SA.hasOwnProperty(A) || (SA[A] = []), SA[A].push((function() {
								i[t] = YA[A], ++r, r === n.length && g(i)
							})))
						})), 0 === n.length && g(i)
					}

					function ZA(A, t, e) {
						if (e = e || {}, !("argPackAdvance" in t)) throw new TypeError("registerType registeredInstance requires argPackAdvance");
						var g = t.name;
						if (A || OA('type "' + g + '" must have a positive integer typeid pointer'), YA.hasOwnProperty(A)) {
							if (e.ignoreDuplicateRegistrations) return;
							OA("Cannot register type '" + g + "' twice")
						}
						if (YA[A] = t, delete JA[A], SA.hasOwnProperty(A)) {
							var i = SA[A];
							delete SA[A], i.forEach((function(A) {
								A()
							}))
						}
					}

					function XA(A, t, e, g, i) {
						var n = RA(e);
						t = UA(t), ZA(A, {
							name: t,
							fromWireType: function(A) {
								return !!A
							},
							toWireType: function(A, t) {
								return t ? g : i
							},
							argPackAdvance: 8,
							readValueFromPointer: function(A) {
								var g;
								if (1 === e) g = L;
								else if (2 === e) g = K;
								else {
									if (4 !== e) throw new TypeError("Unknown boolean type size: " + t);
									g = j
								}
								return this["fromWireType"](g[A >> n])
							},
							destructorFunction: null
						})
					}
					var PA = [],
						VA = [{}, {
							value: void 0
						}, {
							value: null
						}, {
							value: !0
						}, {
							value: !1
						}];

					function _A(A) {
						A > 4 && 0 === --VA[A].refcount && (VA[A] = void 0, PA.push(A))
					}

					function zA() {
						for (var A = 0, t = 5; t < VA.length; ++t) void 0 !== VA[t] && ++A;
						return A
					}

					function $A() {
						for (var A = 5; A < VA.length; ++A)
							if (void 0 !== VA[A]) return VA[A];
						return null
					}

					function At() {
						o["count_emval_handles"] = zA, o["get_first_emval"] = $A
					}
					var tt = {
						toValue: function(A) {
							return A || OA("Cannot use deleted val. handle = " + A), VA[A].value
						},
						toHandle: function(A) {
							switch (A) {
								case void 0:
									return 1;
								case null:
									return 2;
								case !0:
									return 3;
								case !1:
									return 4;
								default:
									var t = PA.length ? PA.pop() : VA.length;
									return VA[t] = {
										refcount: 1,
										value: A
									}, t
							}
						}
					};

					function et(A) {
						return this["fromWireType"](O[A >> 2])
					}

					function gt(A, t) {
						t = UA(t), ZA(A, {
							name: t,
							fromWireType: function(A) {
								var t = tt.toValue(A);
								return _A(A), t
							},
							toWireType: function(A, t) {
								return tt.toHandle(t)
							},
							argPackAdvance: 8,
							readValueFromPointer: et,
							destructorFunction: null
						})
					}

					function it(A) {
						if (null === A) return "null";
						var t = a(A);
						return "object" === t || "array" === t || "function" === t ? A.toString() : "" + A
					}

					function nt(A, t) {
						switch (t) {
							case 2:
								return function(A) {
									return this["fromWireType"](T[A >> 2])
								};
							case 3:
								return function(A) {
									return this["fromWireType"](W[A >> 3])
								};
							default:
								throw new TypeError("Unknown float type: " + A)
						}
					}

					function rt(A, t, e) {
						var g = RA(e);
						t = UA(t), ZA(A, {
							name: t,
							fromWireType: function(A) {
								return A
							},
							toWireType: function(A, t) {
								return t
							},
							argPackAdvance: 8,
							readValueFromPointer: nt(t, g),
							destructorFunction: null
						})
					}

					function It(A) {
						while (A.length) {
							var t = A.pop(),
								e = A.pop();
							e(t)
						}
					}

					function ot(A, t, e, g, i) {
						var n = t.length;
						n < 2 && OA("argTypes array size mismatch! Must at least get return value and 'this' types!");
						for (var r = null !== t[1] && null !== e, I = !1, o = 1; o < t.length; ++o)
							if (null !== t[o] && void 0 === t[o].destructorFunction) {
								I = !0;
								break
							}
						var a = "void" !== t[0].name,
							B = n - 2,
							C = new Array(B),
							Q = [],
							E = [];
						return function() {
							var e;
							arguments.length !== B && OA("function " + A + " called with " + arguments.length + " arguments, expected " + B + " args!"), E.length = 0, Q.length = r ? 2 : 1, Q[0] = i, r && (e = t[1].toWireType(E, this), Q[1] = e);
							for (var n = 0; n < B; ++n) C[n] = t[n + 2].toWireType(E, arguments[n]), Q.push(C[n]);
							var o = g.apply(null, Q);

							function c(A) {
								if (I) It(E);
								else
									for (var g = r ? 1 : 2; g < t.length; g++) {
										var i = 1 === g ? e : C[g - 2];
										null !== t[g].destructorFunction && t[g].destructorFunction(i)
									}
								if (a) return t[0].fromWireType(A)
							}
							return c(o)
						}
					}

					function at(A, t, e) {
						if (void 0 === A[t].overloadTable) {
							var g = A[t];
							A[t] = function() {
								return A[t].overloadTable.hasOwnProperty(arguments.length) || OA("Function '" + e + "' called with an invalid number of arguments (" + arguments.length + ") - expects one of (" + A[t].overloadTable + ")!"), A[t].overloadTable[arguments.length].apply(this, arguments)
							}, A[t].overloadTable = [], A[t].overloadTable[g.argCount] = g
						}
					}

					function Bt(A, t, e) {
						o.hasOwnProperty(A) ? ((void 0 === e || void 0 !== o[A].overloadTable && void 0 !== o[A].overloadTable[e]) && OA("Cannot register public name '" + A + "' twice"), at(o, A, A), o.hasOwnProperty(e) && OA("Cannot register multiple overloads of a function with the same number of arguments (" + e + ")!"), o[A].overloadTable[e] = t) : (o[A] = t, void 0 !== e && (o[A].numArguments = e))
					}

					function Ct(A, t) {
						for (var e = [], g = 0; g < A; g++) e.push(j[(t >> 2) + g]);
						return e
					}

					function Qt(A, t, e) {
						o.hasOwnProperty(A) || WA("Replacing nonexistant public symbol"), void 0 !== o[A].overloadTable && void 0 !== e ? o[A].overloadTable[e] = t : (o[A] = t, o[A].argCount = e)
					}

					function Et(A, t, e) {
						var g = o["dynCall_" + A];
						return e && e.length ? g.apply(null, [t].concat(e)) : g.call(null, t)
					}

					function ct(A, t, e) {
						return A.includes("j") ? Et(A, t, e) : FA(t).apply(null, e)
					}

					function st(A, t) {
						var e = [];
						return function() {
							e.length = arguments.length;
							for (var g = 0; g < arguments.length; g++) e[g] = arguments[g];
							return ct(A, t, e)
						}
					}

					function ut(A, t) {
						function e() {
							return A.includes("j") ? st(A, t) : FA(t)
						}
						A = UA(A);
						var g = e();
						return "function" !== typeof g && OA("unknown function pointer with signature " + A + ": " + t), g
					}
					var lt = void 0;

					function ft(A) {
						var t = Lt(A),
							e = UA(t);
						return Kt(t), e
					}

					function dt(A, t) {
						var e = [],
							g = {};

						function i(A) {
							g[A] || YA[A] || (JA[A] ? JA[A].forEach(i) : (e.push(A), g[A] = !0))
						}
						throw t.forEach(i), new lt(A + ": " + e.map(ft).join([", "]))
					}

					function ht(A, t, e, g, i, n) {
						var r = Ct(t, e);
						A = UA(A), i = ut(g, i), Bt(A, (function() {
							dt("Cannot call " + A + " due to unbound types", r)
						}), t - 1), qA([], r, (function(e) {
							var g = [e[0], null].concat(e.slice(1));
							return Qt(A, ot(A, g, null, i, n), t - 1), []
						}))
					}

					function wt(A, t, e) {
						switch (t) {
							case 0:
								return e ? function(A) {
									return L[A]
								} : function(A) {
									return H[A]
								};
							case 1:
								return e ? function(A) {
									return K[A >> 1]
								} : function(A) {
									return x[A >> 1]
								};
							case 2:
								return e ? function(A) {
									return j[A >> 2]
								} : function(A) {
									return O[A >> 2]
								};
							default:
								throw new TypeError("Unknown integer type: " + A)
						}
					}

					function yt(A, t, e, g, i) {
						t = UA(t), -1 === i && (i = 4294967295);
						var n = RA(e),
							r = function(A) {
								return A
							};
						if (0 === g) {
							var I = 32 - 8 * e;
							r = function(A) {
								return A << I >>> I
							}
						}
						var o = t.includes("unsigned");
						ZA(A, {
							name: t,
							fromWireType: r,
							toWireType: function(A, e) {
								if ("number" !== typeof e && "boolean" !== typeof e) throw new TypeError('Cannot convert "' + it(e) + '" to ' + this.name);
								if (e < g || e > i) throw new TypeError('Passing a number "' + it(e) + '" from JS side to C/C++ side to an argument of type "' + t + '", which is outside the valid range [' + g + ", " + i + "]!");
								return o ? e >>> 0 : 0 | e
							},
							argPackAdvance: 8,
							readValueFromPointer: wt(t, n, 0 !== g),
							destructorFunction: null
						})
					}

					function Dt(A, t, e) {
						var g = [Int8Array, Uint8Array, Int16Array, Uint16Array, Int32Array, Uint32Array, Float32Array, Float64Array],
							i = g[t];

						function n(A) {
							A >>= 2;
							var t = O,
								e = t[A],
								g = t[A + 1];
							return new i(M, g, e)
						}
						e = UA(e), ZA(A, {
							name: e,
							fromWireType: n,
							argPackAdvance: 8,
							readValueFromPointer: n
						}, {
							ignoreDuplicateRegistrations: !0
						})
					}

					function mt(A, t) {
						t = UA(t);
						var e = "std::string" === t;
						ZA(A, {
							name: t,
							fromWireType: function(A) {
								var t, g = O[A >> 2];
								if (e)
									for (var i = A + 4, n = 0; n <= g; ++n) {
										var r = A + 4 + n;
										if (n == g || 0 == H[r]) {
											var I = r - i,
												o = U(i, I);
											void 0 === t ? t = o : (t += String.fromCharCode(0), t += o), i = r + 1
										}
									} else {
										var a = new Array(g);
										for (n = 0; n < g; ++n) a[n] = String.fromCharCode(H[A + 4 + n]);
										t = a.join("")
									}
								return Kt(A), t
							},
							toWireType: function(A, t) {
								var g;
								t instanceof ArrayBuffer && (t = new Uint8Array(t));
								var i = "string" === typeof t;
								i || t instanceof Uint8Array || t instanceof Uint8ClampedArray || t instanceof Int8Array || OA("Cannot pass non-string to std::string"), g = e && i ? function() {
									return J(t)
								} : function() {
									return t.length
								};
								var n = g(),
									r = Ht(4 + n + 1);
								if (O[r >> 2] = n, e && i) Y(t, r + 4, n + 1);
								else if (i)
									for (var I = 0; I < n; ++I) {
										var o = t.charCodeAt(I);
										o > 255 && (Kt(r), OA("String has UTF-16 code units that do not fit in 8 bits")), H[r + 4 + I] = o
									} else
										for (I = 0; I < n; ++I) H[r + 4 + I] = t[I];
								return null !== A && A.push(Kt, r), r
							},
							argPackAdvance: 8,
							readValueFromPointer: et,
							destructorFunction: function(A) {
								Kt(A)
							}
						})
					}

					function bt(A, t, e) {
						var g, i, n, r, I;
						e = UA(e), 2 === t ? (g = Z, i = X, r = P, n = function() {
							return x
						}, I = 1) : 4 === t && (g = V, i = _, r = z, n = function() {
							return O
						}, I = 2), ZA(A, {
							name: e,
							fromWireType: function(A) {
								for (var e, i = O[A >> 2], r = n(), o = A + 4, a = 0; a <= i; ++a) {
									var B = A + 4 + a * t;
									if (a == i || 0 == r[B >> I]) {
										var C = B - o,
											Q = g(o, C);
										void 0 === e ? e = Q : (e += String.fromCharCode(0), e += Q), o = B + t
									}
								}
								return Kt(A), e
							},
							toWireType: function(A, g) {
								"string" !== typeof g && OA("Cannot pass non-string to C++ string type " + e);
								var n = r(g),
									o = Ht(4 + n + t);
								return O[o >> 2] = n >> I, i(g, o + 4, n + t), null !== A && A.push(Kt, o), o
							},
							argPackAdvance: 8,
							readValueFromPointer: et,
							destructorFunction: function(A) {
								Kt(A)
							}
						})
					}

					function pt(A, t) {
						t = UA(t), ZA(A, {
							isVoid: !0,
							name: t,
							argPackAdvance: 0,
							fromWireType: function() {},
							toWireType: function(A, t) {}
						})
					}

					function Nt() {
						fA("")
					}

					function Ft(A, t, e) {
						H.copyWithin(A, t, t + e)
					}

					function vt(A) {
						fA("OOM")
					}

					function Rt(A) {
						H.length;
						A >>>= 0, vt(A)
					}
					GA(), jA = o["BindingError"] = xA(Error, "BindingError"), TA = o["InternalError"] = xA(Error, "InternalError"), At(), lt = o["UnboundTypeError"] = xA(Error, "UnboundTypeError");
					var Gt = !1;

					function kt(A) {
						for (var t = [], e = 0; e < A.length; e++) {
							var g = A[e];
							g > 255 && (Gt && R(!1, "Character code " + g + " (" + String.fromCharCode(g) + ")  at offset " + e + " not in 0x00-0xFF."), g &= 255), t.push(String.fromCharCode(g))
						}
						return t.join("")
					}
					var Ut = "function" === typeof atob ? atob : function(A) {
							var t, e, g, i, n, r, I, o = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",
								a = "",
								B = 0;
							A = A.replace(/[^A-Za-z0-9\+\/\=]/g, "");
							do {
								i = o.indexOf(A.charAt(B++)), n = o.indexOf(A.charAt(B++)), r = o.indexOf(A.charAt(B++)), I = o.indexOf(A.charAt(B++)), t = i << 2 | n >> 4, e = (15 & n) << 4 | r >> 2, g = (3 & r) << 6 | I, a += String.fromCharCode(t), 64 !== r && (a += String.fromCharCode(e)), 64 !== I && (a += String.fromCharCode(g))
							} while (B < A.length);
							return a
						};

					function St(A) {
						if ("boolean" === typeof h && h) {
							var t = n.from(A, "base64");
							return new Uint8Array(t["buffer"], t["byteOffset"], t["byteLength"])
						}
						try {
							for (var e = Ut(A), g = new Uint8Array(e.length), i = 0; i < e.length; ++i) g[i] = e.charCodeAt(i);
							return g
						} catch (r) {
							throw new Error("Converting base64 string to bytes failed.")
						}
					}

					function Yt(A) {
						if (wA(A)) return St(A.slice(hA.length))
					}
					var Jt, Mt = {
							j: vA,
							i: XA,
							h: gt,
							e: rt,
							g: ht,
							b: yt,
							a: Dt,
							f: mt,
							c: bt,
							m: pt,
							d: Nt,
							k: Ft,
							l: Rt
						}, Lt = (bA(), o["___wasm_call_ctors"] = function() {
							return (o["___wasm_call_ctors"] = o["asm"]["o"]).apply(null, arguments)
						}, o["___getTypeName"] = function() {
							return (Lt = o["___getTypeName"] = o["asm"]["q"]).apply(null, arguments)
						}),
						Ht = (o["___embind_register_native_and_builtin_types"] = function() {
							return (o["___embind_register_native_and_builtin_types"] = o["asm"]["r"]).apply(null, arguments)
						}, o["_malloc"] = function() {
							return (Ht = o["_malloc"] = o["asm"]["s"]).apply(null, arguments)
						}),
						Kt = o["_free"] = function() {
							return (Kt = o["_free"] = o["asm"]["t"]).apply(null, arguments)
						};

					function xt(A) {
						this.name = "ExitStatus", this.message = "Program terminated with exit(" + A + ")", this.status = A
					}

					function jt(A) {
						function t() {
							Jt || (Jt = !0, o["calledRun"] = !0, v || (oA(), r(o), o["onRuntimeInitialized"] && o["onRuntimeInitialized"](), aA()))
						}
						A = A || l, EA > 0 || (IA(), EA > 0 || (o["setStatus"] ? (o["setStatus"]("Running..."), setTimeout((function() {
							setTimeout((function() {
								o["setStatus"]("")
							}), 1), t()
						}), 1)) : t()))
					}
					if (o["getValue"] = F, o["UTF8ToString"] = U, o["writeArrayToMemory"] = $, sA = function A() {
						Jt || jt(), Jt || (sA = A)
					}, o["run"] = jt, o["preInit"]) {
						"function" == typeof o["preInit"] && (o["preInit"] = [o["preInit"]]);
						while (o["preInit"].length > 0) o["preInit"].pop()()
					}
					return jt(), A.ready
				}
			}();
			"object" === a(t) && "object" === a(r) ? r.exports = B : (I = [], o = function() {
				return B
			}.apply(t, I), void 0 === o || (r.exports = o))
		}).call(this, "/index.js", e("4362"), "/", e("1c35").Buffer, e("62e4")(A))
	},
	"5c0b": function(A, t, e) {
		"use strict";
		e("9c0c")
	},
	6: function(A, t) {},
	"675e": function(A, t, e) {
		"use strict";
		e.d(t, "a", (function() {
			return g
		}));
		var g = "um.conf.",
			i = "".concat(g, "joox.uuid"),
			n = function() {
				function A() {}
				return A.prototype.saveJooxUUID = function(A) {
					return this.save(i, A)
				}, A.prototype.loadJooxUUID = function(A) {
					return void 0 === A && (A = ""), this.load(i, A)
				}, A
			}();
		t["b"] = n
	},
	"6d95": function(A, t, e) {
		"use strict";
		(function(A) {
			e.d(t, "a", (function() {
				return b
			}));
			e("d3b7"), e("ace4"), e("5cc6"), e("907a"), e("9a8c"), e("a975"), e("735e"), e("c1ac"), e("d139"), e("3a7b"), e("d5d6"), e("82f8"), e("e91f"), e("60bd"), e("5f96"), e("3280"), e("3fcc"), e("ca91"), e("25a1"), e("cd26"), e("3c5d"), e("2954"), e("649e"), e("219c"), e("170b"), e("b39a"), e("72f7"), e("d81d"), e("fb6a"), e("ddb0"), e("25f0"), e("ac1f"), e("5319"), e("159b"), e("4de4"), e("1276"), e("498a"), e("a15b"), e("3ca3"), e("2b3d"), e("9861");
			var g = e("9ab4"),
				i = e("cc74"),
				n = e("cb96"),
				r = e("ef21"),
				I = e.n(r),
				o = e("c198"),
				a = e.n(o),
				B = e("ead9"),
				C = e.n(B),
				Q = e("81bf"),
				E = e.n(Q),
				c = e("17e1"),
				s = e.n(c),
				u = e("1132"),
				l = e.n(u),
				f = e("f8d5"),
				d = e.n(f),
				h = e("640f"),
				w = e.n(h),
				y = w.a.parse("687a4852416d736f356b496e62617857"),
				D = w.a.parse("2331346C6A6B5F215C5D2630553C2728"),
				m = [67, 84, 69, 78, 70, 68, 65, 77];

			function b(A, t, e) {
				return Object(g["a"])(this, void 0, Promise, (function() {
					var e;
					return Object(g["c"])(this, (function(g) {
						switch (g.label) {
							case 0:
								return e = p.bind, [4, Object(i["c"])(A)];
							case 1:
								return [2, (new(e.apply(p, [void 0, g.sent(), t]))).decrypt()]
						}
					}))
				}))
			}
			var p = function() {
				function t(A, t) {
					this.offset = 0, this.format = "", this.mime = "";
					var e = new Uint8Array(A, 0, 8);
					if (!Object(i["b"])(e, m)) throw Error("此ncm文件已损坏");
					this.offset = 10, this.raw = A, this.view = new DataView(A), this.filename = t
				}
				return t.prototype._getKeyData = function() {
					var A = this.view.getUint32(this.offset, !0);
					this.offset += 4;
					var t = new Uint8Array(this.raw, this.offset, A).map((function(A) {
						return 100 ^ A
					}));
					this.offset += A;
					for (var e = a.a.decrypt({
						ciphertext: s.a.create(t)
					}, y, {
						mode: E.a,
						padding: C.a
					}), g = new Uint8Array(e.sigBytes), i = e.words, n = e.sigBytes, r = 0; r < n; r++) g[r] = i[r >>> 2] >>> 24 - r % 4 * 8 & 255;
					return g.slice(17)
				}, t.prototype._getKeyBox = function() {
					for (var A, t = this._getKeyData(), e = new Uint8Array(Array(256).keys()), g = t.length, i = 0, n = 0; n < 256; n++) i = e[n] + i + t[n % g] & 255, A = [e[i], e[n]], e[n] = A[0], e[i] = A[1];
					return e.map((function(A, t, e) {
						t = t + 1 & 255;
						var g = e[t],
							i = e[t + g & 255];
						return e[g + i & 255]
					}))
				}, t.prototype._getMetaData = function() {
					var A = this.view.getUint32(this.offset, !0);
					if (this.offset += 4, 0 === A) return {};
					var t = new Uint8Array(this.raw, this.offset, A).map((function(A) {
						return 99 ^ A
					}));
					this.offset += A, s.a.create();
					var e, g = a.a.decrypt({
							ciphertext: l.a.parse(s.a.create(t.slice(22)).toString(d.a))
						}, D, {
							mode: E.a,
							padding: C.a
						}).toString(d.a),
						i = g.indexOf(":");
					if ("dj" === g.slice(0, i)) {
						var n = JSON.parse(g.slice(i + 1));
						e = n.mainMusic
					} else e = JSON.parse(g.slice(i + 1));
					return e.albumPic && (e.albumPic = e.albumPic.replace("http://", "https://") + "?param=500y500"), e
				}, t.prototype._getAudio = function(A) {
					this.offset += this.view.getUint32(this.offset + 5, !0) + 13;
					for (var t = new Uint8Array(this.raw, this.offset), e = t.length, g = 0; g < e; ++g) t[g] ^= A[255 & g];
					return t
				}, t.prototype._buildMeta = function() {
					var t;
					return Object(g["a"])(this, void 0, void 0, (function() {
						var e, n, r, o, a, B;
						return Object(g["c"])(this, (function(g) {
							switch (g.label) {
								case 0:
									if (!this.oriMeta) throw Error("invalid sequence");
									if (e = Object(i["f"])(this.filename, this.oriMeta.musicName), n = [], this.oriMeta.artist && this.oriMeta.artist.forEach((function(A) {
										return n.push(A[0])
									})), 0 === n.length && e.artist && (n = e.artist.split(",").map((function(A) {
										return A.trim()
									})).filter((function(A) {
										return "" != A
									}))), !this.oriMeta.albumPic) return [3, 9];
									g.label = 1;
								case 1:
									return g.trys.push([1, 8, , 9]), r = this, [4, Object(i["e"])(this.oriMeta.albumPic)];
								case 2:
									r.image = g.sent(), g.label = 3;
								case 3:
									return this.image && this.image.buffer.byteLength >= 1 << 24 ? [4, I.a.read(A.from(this.image.buffer))] : [3, 7];
								case 4:
									return o = g.sent(), [4, o.resize(Math.round(o.getHeight() / 2), I.a.AUTO)];
								case 5:
									return g.sent(), a = this.image, [4, o.getBufferAsync("image/jpeg")];
								case 6:
									return a.buffer = g.sent(), [3, 3];
								case 7:
									return [3, 9];
								case 8:
									return B = g.sent(), console.log("get cover image failed", B), [3, 9];
								case 9:
									return this.newMeta = {
										title: e.title,
										artists: n,
										album: this.oriMeta.album,
										picture: null === (t = this.image) || void 0 === t ? void 0 : t.buffer
									}, [2]
							}
						}))
					}))
				}, t.prototype._writeMeta = function() {
					return Object(g["a"])(this, void 0, void 0, (function() {
						var t, e;
						return Object(g["c"])(this, (function(g) {
							switch (g.label) {
								case 0:
									if (!this.audio || !this.newMeta) throw Error("invalid sequence");
									return this.blob || (this.blob = new Blob([this.audio], {
										type: this.mime
									})), [4, Object(n["parseBlob"])(this.blob)];
								case 1:
									if (t = g.sent(), e = !t.common.album && !t.common.artists && !t.common.title, e || this.newMeta.picture) {
										if ("mp3" === this.format) this.audio = Object(i["l"])(A.from(this.audio), this.newMeta, t);
										else {
											if ("flac" !== this.format) return console.info("writing meta for ".concat(this.format, " is not being supported for now")), [2];
											this.audio = Object(i["k"])(A.from(this.audio), this.newMeta, t)
										}
										this.blob = new Blob([this.audio], {
											type: this.mime
										})
									}
									return [2]
							}
						}))
					}))
				}, t.prototype.gatherResult = function() {
					var A, t;
					if (!this.newMeta || !this.blob) throw Error("bad sequence");
					return {
						title: this.newMeta.title,
						artist: null === (A = this.newMeta.artists) || void 0 === A ? void 0 : A.join("; "),
						ext: this.format,
						album: this.newMeta.album,
						picture: null === (t = this.image) || void 0 === t ? void 0 : t.url,
						file: URL.createObjectURL(this.blob),
						blob: this.blob,
						mime: this.mime
					}
				}, t.prototype.decrypt = function() {
					return Object(g["a"])(this, void 0, void 0, (function() {
						var A, t;
						return Object(g["c"])(this, (function(e) {
							switch (e.label) {
								case 0:
									return A = this._getKeyBox(), this.oriMeta = this._getMetaData(), this.audio = this._getAudio(A), this.format = this.oriMeta.format || Object(i["i"])(this.audio), this.mime = i["a"][this.format], [4, this._buildMeta()];
								case 1:
									e.sent(), e.label = 2;
								case 2:
									return e.trys.push([2, 4, , 5]), [4, this._writeMeta()];
								case 3:
									return e.sent(), [3, 5];
								case 4:
									return t = e.sent(), console.warn("write meta data failed", t), [3, 5];
								case 5:
									return [2, this.gatherResult()]
							}
						}))
					}))
				}, t
			}()
		}).call(this, e("1c35").Buffer)
	},
	"730d": function(A, t, e) {
		"use strict";
		var g = function() {
			var A = this,
				t = A.$createElement,
				e = A._self._c || t;
			return e("el-dialog", {
				attrs: {
					title: "解密设定",
					visible: A.show,
					"custom-class": "um-config-dialog",
					center: ""
				},
				on: {
					close: function(t) {
						return A.cancel()
					}
				}
			}, [e("el-form", {
				ref: "form",
				attrs: {
					rules: A.rules,
					"status-icon": "",
					model: A.form,
					"label-width": "0"
				}
			}, [e("section", [e("label", [e("span", [A._v(" JOOX Music · "), e("Ruby", {
				attrs: {
					caption: "Unique Device Identifier"
				}
			}, [A._v("设备唯一识别码")])], 1), e("el-form-item", {
				attrs: {
					prop: "jooxUUID"
				}
			}, [e("el-input", {
				attrs: {
					type: "text",
					clearable: "",
					maxlength: "32",
					"show-word-limit": ""
				},
				model: {
					value: A.form.jooxUUID,
					callback: function(t) {
						A.$set(A.form, "jooxUUID", t)
					},
					expression: "form.jooxUUID"
				}
			})], 1)], 1), e("p", {
				staticClass: "item-desc"
			}, [A._v(" 下载该加密文件的 JOOX 应用所记录的设备唯一识别码。 "), e("br"), A._v(" 参见： "), e("a", {
				attrs: {
					href: "https://github.com/unlock-music/joox-crypto/wiki/%E8%8E%B7%E5%8F%96%E8%AE%BE%E5%A4%87-UUID"
				}
			}, [A._v(" 获取设备 UUID · unlock-music/joox-crypto Wiki")]), A._v("。 ")])])]), e("span", {
				staticClass: "dialog-footer",
				attrs: {
					slot: "footer"
				},
				slot: "footer"
			}, [e("el-button", {
				attrs: {
					type: "primary",
					loading: A.saving
				},
				on: {
					click: function(t) {
						return A.emitConfirm()
					}
				}
			}, [A._v("确 定")])], 1)], 1)
		}, i = [],
			n = e("1da1"),
			r = (e("96cf"), e("ac1f"), e("00b4"), e("b7c7")),
			I = e("77b3");

		function o(A, t, e) {
			t && /^[\da-fA-F]{32}$/.test(t) ? e() : e(new Error("无效的 Joox UUID，请参考 Wiki 获取。"))
		}
		var a = {
			jooxUUID: {
				validator: o,
				trigger: "change"
			}
		}, B = {
				components: {
					Ruby: I["a"]
				},
				props: {
					show: {
						type: Boolean,
						required: !0
					}
				},
				data: function() {
					return {
						rules: a,
						saving: !1,
						form: {
							jooxUUID: ""
						},
						centerDialogVisible: !1
					}
				},
				mounted: function() {
					var A = this;
					return Object(n["a"])(regeneratorRuntime.mark((function t() {
						return regeneratorRuntime.wrap((function(t) {
							while (1) switch (t.prev = t.next) {
								case 0:
									return t.next = 2, A.resetForm();
								case 2:
								case "end":
									return t.stop()
							}
						}), t)
					})))()
				},
				methods: {
					resetForm: function() {
						var A = this;
						return Object(n["a"])(regeneratorRuntime.mark((function t() {
							return regeneratorRuntime.wrap((function(t) {
								while (1) switch (t.prev = t.next) {
									case 0:
										return t.next = 2, r["a"].loadJooxUUID();
									case 2:
										A.form.jooxUUID = t.sent;
									case 3:
									case "end":
										return t.stop()
								}
							}), t)
						})))()
					},
					cancel: function() {
						var A = this;
						return Object(n["a"])(regeneratorRuntime.mark((function t() {
							return regeneratorRuntime.wrap((function(t) {
								while (1) switch (t.prev = t.next) {
									case 0:
										return t.next = 2, A.resetForm();
									case 2:
										A.$emit("done");
									case 3:
									case "end":
										return t.stop()
								}
							}), t)
						})))()
					},
					emitConfirm: function() {
						var A = this;
						return Object(n["a"])(regeneratorRuntime.mark((function t() {
							return regeneratorRuntime.wrap((function(t) {
								while (1) switch (t.prev = t.next) {
									case 0:
										return A.saving = !0, t.next = 3, r["a"].saveJooxUUID(A.form.jooxUUID);
									case 3:
										A.saving = !1, A.$emit("done");
									case 5:
									case "end":
										return t.stop()
								}
							}), t)
						})))()
					}
				}
			}, C = B,
			Q = (e("b0a8"), e("2877")),
			E = Object(Q["a"])(C, g, i, !1, null, "58e4a4a6", null);
		t["a"] = E.exports
	},
	"73ec": function(A, t, e) {
		"use strict";
		e.d(t, "e", (function() {
			return g
		})), e.d(t, "d", (function() {
			return n
		})), e.d(t, "b", (function() {
			return I
		})), e.d(t, "c", (function() {
			return o
		})), e.d(t, "f", (function() {
			return a
		})), e.d(t, "a", (function() {
			return B
		}));
		e("99af"), e("d3b7"), e("3ca3"), e("ddb0"), e("2b3d"), e("9861"), e("2ca0");
		var g, i = e("9ab4");
		(function(A) {
			A[A["ArtistAndTitle"] = 0] = "ArtistAndTitle", A[A["TitleOnly"] = 1] = "TitleOnly", A[A["TitleAndArtist"] = 2] = "TitleAndArtist", A[A["SameAsOriginal"] = 3] = "SameAsOriginal"
		})(g || (g = {}));
		var n = [{
			key: g.ArtistAndTitle,
			text: "歌手-歌曲名"
		}, {
			key: g.TitleOnly,
			text: "歌曲名"
		}, {
			key: g.TitleAndArtist,
			text: "歌曲名-歌手"
		}, {
			key: g.SameAsOriginal,
			text: "同源文件名"
		}];

		function r(A, t) {
			switch (t) {
				case g.TitleOnly:
					return "".concat(A.title, ".").concat(A.ext);
				case g.TitleAndArtist:
					return "".concat(A.title, " - ").concat(A.artist, ".").concat(A.ext);
				case g.SameAsOriginal:
					return "".concat(A.rawFilename, ".").concat(A.ext);
				default:
				case g.ArtistAndTitle:
					return "".concat(A.artist, " - ").concat(A.title, ".").concat(A.ext)
			}
		}

		function I(A, t, e) {
			return Object(i["a"])(this, void 0, void 0, (function() {
				var g, n, I;
				return Object(i["c"])(this, (function(i) {
					switch (i.label) {
						case 0:
							g = r(A, t), i.label = 1;
						case 1:
							return i.trys.push([1, 3, , 4]), [4, e.getFileHandle(g)];
						case 2:
							return i.sent(), g = "".concat((new Date).getTime(), " - ").concat(g), [3, 4];
						case 3:
							return i.sent(), [3, 4];
						case 4:
							return [4, e.getFileHandle(g, {
								create: !0
							})];
						case 5:
							return n = i.sent(), [4, n.createWritable()];
						case 6:
							return I = i.sent(), [4, I.write(A.blob)];
						case 7:
							return i.sent(), [4, I.close()];
						case 8:
							return i.sent(), [2]
					}
				}))
			}))
		}

		function o(A, t) {
			var e = document.createElement("a");
			e.href = A.file, e.download = r(A, t), document.body.append(e), e.click(), e.remove()
		}

		function a(A) {
			var t;
			URL.revokeObjectURL(A.file), (null === (t = A.picture) || void 0 === t ? void 0 : t.startsWith("blob:")) && URL.revokeObjectURL(A.picture)
		}
		var B = function() {
			function A() {
				this.pending = []
			}
			return A.prototype.queue = function(A) {
				this.pending.push(A), this.consume()
			}, A.prototype.consume = function() {
				var A = this,
					t = this.pending.shift();
				t && t().then((function() {
					return A.consume
				})).
				catch (console.error)
			}, A
		}()
	},
	"77b3": function(A, t, e) {
		"use strict";
		var g = function() {
			var A = this,
				t = A.$createElement,
				e = A._self._c || t;
			return e("ruby", {
				attrs: {
					title: A.caption
				}
			}, [A._t("default"), e("rp", [A._v("（")]), e("rt", {
				domProps: {
					textContent: A._s(A.caption)
				}
			}), e("rp", [A._v("）")])], 2)
		}, i = [],
			n = {
				name: "Ruby",
				props: {
					caption: {
						type: String,
						required: !0
					}
				}
			}, r = n,
			I = e("2877"),
			o = Object(I["a"])(r, g, i, !1, null, null, null);
		t["a"] = o.exports
	},
	"8c80": function(A, t, e) {},
	9224: function(A) {
		A.exports = JSON.parse('{"name":"unlock-music","version":"1.10.3","ext_build":0,"updateInfo":"完善音乐标签编辑功能，支持编辑更多标签","license":"MIT","description":"Unlock encrypted music file in browser.","repository":{"type":"git","url":"https://github.com/ix64/unlock-music"},"private":true,"scripts":{"postinstall":"patch-package","serve":"vue-cli-service serve","build":"vue-cli-service build","test":"jest","pretty":"prettier --write src/{**/*,*}.{js,ts,jsx,tsx,vue}","pretty:check":"prettier --check src/{**/*,*}.{js,ts,jsx,tsx,vue}","make-extension":"node ./make-extension.js"},"dependencies":{"@babel/preset-typescript":"^7.16.5","@jixun/kugou-crypto":"^1.0.3","@unlock-music/joox-crypto":"^0.0.1-R5","base64-js":"^1.5.1","browser-id3-writer":"^4.4.0","core-js":"^3.16.0","crypto-js":"^4.1.1","element-ui":"^2.15.5","iconv-lite":"^0.6.3","jimp":"^0.16.1","metaflac-js":"^1.0.5","music-metadata":"7.9.0","music-metadata-browser":"2.2.7","register-service-worker":"^1.7.2","threads":"^1.6.5","vue":"^2.6.14"},"devDependencies":{"@types/crypto-js":"^4.0.2","@types/jest":"^27.0.3","@vue/cli-plugin-babel":"^4.5.13","@vue/cli-plugin-pwa":"^4.5.13","@vue/cli-plugin-typescript":"^4.5.13","@vue/cli-service":"^4.5.13","babel-plugin-component":"^1.1.1","jest":"^27.4.5","patch-package":"^6.4.7","prettier":"2.5.1","sass":"^1.38.1","sass-loader":"^10.2.0","semver":"^7.3.5","threads-plugin":"^1.4.0","typescript":"^4.5.4","vue-cli-plugin-element":"^1.0.1","vue-template-compiler":"^2.6.14"}}')
	},
	"97e5": function(A, t, e) {
		"use strict";
		e.d(t, "a", (function() {
			return n
		})), e.d(t, "c", (function() {
			return r
		})), e.d(t, "d", (function() {
			return I
		})), e.d(t, "b", (function() {
			return o
		}));
		e("d3b7"), e("e9c4"), e("3ca3"), e("ddb0"), e("9861"), e("99af"), e("25f0");
		var g = e("9ab4"),
			// i = "https://um-api.ixarea.com";
            i = "https://localhost";

		function n(A) {
			return Object(g["a"])(this, void 0, Promise, (function() {
				var t;
				return Object(g["c"])(this, (function(e) {
					switch (e.label) {
						case 0:
							return [4, fetch(i + "/music/app-version", {
								method: "POST",
								headers: {
									"Content-Type": "application/json"
								},
								body: JSON.stringify({
									Version: A
								})
							})];
						case 1:
							return t = e.sent(), [4, t.json()];
						case 2:
							return [2, e.sent()]
					}
				}))
			}))
		}

		function r(A, t, e) {
			return Object(g["a"])(this, void 0, Promise, (function() {
				var n, r, I;
				return Object(g["c"])(this, (function(g) {
					switch (g.label) {
						case 0:
							return n = i + "/music/qq-cover", r = new URLSearchParams([
								["Title", A],
								["Artist", null !== t && void 0 !== t ? t : ""],
								["Album", null !== e && void 0 !== e ? e : ""]
							]), [4, fetch("".concat(n, "?").concat(r.toString()))];
						case 1:
							return I = g.sent(), [4, I.json()];
						case 2:
							return [2, g.sent()]
					}
				}))
			}))
		}

		function I(A) {
			return Object(g["a"])(this, void 0, Promise, (function() {
				var t, e;
				return Object(g["c"])(this, (function(g) {
					switch (g.label) {
						case 0:
							return t = "".concat(i, "/meta/qq-music-raw/").concat(A), [4, fetch(t).then((function(A) {
								return A.json()
							}))];
						case 1:
							if (e = g.sent(), 0 === e.code && 0 === e.req_1.code) return [2, e.req_1.data];
							throw new Error("请求信息失败")
					}
				}))
			}))
		}

		function o(A, t) {
			return void 0 === t && (t = 1), "".concat(i, "/music/qq-cover/").concat(t, "/").concat(A)
		}
	},
	"9b2d": function(A, t, e) {
		"use strict";
		(function(A) {
			e.d(t, "a", (function() {
				return B
			}));
			e("d3b7"), e("ace4"), e("5cc6"), e("907a"), e("9a8c"), e("a975"), e("735e"), e("c1ac"), e("d139"), e("3a7b"), e("d5d6"), e("82f8"), e("e91f"), e("60bd"), e("5f96"), e("3280"), e("3fcc"), e("ca91"), e("25a1"), e("cd26"), e("3c5d"), e("2954"), e("649e"), e("219c"), e("170b"), e("b39a"), e("72f7"), e("fb6a"), e("caad"), e("25f0");
			var g = e("9ab4"),
				i = e("0ba7"),
				n = e("cc74"),
				r = 32;

			function I(A) {
				return A >= 32 && A <= 126
			}

			function o(A) {
				return A >= 48 && A <= 57 || A >= 65 && A <= 70
			}

			function a(t, e) {
				for (var g = 0; g < t.byteLength; g++) t[g] -= e[g % r];
				return A.from(t)
			}

			function B(t, e) {
				return Object(g["a"])(this, void 0, Promise, (function() {
					var B, C, Q, E, c, s, u, l, f, d, h, w, y, D, m, b;
					return Object(g["c"])(this, (function(g) {
						switch (g.label) {
							case 0:
								return C = Uint8Array.bind, [4, Object(n["c"])(t)];
							case 1:
								for (B = new(C.apply(Uint8Array, [void 0, g.sent()])), Q = B.slice(0, 256), E = A.from("RIFF", "ascii"), c = A.from("WAVEfmt ", "ascii"), s = [], u = r; u < 20 * r; u += r) l = B.slice(u, u + r), l.every(o) && (f = a(Q, l), f.slice(0, 4).compare(E) || f.slice(8, 16).compare(c) || (d = f.readUInt32LE(16), [16, 18, 40].includes(d) && (h = 20 + d, w = f.slice(h, h + 4), w.every(I) && (y = h + 8 + f.readUInt32LE(h + 4), y <= Q.byteLength && (D = f.slice(y, y + 4), !D.every(I)) || s.push(A.from(l).toString("ascii"))))));
								if (s.length <= 0) throw new Error("ERROR: no suitable key discovered");
								return m = A.from(s[0], "ascii"), a(B, m), b = new Blob([B], {
									type: "audio/x-wav"
								}), [4, Object(i["a"])(b, e, "wav", !1)];
							case 2:
								return [2, g.sent()]
						}
					}))
				}))
			}
		}).call(this, e("1c35").Buffer)
	},
	"9c0c": function(A, t, e) {},
	ab8f: function(A, t, e) {
		"use strict";
		var g = function() {
			var A = this,
				t = A.$createElement,
				e = A._self._c || t;
			return e("el-upload", {
				attrs: {
					"auto-upload": !1,
					"on-change": A.addFile,
					"show-file-list": !1,
					action: "",
					drag: "",
					multiple: ""
				}
			}, [e("i", {
				staticClass: "el-icon-upload"
			}), e("div", {
				staticClass: "el-upload__text"
			}, [A._v("将文件拖到此处，或 "), e("em", [A._v("点击选择")])]), e("div", {
				staticClass: "el-upload__tip",
				attrs: {
					slot: "tip"
				},
				slot: "tip"
			}, [e("div", [A._v(" 仅在浏览器内对文件进行解锁，无需消耗流量 "), e("el-tooltip", {
				attrs: {
					effect: "dark",
					placement: "top-start"
				}
			}, [e("div", {
				attrs: {
					slot: "content"
				},
				slot: "content"
			}, [A._v("算法在源代码中已经提供，所有运算都发生在本地")]), e("i", {
				staticClass: "el-icon-info",
				staticStyle: {
					"font-size": "12px"
				}
			})])], 1), e("div", [A._v(" 工作模式: " + A._s(A.parallel ? "多线程 Worker" : "单线程 Queue") + " "), e("el-tooltip", {
				attrs: {
					effect: "dark",
					placement: "top-start"
				}
			}, [e("div", {
				attrs: {
					slot: "content"
				},
				slot: "content"
			}, [A._v(" 将此工具部署在HTTPS环境下，可以启用Web Worker特性，"), e("br"), A._v(" 从而更快的利用并行处理完成解锁 ")]), e("i", {
				staticClass: "el-icon-info",
				staticStyle: {
					"font-size": "12px"
				}
			})])], 1)]), e("transition", {
				attrs: {
					name: "el-fade-in"
				}
			}, [e("el-progress", {
				directives: [{
					name: "show",
					rawName: "v-show",
					value: A.progress_show,
					expression: "progress_show"
				}],
				staticStyle: {
					margin: "16px 6px 0 6px"
				},
				attrs: {
					format: A.progress_string,
					percentage: A.progress_value,
					"stroke-width": 16,
					"text-inside": !0
				}
			})], 1)], 1)
		}, i = [],
			n = e("0565"),
			r = n["a"],
			I = e("2877"),
			o = Object(I["a"])(r, g, i, !1, null, null, null);
		t["a"] = o.exports
	},
	af85: function(A, t, e) {
		"use strict";
		var g = function() {
			var A = this,
				t = A.$createElement,
				e = A._self._c || t;
			return e("el-table", {
				staticStyle: {
					width: "100%"
				},
				attrs: {
					data: A.tableData
				}
			}, [e("el-table-column", {
				attrs: {
					label: "封面"
				},
				scopedSlots: A._u([{
					key: "default",
					fn: function(t) {
						return [e("el-image", {
							staticStyle: {
								width: "100px",
								height: "100px"
							},
							attrs: {
								src: t.row.picture
							}
						}, [e("div", {
							staticClass: "image-slot el-image__error",
							attrs: {
								slot: "error"
							},
							slot: "error"
						}, [A._v("暂无封面")])])]
					}
				}])
			}), e("el-table-column", {
				attrs: {
					label: "歌曲"
				},
				scopedSlots: A._u([{
					key: "default",
					fn: function(t) {
						return [e("span", [A._v(A._s(t.row.title))])]
					}
				}])
			}), e("el-table-column", {
				attrs: {
					label: "歌手"
				},
				scopedSlots: A._u([{
					key: "default",
					fn: function(t) {
						return [e("p", [A._v(A._s(t.row.artist))])]
					}
				}])
			}), e("el-table-column", {
				attrs: {
					label: "专辑"
				},
				scopedSlots: A._u([{
					key: "default",
					fn: function(t) {
						return [e("p", [A._v(A._s(t.row.album))])]
					}
				}])
			}), e("el-table-column", {
				attrs: {
					label: "操作"
				},
				scopedSlots: A._u([{
					key: "default",
					fn: function(t) {
						return [e("el-button", {
							attrs: {
								circle: "",
								icon: "el-icon-video-play",
								type: "success"
							},
							on: {
								click: function(e) {
									return A.handlePlay(t.$index, t.row)
								}
							}
						}), e("el-button", {
							attrs: {
								circle: "",
								icon: "el-icon-download"
							},
							on: {
								click: function(e) {
									return A.handleDownload(t.row)
								}
							}
						}), e("el-button", {
							attrs: {
								circle: "",
								icon: "el-icon-edit"
							},
							on: {
								click: function(e) {
									return A.handleEdit(t.row)
								}
							}
						}), e("el-button", {
							attrs: {
								circle: "",
								icon: "el-icon-delete",
								type: "danger"
							},
							on: {
								click: function(e) {
									return A.handleDelete(t.$index, t.row)
								}
							}
						})]
					}
				}])
			})], 1)
		}, i = [],
			n = (e("a9e3"), e("a434"), e("73ec")),
			r = {
				name: "PreviewTable",
				props: {
					tableData: {
						type: Array,
						required: !0
					},
					policy: {
						type: Number,
						required: !0
					}
				},
				methods: {
					handlePlay: function(A, t) {
						this.$emit("play", t.file)
					},
					handleDelete: function(A, t) {
						Object(n["f"])(t), this.tableData.splice(A, 1)
					},
					handleDownload: function(A) {
						this.$emit("download", A)
					},
					handleEdit: function(A) {
						this.$emit("edit", A)
					}
				}
			}, I = r,
			o = e("2877"),
			a = Object(o["a"])(I, g, i, !1, null, "7ee4db14", null);
		t["a"] = a.exports
	},
	b025: function(A, t, e) {},
	b0a8: function(A, t, e) {
		"use strict";
		e("b025")
	},
	b7c7: function(A, t, e) {
		"use strict";
		e.d(t, "a", (function() {
			return C
		}));
		e("d3b7"), e("e9c4"), e("4fad"), e("2ca0");
		var g = e("9ab4"),
			i = e("675e"),
			n = function(A) {
				function t() {
					return null !== A && A.apply(this, arguments) || this
				}
				return Object(g["b"])(t, A), Object.defineProperty(t, "works", {
					get: function() {
						return "undefined" !== typeof localStorage && localStorage.getItem
					},
					enumerable: !1,
					configurable: !0
				}), t.prototype.load = function(A, t) {
					return Object(g["a"])(this, void 0, Promise, (function() {
						var e;
						return Object(g["c"])(this, (function(g) {
							if (e = localStorage.getItem(A), null === e) return [2, t];
							try {
								return [2, JSON.parse(e)]
							} catch (i) {
								return [2, t]
							}
							return [2]
						}))
					}))
				}, t.prototype.save = function(A, t) {
					return Object(g["a"])(this, void 0, Promise, (function() {
						return Object(g["c"])(this, (function(e) {
							return localStorage.setItem(A, JSON.stringify(t)), [2]
						}))
					}))
				}, t.prototype.getAll = function() {
					return Object(g["a"])(this, void 0, Promise, (function() {
						var A, t, e, n, r, I, o;
						return Object(g["c"])(this, (function(g) {
							for (A = {}, t = 0, e = Object.entries(localStorage); t < e.length; t++)
								if (n = e[t], r = n[0], I = n[1], r.startsWith(i["a"])) try {
									Object.assign(A, (o = {}, o[r] = JSON.parse(I), o))
								} catch (a) {}
								return [2, A]
						}))
					}))
				}, t.prototype.setAll = function(A) {
					return Object(g["a"])(this, void 0, Promise, (function() {
						var t, e, i, n, r;
						return Object(g["c"])(this, (function(g) {
							switch (g.label) {
								case 0:
									t = 0, e = Object.entries(A), g.label = 1;
								case 1:
									return t < e.length ? (i = e[t], n = i[0], r = i[1], [4, this.save(n, r)]) : [3, 4];
								case 2:
									g.sent(), g.label = 3;
								case 3:
									return t++, [3, 1];
								case 4:
									return [2]
							}
						}))
					}))
				}, t
			}(i["b"]),
			r = n,
			I = function(A) {
				function t() {
					return null !== A && A.apply(this, arguments) || this
				}
				return Object(g["b"])(t, A), Object.defineProperty(t, "works", {
					get: function() {
						var A, t;
						return "undefined" !== typeof chrome && Boolean(null === (t = null === (A = null === chrome || void 0 === chrome ? void 0 : chrome.storage) || void 0 === A ? void 0 : A.local) || void 0 === t ? void 0 : t.set)
					},
					enumerable: !1,
					configurable: !0
				}), t.prototype.load = function(A, t) {
					return Object(g["a"])(this, void 0, Promise, (function() {
						return Object(g["c"])(this, (function(e) {
							return [2, new Promise((function(e) {
								var g;
								chrome.storage.local.get((g = {}, g[A] = t, g), (function(g) {
									Object.prototype.hasOwnProperty.call(g, A) ? e(g[A]) : e(t)
								}))
							}))]
						}))
					}))
				}, t.prototype.save = function(A, t) {
					return Object(g["a"])(this, void 0, Promise, (function() {
						return Object(g["c"])(this, (function(e) {
							return [2, new Promise((function(e) {
								var g;
								chrome.storage.local.set((g = {}, g[A] = t, g), e)
							}))]
						}))
					}))
				}, t.prototype.getAll = function() {
					return Object(g["a"])(this, void 0, Promise, (function() {
						return Object(g["c"])(this, (function(A) {
							return [2, new Promise((function(A) {
								chrome.storage.local.get(null, (function(t) {
									for (var e = {}, g = 0, n = Object.entries(t); g < n.length; g++) {
										var r = n[g],
											I = r[0],
											o = r[1];
										I.startsWith(i["a"]) && (e[I] = o)
									}
									A(e)
								}))
							}))]
						}))
					}))
				}, t.prototype.setAll = function(A) {
					return Object(g["a"])(this, void 0, Promise, (function() {
						return Object(g["c"])(this, (function(t) {
							return [2, new Promise((function(t) {
								chrome.storage.local.set(A, t)
							}))]
						}))
					}))
				}, t
			}(i["b"]),
			o = I,
			a = e("efc0");

		function B() {
			return o.works ? new o : r.works ? new r : new a["a"]
		}
		var C = B()
	},
	cc74: function(A, t, e) {
		"use strict";
		(function(A) {
			e.d(t, "m", (function() {
				return o
			})), e.d(t, "a", (function() {
				return l
			})), e.d(t, "b", (function() {
				return f
			})), e.d(t, "i", (function() {
				return d
			})), e.d(t, "c", (function() {
				return h
			})), e.d(t, "d", (function() {
				return w
			})), e.d(t, "f", (function() {
				return y
			})), e.d(t, "e", (function() {
				return D
			})), e.d(t, "l", (function() {
				return m
			})), e.d(t, "k", (function() {
				return b
			})), e.d(t, "h", (function() {
				return p
			})), e.d(t, "g", (function() {
				return N
			})), e.d(t, "j", (function() {
				return F
			}));
			e("d3b7"), e("fb6a"), e("3ca3"), e("ddb0"), e("2b3d"), e("9861"), e("ac1f"), e("1276"), e("498a"), e("2ca0"), e("159b");
			var g = e("9ab4"),
				i = e("7907"),
				n = e.n(i),
				r = e("b686"),
				I = e.n(r),
				o = /[ ]?[,;/_、][ ]?/,
				a = [102, 76, 97, 67],
				B = [73, 68, 51],
				C = [79, 103, 103, 83],
				Q = [102, 116, 121, 112],
				E = [48, 38, 178, 117, 142, 102, 207, 17, 166, 217, 0, 170, 0, 98, 206, 108],
				c = [82, 73, 70, 70],
				s = [255, 241],
				u = [70, 82, 77, 56],
				l = {
					mp3: "audio/mpeg",
					flac: "audio/flac",
					m4a: "audio/mp4",
					ogg: "audio/ogg",
					wma: "audio/x-ms-wma",
					wav: "audio/x-wav",
					dff: "audio/x-dff"
				};

			function f(A, t) {
				return !(t.length > A.length) && t.every((function(t, e) {
					return t === A[e]
				}))
			}

			function d(A, t) {
				return void 0 === t && (t = "mp3"), f(A, B) ? "mp3" : f(A, a) ? "flac" : f(A, C) ? "ogg" : A.length >= 4 + Q.length && f(A.slice(4), Q) ? "m4a" : f(A, c) ? "wav" : f(A, E) ? "wma" : f(A, s) ? "aac" : f(A, u) ? "dff" : t
			}

			function h(A) {
				return A.arrayBuffer ? A.arrayBuffer() : new Promise((function(t, e) {
					var g = new FileReader;
					g.onload = function(A) {
						var g, i = null === (g = A.target) || void 0 === g ? void 0 : g.result;
						i ? t(i) : e("read file failed")
					}, g.readAsArrayBuffer(A)
				}))
			}

			function w(A) {
				var t;
				return (null === (t = A.common) || void 0 === t ? void 0 : t.picture) && A.common.picture.length > 0 ? URL.createObjectURL(new Blob([A.common.picture[0].data], {
					type: A.common.picture[0].format
				})) : ""
			}

			function y(A, t, e, g) {
				void 0 === g && (g = "-");
				var i = {
					title: null !== t && void 0 !== t ? t : "",
					artist: e
				}, n = A.split(g);
				return n.length > 1 ? ((!i.artist || i.artist.split(o).length < n[0].trim().split(o).length) && (i.artist = n[0].trim()), i.title || (i.title = n[1].trim())) : 1 === n.length && (i.title || (i.title = n[0].trim())), i
			}

			function D(A) {
				return Object(g["a"])(this, void 0, Promise, (function() {
					var t, e, i, n, r;
					return Object(g["c"])(this, (function(g) {
						switch (g.label) {
							case 0:
								return g.trys.push([0, 4, , 5]), [4, fetch(A)];
							case 1:
								return t = g.sent(), e = t.headers.get("Content-Type"), (null === e || void 0 === e ? void 0 : e.startsWith("image/")) ? [4, t.arrayBuffer()] : [3, 3];
							case 2:
								return i = g.sent(), n = URL.createObjectURL(new Blob([i], {
									type: e
								})), [2, {
									buffer: i,
									url: n,
									mime: e
								}];
							case 3:
								return [3, 5];
							case 4:
								return r = g.sent(), console.warn(r), [3, 5];
							case 5:
								return [2]
						}
					}))
				}))
			}

			function m(A, t, e) {
				var g = new n.a(A),
					i = e.native["ID3v2.4"] || e.native["ID3v2.3"] || e.native["ID3v2.2"] || [];
				i.forEach((function(A) {
					if ("TPE1" !== A.id && "TIT2" !== A.id && "TALB" !== A.id) try {
						g.setFrame(A.id, A.value)
					} catch (t) {
						console.warn("failed to write ID3 tag '".concat(A.id, "'"))
					}
				}));
				var r = e.common;
				return g.setFrame("TPE1", (null === r || void 0 === r ? void 0 : r.artists) || t.artists || []).setFrame("TIT2", (null === r || void 0 === r ? void 0 : r.title) || t.title).setFrame("TALB", (null === r || void 0 === r ? void 0 : r.album) || t.album || ""), t.picture && g.setFrame("APIC", {
					type: 3,
					data: t.picture,
					description: t.picture_desc || ""
				}), g.addTag()
			}

			function b(t, e, g) {
				var i = new I.a(t),
					n = g.common;
				return n.title || n.album || !n.artists || (i.setTag("TITLE=" + e.title), i.setTag("ALBUM=" + e.album), e.artists && (i.removeTag("ARTIST"), e.artists.forEach((function(A) {
					return i.setTag("ARTIST=" + A)
				})))), e.picture && i.importPictureFromBuffer(A.from(e.picture)), i.save()
			}

			function p(A, t, e) {
				var g = new n.a(A),
					i = e.native["ID3v2.4"] || e.native["ID3v2.3"] || e.native["ID3v2.2"] || [];
				i.forEach((function(A) {
					if ("TPE1" !== A.id && "TIT2" !== A.id && "TALB" !== A.id && "TPE2" !== A.id && "TCON" !== A.id) try {
						g.setFrame(A.id, A.value)
					} catch (t) {
						throw new Error("failed to write ID3 tag '".concat(A.id, "'"))
					}
				}));
				var r = e.common;
				return g.setFrame("TPE1", (null === t || void 0 === t ? void 0 : t.artists) || r.artists || []).setFrame("TIT2", (null === t || void 0 === t ? void 0 : t.title) || r.title).setFrame("TALB", (null === t || void 0 === t ? void 0 : t.album) || r.album || "").setFrame("TPE2", (null === t || void 0 === t ? void 0 : t.albumartist) || r.albumartist || "").setFrame("TCON", (null === t || void 0 === t ? void 0 : t.genre) || r.genre || []), t.picture && g.setFrame("APIC", {
					type: 3,
					data: t.picture,
					description: t.picture_desc || ""
				}), g.addTag()
			}

			function N(t, e, g) {
				var i = new I.a(t),
					n = g.common;
				return e.title && (n.title && i.removeTag("TITLE"), i.setTag("TITLE=" + e.title)), e.album && (n.album && i.removeTag("ALBUM"), i.setTag("ALBUM=" + e.album)), e.albumartist && (n.albumartist && i.removeTag("ALBUMARTIST"), i.setTag("ALBUMARTIST=" + e.albumartist)), e.artists && (n.artists && i.removeTag("ARTIST"), e.artists.forEach((function(A) {
					return i.setTag("ARTIST=" + A)
				}))), e.genre && (n.genre && i.removeTag("GENRE"), e.genre.forEach((function(A) {
					return i.setTag("GENRE=" + A)
				}))), e.picture && i.importPictureFromBuffer(A.from(e.picture)), i.save()
			}

			function F(A) {
				var t = A.lastIndexOf(".");
				return {
					ext: A.substring(t + 1).toLowerCase(),
					name: A.substring(0, t)
				}
			}
		}).call(this, e("1c35").Buffer)
	},
	cd49: function(A, t, e) {
		"use strict";
		e.r(t);
		e("9e1f"), e("450d");
		var g = e("6ed5"),
			i = e.n(g),
			n = (e("46a1"), e("e5f2")),
			r = e.n(n),
			I = (e("6b30"), e("c284")),
			o = e.n(I),
			a = (e("0c67"), e("299c")),
			B = e.n(a),
			C = (e("b5d8"), e("f494")),
			Q = e.n(C),
			E = (e("560b"), e("dcdc")),
			c = e.n(E),
			s = (e("f225"), e("89a9")),
			u = e.n(s),
			l = (e("f4f9"), e("c2cc")),
			f = e.n(l),
			d = (e("7a0f"), e("0f6c")),
			h = e.n(d),
			w = (e("aaa5"), e("a578")),
			y = e.n(w),
			D = (e("adec"), e("3d2d")),
			m = e.n(D),
			b = (e("bdc7"), e("aa2f")),
			p = e.n(b),
			N = (e("de31"), e("c69e")),
			F = e.n(N),
			v = (e("5466"), e("ecdf")),
			R = e.n(v),
			G = (e("38a0"), e("ad41")),
			k = e.n(G),
			U = (e("10cb"), e("f3ad")),
			S = e.n(U),
			Y = (e("eca7"), e("3787")),
			J = e.n(Y),
			M = (e("425f"), e("4105")),
			L = e.n(M),
			H = (e("a7cc"), e("df33")),
			K = e.n(H),
			x = (e("1951"), e("eedf")),
			j = e.n(x),
			O = (e("acb6"), e("c673")),
			T = e.n(O),
			W = (e("fd71"), e("a447")),
			q = e.n(W),
			Z = (e("e260"), e("e6cf"), e("cca6"), e("a79d"), e("2b0e")),
			X = function() {
				var A = this,
					t = A.$createElement,
					e = A._self._c || t;
				return e("el-container", {
					attrs: {
						id: "app"
					}
				}, [e("el-main", [e("Home")], 1), e("el-footer", {
					attrs: {
						id: "app-footer"
					}
				}, [e("el-row", [e("a", {
					attrs: {
						href: "https://github.com/ix64/unlock-music",
						target: "_blank"
					}
				}, [A._v("音乐解锁")]), A._v("(" + A._s(A.version) + ") ：移除已购音乐的加密保护。 "), e("a", {
					attrs: {
						href: "https://github.com/ix64/unlock-music/wiki/使用提示",
						target: "_blank"
					}
				}, [A._v("使用提示")])]), e("el-row", [A._v(" 目前支持 网易云音乐(ncm), QQ音乐(qmc, mflac, mgg), 酷狗音乐(kgm), 虾米音乐(xm), 酷我音乐(.kwm) "), e("a", {
					attrs: {
						href: "https://github.com/ix64/unlock-music/blob/master/README.md",
						target: "_blank"
					}
				}, [A._v("更多")]), A._v("。 ")]), e("el-row", [e("span", [A._v("Copyright © 2019 - " + A._s((new Date).getFullYear()) + " MengYX")]), A._v(" 音乐解锁使用 "), e("a", {
					attrs: {
						href: "https://github.com/ix64/unlock-music/blob/master/LICENSE",
						target: "_blank"
					}
				}, [A._v("MIT许可协议")]), A._v(" 开放源代码 ")])], 1)], 1)
			}, P = [],
			V = e("1da1"),
			_ = (e("96cf"), e("99af"), e("ab8f")),
			z = e("af85"),
			$ = e("9224"),
			AA = function() {
				var A = this,
					t = A.$createElement,
					e = A._self._c || t;
				return e("div", [e("file-selector", {
					on: {
						error: A.showFail,
						success: A.showSuccess
					}
				}), e("div", {
					attrs: {
						id: "app-control"
					}
				}, [e("el-row", {
					staticClass: "mb-3"
				}, [e("span", [A._v("歌曲命名格式：")]), A._l(A.FilenamePolicies, (function(t) {
					return e("el-radio", {
						key: t.key,
						attrs: {
							label: t.key
						},
						model: {
							value: A.filename_policy,
							callback: function(t) {
								A.filename_policy = t
							},
							expression: "filename_policy"
						}
					}, [A._v(" " + A._s(t.text) + " ")])
				}))], 2), e("el-row", [e("edit-dialog", {
					attrs: {
						show: A.showEditDialog,
						picture: A.editing_data.picture,
						title: A.editing_data.title,
						artist: A.editing_data.artist,
						album: A.editing_data.album,
						albumartist: A.editing_data.albumartist,
						genre: A.editing_data.genre
					},
					on: {
						cancel: function(t) {
							A.showEditDialog = !1
						},
						ok: A.handleEdit
					}
				}), e("config-dialog", {
					attrs: {
						show: A.showConfigDialog
					},
					on: {
						done: function(t) {
							A.showConfigDialog = !1
						}
					}
				}), e("el-tooltip", {
					staticClass: "item",
					attrs: {
						effect: "dark",
						placement: "top"
					}
				}, [e("div", {
					attrs: {
						slot: "content"
					},
					slot: "content"
				}, [e("span", [A._v(" 部分解密方案需要设定解密参数。 ")])]), e("el-button", {
					attrs: {
						icon: "el-icon-s-tools",
						plain: ""
					},
					on: {
						click: function(t) {
							A.showConfigDialog = !0
						}
					}
				}, [A._v("解密设定")])], 1), e("el-button", {
					attrs: {
						icon: "el-icon-download",
						plain: ""
					},
					on: {
						click: A.handleDownloadAll
					}
				}, [A._v("下载全部")]), e("el-button", {
					attrs: {
						icon: "el-icon-delete",
						plain: "",
						type: "danger"
					},
					on: {
						click: A.handleDeleteAll
					}
				}, [A._v("清除全部")]), e("el-tooltip", {
					staticClass: "item",
					attrs: {
						effect: "dark",
						placement: "top-start"
					}
				}, [e("div", {
					attrs: {
						slot: "content"
					},
					slot: "content"
				}, [A.instant_save ? e("span", [A._v("工作模式: " + A._s(A.dir ? "写入本地文件系统" : "调用浏览器下载"))]) : e("span", [A._v(" 当您使用此工具进行大量文件解锁的时候，建议开启此选项。"), e("br"), A._v(" 开启后，解锁结果将不会存留于浏览器中，防止内存不足。 ")])]), e("el-checkbox", {
					staticClass: "ml-2",
					attrs: {
						type: "success",
						border: ""
					},
					model: {
						value: A.instant_save,
						callback: function(t) {
							A.instant_save = t
						},
						expression: "instant_save"
					}
				}, [A._v("立即保存")])], 1)], 1)], 1), e("audio", {
					attrs: {
						autoplay: A.playing_auto,
						src: A.playing_url,
						controls: ""
					}
				}), e("PreviewTable", {
					attrs: {
						policy: A.filename_policy,
						"table-data": A.tableData
					},
					on: {
						download: A.saveFile,
						edit: A.editFile,
						play: A.changePlaying
					}
				})], 1)
			}, tA = [],
			eA = e("321b"),
			gA = eA["a"],
			iA = e("2877"),
			nA = Object(iA["a"])(gA, AA, tA, !1, null, null, null),
			rA = nA.exports,
			IA = e("97e5"),
			oA = {
				name: "app",
				components: {
					FileSelector: _["a"],
					PreviewTable: z["a"],
					Home: rA
				},
				data: function() {
					return {
						version: $.version
					}
				},
				created: function() {
					var A = this;
					this.$nextTick((function() {
						return A.finishLoad()
					}))
				},
				methods: {
					finishLoad: function() {
						var A = this;
						return Object(V["a"])(regeneratorRuntime.mark((function t() {
							var e, g;
							return regeneratorRuntime.wrap((function(t) {
								while (1) switch (t.prev = t.next) {
									case 0:
										return e = document.getElementById("loader-mask"), e && e.remove(), t.prev = 2, t.next = 5, Object(IA["a"])(A.version);
									case 5:
										g = t.sent, t.next = 11;
										break;
									case 8:
										t.prev = 8, t.t0 = t["catch"](2), console.warn("check version info failed", t.t0);
									case 11:
										g && (g.HttpsFound || g.Found && "https:" !== window.location.protocol) ? A.$notify.warning({
											title: "发现更新",
											message: "发现新版本 v".concat(g.Version, "<br/>更新详情：").concat(g.Detail, '<br/> <a target="_blank" href="').concat(g.URL, '">获取更新</a>'),
											dangerouslyUseHTMLString: !0,
											duration: 15e3,
											position: "top-left"
										}) : A.$notify.info({
											title: "QQ音乐解锁失败？",
											message: '<div>\n                        <p>请确保使用的QQ音乐客户端版本为</p>\n                        <br>MacOS: <a target="_blank" href="./qq/QQMusicMac_Mgr.dmg">8.8.0</a><br>Windows: <a target="_blank" href="./qq/QQMusic_Setup_1951.exe">19.51</a> 或更低版本<br><div class="update-info">\n                            <div class="update-title">最近更新</div>\n                            <div class="update-content"> '.concat($.updateInfo, ' </div>\n                        </div>\n                        <!-- <a target="_blank" href="https://github.com/ix64/unlock-music/wiki/使用提示">使用提示</a> -->\n                    </div>'),
											dangerouslyUseHTMLString: !0,
											duration: 1e4,
											position: "top-left"
										});
									case 12:
									case "end":
										return t.stop()
								}
							}), t, null, [
								[2, 8]
							])
						})))()
					}
				}
			}, aA = oA,
			BA = (e("5c0b"), Object(iA["a"])(aA, X, P, !1, null, null, null)),
			CA = BA.exports,
			QA = e("9483");
		"https:" === window.location.protocol && Object(QA["a"])("".concat("", "service-worker.js"), {
			ready: function() {
				console.log("App is being served from cache by a service worker.")
			},
			registered: function() {
				console.log("Service worker has been registered.")
			},
			cached: function() {
				console.log("Content has been cached for offline use.")
			},
			updatefound: function() {
				console.log("New content is downloading.")
			},
			updated: function() {
				console.log("New content is available."), window.location.reload()
			},
			offline: function() {
				console.log("No internet connection found. App is running in offline mode.")
			},
			error: function(A) {
				console.error("Error during service worker registration:", A)
			}
		}), Z["default"].use(q.a), Z["default"].use(T.a), Z["default"].use(j.a), Z["default"].use(K.a), Z["default"].use(L.a), Z["default"].use(J.a), Z["default"].use(S.a), Z["default"].use(k.a), Z["default"].use(R.a), Z["default"].use(F.a), Z["default"].use(p.a), Z["default"].use(m.a), Z["default"].use(y.a), Z["default"].use(h.a), Z["default"].use(f.a), Z["default"].use(u.a), Z["default"].use(c.a), Z["default"].use(Q.a), Z["default"].use(B.a), Z["default"].use(o.a), Z["default"].prototype.$notify = r.a, Z["default"].prototype.$confirm = i.a.confirm, Z["default"].config.productionTip = !1, new Z["default"]({
			render: function(A) {
				return A(CA)
			}
		}).$mount("#app")
	},
	d4ec: function(A, t, e) {
		"use strict";
		var g = function() {
			var A = this,
				t = A.$createElement,
				e = A._self._c || t;
			return e("el-dialog", {
				attrs: {
					title: "音乐标签编辑",
					visible: A.show,
					"custom-class": "um-edit-dialog",
					center: ""
				},
				on: {
					close: function(t) {
						return A.cancel()
					}
				}
			}, [e("el-form", {
				ref: "form",
				attrs: {
					"status-icon": "",
					model: A.form,
					"label-width": "0"
				}
			}, [e("section", [e("el-image", {
				directives: [{
					name: "show",
					rawName: "v-show",
					value: !A.editPicture,
					expression: "!editPicture"
				}],
				staticStyle: {
					width: "100px",
					height: "100px"
				},
				attrs: {
					src: A.imgFile.url || A.picture
				}
			}, [e("div", {
				staticClass: "image-slot el-image__error",
				attrs: {
					slot: "error"
				},
				slot: "error"
			}, [A._v("暂无封面")])]), e("el-upload", {
				directives: [{
					name: "show",
					rawName: "v-show",
					value: A.editPicture,
					expression: "editPicture"
				}],
				attrs: {
					"auto-upload": !1,
					"on-change": A.addFile,
					"on-remove": A.rmvFile,
					"show-file-list": !0,
					limit: 1,
					"list-type": "picture",
					action: "",
					drag: ""
				}
			}, [e("i", {
				staticClass: "el-icon-upload"
			}), e("div", {
				staticClass: "el-upload__text"
			}, [A._v("将新图片拖到此处，或"), e("em", [A._v("点击选择")]), e("br"), A._v("以替换自动匹配的图片")]), e("div", {
				staticClass: "el-upload__tip",
				attrs: {
					slot: "tip"
				},
				slot: "tip"
			}, [A._v(" 新拖到此处的图片将覆盖原始图片 ")])]), e("i", {
				class: {
					"el-icon-edit": !A.editPicture,
					"el-icon-check": A.editPicture
				},
				on: {
					click: A.changeCover
				}
			}), e("br"), A._v(" 标题: "), e("span", {
				directives: [{
					name: "show",
					rawName: "v-show",
					value: !A.editTitle,
					expression: "!editTitle"
				}]
			}, [A._v(A._s(A.title))]), e("el-input", {
				directives: [{
					name: "show",
					rawName: "v-show",
					value: A.editTitle,
					expression: "editTitle"
				}],
				model: {
					value: A.title,
					callback: function(t) {
						A.title = t
					},
					expression: "title"
				}
			}), e("i", {
				class: {
					"el-icon-edit": !A.editTitle,
					"el-icon-check": A.editTitle
				},
				on: {
					click: function(t) {
						A.editTitle = !A.editTitle
					}
				}
			}), e("br"), A._v(" 艺术家: "), e("span", {
				directives: [{
					name: "show",
					rawName: "v-show",
					value: !A.editArtist,
					expression: "!editArtist"
				}]
			}, [A._v(A._s(A.artist))]), e("el-input", {
				directives: [{
					name: "show",
					rawName: "v-show",
					value: A.editArtist,
					expression: "editArtist"
				}],
				model: {
					value: A.artist,
					callback: function(t) {
						A.artist = t
					},
					expression: "artist"
				}
			}), e("i", {
				class: {
					"el-icon-edit": !A.editArtist,
					"el-icon-check": A.editArtist
				},
				on: {
					click: function(t) {
						A.editArtist = !A.editArtist
					}
				}
			}), e("br"), A._v(" 专辑: "), e("span", {
				directives: [{
					name: "show",
					rawName: "v-show",
					value: !A.editAlbum,
					expression: "!editAlbum"
				}]
			}, [A._v(A._s(A.album))]), e("el-input", {
				directives: [{
					name: "show",
					rawName: "v-show",
					value: A.editAlbum,
					expression: "editAlbum"
				}],
				model: {
					value: A.album,
					callback: function(t) {
						A.album = t
					},
					expression: "album"
				}
			}), e("i", {
				class: {
					"el-icon-edit": !A.editAlbum,
					"el-icon-check": A.editAlbum
				},
				on: {
					click: function(t) {
						A.editAlbum = !A.editAlbum
					}
				}
			}), e("br"), A._v(" 专辑艺术家: "), e("span", {
				directives: [{
					name: "show",
					rawName: "v-show",
					value: !A.editAlbumartist,
					expression: "!editAlbumartist"
				}]
			}, [A._v(A._s(A.albumartist))]), e("el-input", {
				directives: [{
					name: "show",
					rawName: "v-show",
					value: A.editAlbumartist,
					expression: "editAlbumartist"
				}],
				model: {
					value: A.albumartist,
					callback: function(t) {
						A.albumartist = t
					},
					expression: "albumartist"
				}
			}), e("i", {
				class: {
					"el-icon-edit": !A.editAlbumartist,
					"el-icon-check": A.editAlbumartist
				},
				on: {
					click: function(t) {
						A.editAlbumartist = !A.editAlbumartist
					}
				}
			}), e("br"), A._v(" 风格: "), e("span", {
				directives: [{
					name: "show",
					rawName: "v-show",
					value: !A.editGenre,
					expression: "!editGenre"
				}]
			}, [A._v(A._s(A.genre))]), e("el-input", {
				directives: [{
					name: "show",
					rawName: "v-show",
					value: A.editGenre,
					expression: "editGenre"
				}],
				model: {
					value: A.genre,
					callback: function(t) {
						A.genre = t
					},
					expression: "genre"
				}
			}), e("i", {
				class: {
					"el-icon-edit": !A.editGenre,
					"el-icon-check": A.editGenre
				},
				on: {
					click: function(t) {
						A.editGenre = !A.editGenre
					}
				}
			}), e("br"), e("p", {
				staticClass: "item-desc"
			}, [A._v(" 为了节省您设备的资源，请在确定前充分检查，避免反复修改。"), e("br"), A._v(" 直接关闭此对话框不会保留所作的更改。 ")])], 1)]), e("span", {
				staticClass: "dialog-footer",
				attrs: {
					slot: "footer"
				},
				slot: "footer"
			}, [e("el-button", {
				attrs: {
					type: "primary"
				},
				on: {
					click: function(t) {
						return A.emitConfirm()
					}
				}
			}, [A._v("确 定")])], 1)], 1)
		}, i = [],
			n = e("1da1"),
			r = (e("d3b7"), e("3ca3"), e("ddb0"), e("2b3d"), e("9861"), e("96cf"), e("77b3")),
			I = {
				components: {
					Ruby: r["a"]
				},
				props: {
					show: {
						type: Boolean,
						required: !0
					},
					picture: {
						type: void 0 | String,
						required: !0
					},
					title: {
						type: void 0 | String,
						required: !0
					},
					artist: {
						type: void 0 | String,
						required: !0
					},
					album: {
						type: void 0 | String,
						required: !0
					},
					albumartist: {
						type: void 0 | String,
						required: !0
					},
					genre: {
						type: void 0 | String,
						required: !0
					}
				},
				data: function() {
					return {
						form: {},
						imgFile: {
							tmpblob: void 0,
							blob: void 0,
							url: void 0
						},
						editPicture: !1,
						editTitle: !1,
						editArtist: !1,
						editAlbum: !1,
						editAlbumartist: !1,
						editGenre: !1
					}
				},
				mounted: function() {
					var A = this;
					return Object(n["a"])(regeneratorRuntime.mark((function t() {
						return regeneratorRuntime.wrap((function(t) {
							while (1) switch (t.prev = t.next) {
								case 0:
									A.refreshForm();
								case 1:
								case "end":
									return t.stop()
							}
						}), t)
					})))()
				},
				methods: {
					addFile: function(A) {
						this.imgFile.tmpblob = A.raw
					},
					rmvFile: function() {
						this.imgFile.tmpblob = void 0
					},
					changeCover: function() {
						this.editPicture = !this.editPicture, !this.editPicture && this.imgFile.tmpblob && (this.imgFile.blob = this.imgFile.tmpblob, this.imgFile.url && URL.revokeObjectURL(this.imgFile.url), this.imgFile.url = URL.createObjectURL(this.imgFile.blob))
					},
					refreshForm: function() {
						var A = this;
						return Object(n["a"])(regeneratorRuntime.mark((function t() {
							return regeneratorRuntime.wrap((function(t) {
								while (1) switch (t.prev = t.next) {
									case 0:
										A.imgFile.url && URL.revokeObjectURL(A.imgFile.url), A.imgFile = {
											tmpblob: void 0,
											blob: void 0,
											url: void 0
										}, A.editPicture = !1, A.editTitle = !1, A.editArtist = !1, A.editAlbum = !1, A.editAlbumartist = !1, A.editGenre = !1;
									case 8:
									case "end":
										return t.stop()
								}
							}), t)
						})))()
					},
					cancel: function() {
						var A = this;
						return Object(n["a"])(regeneratorRuntime.mark((function t() {
							return regeneratorRuntime.wrap((function(t) {
								while (1) switch (t.prev = t.next) {
									case 0:
										A.refreshForm(), A.$emit("cancel");
									case 2:
									case "end":
										return t.stop()
								}
							}), t)
						})))()
					},
					emitConfirm: function() {
						var A = this;
						return Object(n["a"])(regeneratorRuntime.mark((function t() {
							return regeneratorRuntime.wrap((function(t) {
								while (1) switch (t.prev = t.next) {
									case 0:
										A.editPicture && A.changeCover(), A.imgFile.url && URL.revokeObjectURL(A.imgFile.url), A.$emit("ok", {
											picture: A.imgFile.blob,
											title: A.title,
											artist: A.artist,
											album: A.album,
											albumartist: A.albumartist,
											genre: A.genre
										});
									case 3:
									case "end":
										return t.stop()
								}
							}), t)
						})))()
					}
				}
			}, o = I,
			a = (e("1708"), e("2877")),
			B = Object(a["a"])(o, g, i, !1, null, "b553147e", null);
		t["a"] = B.exports
	},
	efc0: function(A, t, e) {
		"use strict";
		e("d3b7"), e("ddb0"), e("4ec9"), e("3ca3"), e("159b"), e("4fad");
		var g = e("9ab4"),
			i = e("675e"),
			n = function(A) {
				function t() {
					var t = null !== A && A.apply(this, arguments) || this;
					return t.values = new Map, t
				}
				return Object(g["b"])(t, A), t.prototype.load = function(A, t) {
					return Object(g["a"])(this, void 0, Promise, (function() {
						return Object(g["c"])(this, (function(e) {
							return this.values.has(A) ? [2, this.values.get(A)] : [2, t]
						}))
					}))
				}, t.prototype.save = function(A, t) {
					return Object(g["a"])(this, void 0, Promise, (function() {
						return Object(g["c"])(this, (function(e) {
							return this.values.set(A, t), [2]
						}))
					}))
				}, t.prototype.getAll = function() {
					return Object(g["a"])(this, void 0, Promise, (function() {
						var A;
						return Object(g["c"])(this, (function(t) {
							return A = {}, this.values.forEach((function(t, e) {
								var g;
								Object.assign(A, (g = {}, g[e] = t, g))
							})), [2, A]
						}))
					}))
				}, t.prototype.setAll = function(A) {
					return Object(g["a"])(this, void 0, Promise, (function() {
						var t, e, i, n, r;
						return Object(g["c"])(this, (function(g) {
							for (t = 0, e = Object.entries(A); t < e.length; t++) i = e[t], n = i[0], r = i[1], this.values.set(n, r);
							return [2]
						}))
					}))
				}, t
			}(i["b"]);
		t["a"] = n
	},
	fa01: function(A, t, e) {
		"use strict";
		(function(A) {
			e.d(t, "a", (function() {
				return r
			}));
			e("ace4"), e("d3b7"), e("5cc6"), e("907a"), e("9a8c"), e("a975"), e("735e"), e("c1ac"), e("d139"), e("3a7b"), e("d5d6"), e("82f8"), e("e91f"), e("60bd"), e("5f96"), e("3280"), e("3fcc"), e("ca91"), e("25a1"), e("cd26"), e("3c5d"), e("2954"), e("649e"), e("219c"), e("170b"), e("b39a"), e("72f7"), e("fb6a");
			var g = e("08fe"),
				i = 2,
				n = 7;

			function r(t) {
				var e = new TextDecoder,
					g = A.from(e.decode(t), "base64"),
					i = g.length;
				if (i < 16) throw Error("key length is too short");
				g = B(g);
				for (var n = I(106, 8), r = new Uint8Array(16), o = 0; o < 8; o++) r[o << 1] = n[o], r[1 + (o << 1)] = g[o];
				var a = C(g.subarray(8), r);
				return g.set(a, 8), g.subarray(0, 8 + a.length)
			}

			function I(A, t) {
				for (var e = [], g = 0; g < t; g++) {
					var i = Math.tan(A + .1 * g);
					e[g] = 255 & 100 * Math.abs(i)
				}
				return e
			}
			var o = new Uint8Array([51, 56, 54, 90, 74, 89, 33, 64, 35, 42, 36, 37, 94, 38, 41, 40]),
				a = new Uint8Array([42, 42, 35, 33, 40, 35, 36, 37, 38, 94, 97, 49, 99, 90, 44, 84]);

			function B(t) {
				var e = new TextDecoder;
				if (t.length < 18 || "QQMusic EncV2,Key:" !== e.decode(t.slice(0, 18))) return t;
				var g = C(t.slice(18), o);
				g = C(g, a);
				var i = new TextDecoder,
					n = A.from(i.decode(g), "base64"),
					r = n.length;
				if (r < 16) throw Error("EncV2 key decode failed");
				return n
			}

			function C(A, t) {
				if (A.length % 8 != 0) throw Error("inBuf size not a multiple of the block size");
				if (A.length < 16) throw Error("inBuf size too small");
				var e = new g["a"](t, 32),
					r = new Uint8Array(8),
					I = new DataView(r.buffer);
				e.decrypt(I, new DataView(A.buffer, A.byteOffset, 8));
				for (var o = 7 & r[0], a = A.length - 1 - o - i - n, B = new Uint8Array(a), C = new Uint8Array(8), Q = A.slice(0, 8), E = 8, c = 1 + o, s = function() {
						C = Q, Q = A.slice(E, E + 8);
						for (var t = 0; t < 8; t++) r[t] ^= Q[t];
						e.decrypt(I, I), E += 8, c = 0
					}, u = 1; u <= i;) c < 8 ? (c++, u++) : s();
				var l = 0;
				while (l < a) c < 8 ? (B[l] = r[c] ^ C[c], l++, c++) : s();
				for (u = 1; u <= n; u++)
					if (r[c] != C[c]) throw Error("zero check failed");
				return B
			}
		}).call(this, e("1c35").Buffer)
	}
});